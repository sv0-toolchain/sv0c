...
| C → native | `cc` + run each pilot `main` (**`lib/self-host-sv0-loop.list`** — **23** parser / lexer / lib / IR seeds that link as a single TU today; expand only after `emit-c` + `cc` + run succeeds for each path). |
