...
| C → native | `cc` + run each pilot `main` for **every non-comment line** in **`lib/self-host-sv0-loop.list`** (pilot set is a **superset** of vm-parity + integration + golden-pass paths that link as one TU — **not** necessarily equal to **`bootstrap-sources.list`** line count). Recompute list length with: `grep -cve '^\s*$\|^\s*#' sv0c/lib/self-host-sv0-loop.list`. |
