...
- **D1b UNBLOCKED — IR ADTs converged (2026-07-31).** Chose fix path B: `vm_codegen.sv0`'s `Value`/`Expr` now match `lowering.sv0` exactly (same variants + order — `VBoolTrue`/`VBoolFalse` not `VBool(bool)`; dropped the unused `VFloat`/`IndexAccess`), so the VM backend consumes lower's boxed IR directly — mirroring SML's single shared `Ir`. A **second drift** surfaced and is also fixed: lower stores the operator as an **AST binop/unop tag** (the flat i32 IR can't inline SML's `Ir.Binop of string`), but `emit_expr` was resolving it as a **source token span** (`binop_handle_to_insn`); added self-contained `binop_tag_to_insn`/`unop_tag_to_insn` (same tag numbering as lowering's `ast_binop_to_c`/`ast_unop_to_c`) and switched `emit_expr` to them. **Result: `build/sv0-megatu-vm-native` emits byte-identical `.sv0b` vs the SML `--target=vm` reference across the expression surface** — verified `cmp`-clean on arithmetic (`1+2*3-4`), logic (`&&`), unary (`-`), and bitwise (`& | << `) programs. Full `sv0 test` green (refreshed vm_codegen stage0 + its own vm-parity golden); C-backend corpus stays 98/98. Updated `test_emit_expr` operand values from token handles to AST tags. **Remaining for D2:** programs with **locals / calls / while-loops** emit `emit_program < 0` (compose-main exit 5) — the VM slot-allocation path (`local_count_and_env`) vs lower's `DeclNamed`/`Assign`/`Store`/`Call` encoding has its own convergence gap (SML produces these fine: letvar 87B, call 135B, whilelp 108B). That is the next slice.
- **D2 locals/calls/loops emit gap CLOSED (2026-07-31).** Root cause: `lookup_slot` keyed the VM slot env on the raw token index, but a source variable carries a *different* token handle at its declaration vs each use site (lower emits `VVar`/`Assign` with the use-site token) — so `x` in `x + 1` (token 15) missed the slot registered for the `let x` decl (token 8), yielding `emit_program < 0` (compose exit 5). The C backend resolves variables by `handle_to_str`; the VM emitter now does too: `slot_name_eq` compares two handles by source string when **both** index the token arena, else by integer — which also correctly discriminates freshTmp temps (offset past the arena → integer identity) from real vars (in-arena → name identity), with no test changes. Threaded `source/starts/ends` through `lookup_slot`/`emit_value`/`emit_ret_width`/`emit_member_load_from_var` and the env-build chain (`local_count_and_env`→`scan_instrs_env`→`scan_instr_env`→`ensure_var_int`). **Result: locals, calls, params, while-loops (simple + nested), re-assignment, and if-else statements all emit byte-identical `.sv0b` vs SML** — `let`-locals (87 B), multi-local (97 B), calls (135 B), `while i<3` (108 B), `x=x+10` (87 B), if-else (128 B), nested-while (134 B). Full `sv0 test` green; C corpus 98/98.

  Three follow-on fixes landed on top of the slot fix to reach byte-parity on loops/assignment/if:
  1. **Faithful `lower_expr_with_instrs` (`lowering.sv0`).** It was an unfaithful transliteration (`lower_expr_to_value` + `value_to_expr`, spilling a top-level binop to a temp); now a binop RHS returns `Ir.Binop(op, vl, vr)` directly, matching SML's `lowerExprWithInstrs`. Pointed tag-18 `ExprAssign` and the `IfElse` condition (`lower_cond_expr`, same helper the `while` uses) at the faithful path, so `x = x+10` / `if x>3` no longer spill an extra slot. C-safe (tag-19 `+=` already produced `Store(x, Binop)`); corpus stays 98/98.
  2. **Jump opcodes (`vm_codegen.sv0`).** WhileLoop + IfElse emitted `JUMP_IF` (113); SML uses `JUMP_IF_NOT` (114) for the branch-past-body/else. Fixed both, and updated the stale `test_emit_while_loop`/`test_emit_ifelse_block` assertions (they hard-coded 113; a failing assert made vm_codegen.sv0's own corpus run exit non-zero).
  3. **Negative LE encoding (`bytecode.sv0`).** `le_byte1/2/3` tried to fix negatives by adding `256*256*256*256`, which overflows i32 to 0 (no-op), and truncating division of a negative yields 0 for the high bytes — so a backward loop-jump offset like −37 encoded `db 00 00 00` instead of `db ff ff ff`. Rewrote as bitwise `(val >> 8) & 255` (arithmetic shift; correct for negatives, identical for non-negatives).

  **if-as-value slot ordering — CLOSED (2026-07-31).** `let y = if c { a } else { b }` was same-size but differed in slot *numbers*: sv0's let lowering emitted `DeclVar(y)` **before** lowering the initializer, so `y` grabbed the lower slot; SML's `lowerStmt StmtLet` is `instrs @ [Store(x, rhs)]` — init first, variable declared after, so `y`'s slot comes after the if-result temps. Moved `DeclVar(name_tok)` to after the init lowering in **both** let-lowering paths (`lower_stmt` — the active fn-body path — and `lower_tag_block`), guarded so a struct/enum/string special-case `DeclNamed` still comes first. C-safe (declaration order is free in C; corpus stays 98/98). Now byte-identical vs SML for `let y = if x>0 {10} else {20}` (with and without a type annotation). The scalar + control-flow + if-as-value surface is fully byte-clean.
- **D2. Byte-exact convergence on the 4-program tier-2 manifest — `lib/bytecode.sv0` DONE (2026-07-31).** Ran `build/sv0-megatu-vm-native` on all four tier programs vs the SML `--target=vm` reference. **`bytecode.sv0` is byte-identical.** Two fixes got it there:
  1. **String pool (value-keyed + unquoted).** SML's `poolAdd` dedups by string VALUE and `VString` holds the already-parsed literal; sv0 pooled raw token HANDLES and deduped by handle, so a literal `"main"` kept its quotes AND stayed distinct from the `main` function name (+1 string, wrong indices). Added `pool_resolve`/`pool_key_eq` (dedup by decoded value; `slot_handle_in_range` guard + empty-source fallback for the synthetic tests) and `decode_pool_string`/`decode_one_escape` in `encode_strings` (strip quotes + interpret escapes, mirroring the lexer). Threaded `source/starts/ends` through `pool_add` + its callers.
  2. **Effect lowering for statement ifs.** A bare `if c {…} else {…}` statement (e.g. inside a loop body) was lowered through the VALUE path (`lower_tag_if`), allocating two dead result slots per if. Routed `lower_tag_block`'s SemiStmt through `lower_for_effect` (matching `lower_stmt` and SML `lowerStmt StmtSemi`), and gave `lower_for_effect` a real `ExprIf` case (`IfElse(ec, effect-then, effect-else)`, no temps) mirroring SML `lowerExprForEffect`. C corpus stays 98/98.
- **Struct field layout IMPLEMENTED — `span.sv0` EMITS + byte-close (2026-07-31).** The panic was: struct field name→offset layout (`env_fields_flat`) was never populated, so `StoreField`/`FieldAccess` on a struct local read `vec_get(empty, …)`. Built the full flat struct model matching SML (`widthOfCty struct = field count`, each field 1 slot, offset = `indexOfField`):
  - **String-based type/field matching.** `width_of_cty`, `index_of_field_range` now compare by resolved name (`slot_name_eq`) — a struct type/field token at a declaration differs from its definition token. Empty-source + in-range fallbacks keep the synthetic unit tests integer-keyed.
  - **Field-layout population.** New `struct_fields_build` (per-struct field-name flat + starts, from the parser's `sfn`/`id3`) + `record_field_layout` (append a struct local's field names to `env_fields_flat`, return its `env_field_starts`). Called from `scan_instr_env` (DeclNamed, Call-dst) and the `local_count_and_env` param loop. Threaded `struct_fields_flat`/`struct_field_starts`/`env_fields_flat` through the env-build chain and `struct_field_name_toks`/`item_field_bases` into `emit_program` (+ the VM tail passes `sfn`/`id3`).
  - **Field resolution unified.** `emit_member_load_from_var` (read) + `StoreField` (write) now resolve a struct field token via `index_of_field_range`, with the enum sentinel (`<0`) + empty-layout paths falling back to `member_offset_from_field`. Threaded `env_field_starts`/`env_fields_flat` through `emit_value`/`emit_expr`.

  **Result: span emits (7834 B, same as SML) with no regression** (scalar matrix, bytecode byte-identical, C corpus 98/98). Remaining span diff: **`span_bogus` only** — `let z: Pos = pos_new(…)` (a struct-returning-**call** let) declares `z` *before* the init temp, but SML's general `let` path (`instrs @ [DeclNamed, Store]`) declares it *after*, so `z` gets a different base slot. Same class as the if-as-value decl-after-init fix: the `lower_call_ret_user_ty_tok` special case in `lowering.sv0` (lower_stmt ~2964, lower_tag_block ~590) emits `DeclNamed` before the init; it should defer the decl to after the init like the general path. A clean, separable lowering-order follow-up.
- **`lexer/token.sv0` + `lib/ast.sv0` (emit_program < 0) — NEXT after span_bogus.** These add **enums** + **`match`**: enum layout (`tag`/`p0…` synthetic fields, `CONSTRUCT_VARIANT`/`GET_TAG`/`GET_VARIANT_FIELD` opcodes) and match→VM lowering, plus struct-param widths in `compute_arity` (now string-matched — verify multi-slot params emit right).
