...
- **D1. Native VM emit path — SCOPED; blocked on a binary-write primitive (2026-07-27).** The emit chain is `vm_codegen.sv0 emit_program` (item arena → VM function-table entries with code vecs, `lib/vm_codegen.sv0:1476`) → `bytecode.sv0 encode_file(str_sec, str_sec_len, ft_entries, out)` → `out` is a `Vec<i32>` of BYTES (header + string section + function table + code; SML `main.sml writeBytes` uses `BinIO`). **Blocker: `sv0_write_file` writes a C string via `strlen(contents)`, which truncates at the first NUL — and `.sv0b` is NUL-heavy (span golden: 4565 of 7834 bytes are NUL; the header `SV0B\x01\x00…` NULs at byte 5).** So a native VM emitter needs a NEW runtime primitive `sv0_write_bytes(path, byte_vec, n)` (fwrite the vec length, not strlen), registered as an sv0 builtin (id 18, next after read_dir=17: `lowering build_builtin_map` + `resolver is_intrinsic`/`intrinsic_arity` + emit `megatu_builtin_name`), plus a VM compose main (mirror `megaTU-main.sv0` but `... → lower → vm_codegen.emit_program → bytecode.encode_file → write_bytes(path, bytes)`). Then wire the emitter contract (`SV0_VM_BYTECODE_EMITTER <rel>` → `build/vm/<stem>.sv0b`). This is the substantive D1 unit (runtime + builtin + VM main); build `write_bytes` and the VM main together (write_bytes has no consumer until the VM main exists).
