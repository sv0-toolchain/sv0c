...
**B3. CI wiring.** The behavioral gate is already in `ci.yml` (the corpus-parity step). Remaining: add a `self-host-native.yml` step that runs `build-sv0-megatu-native.sh` + the native behavioral loop (emit+cc+run via `build/sv0-megatu-compiler-native`, **not** the byte-diff leg). Promoting the native binary to the *default* `SV0_SELF_HOST_COMPILER` is deferred: that default feeds the byte-diff loop, so it would need either a behavioral-mode loop flag or accepting the diff — a separate decision from shipping the recipe.
