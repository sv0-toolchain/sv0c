...
- **D1b. VM compose main — SCOPED, next (2026-07-27).** A VM compose main mirroring `megaTU-main.sv0` (same tokenize→parse→resolve→check→**lower**) but with the VM tail: `vm_codegen.emit_program(item-arena, blocks, source, starts, ends, out_pool, out_ft)` → `bytecode.encode_strings(out_pool, source, starts, ends, str_buf)` → `bytecode.encode_file(str_buf, str_len, out_ft, out)` → `write_bytes(path, out)`. **Bridge needed:** `lower(...)` returns `out_blocks` as interleaved boxed quads (`[label, ret, params-box, instrs-box]` per `idx*4`, params EMPTY — the C emit reconstructs them via `megatu_param_list`), but `emit_program` wants **4 parallel vecs** (`block_labels`, `block_param_names`, `block_param_ctys`, `block_instrs`). So D1b must unpack `out_blocks` → labels + `box_deref` instrs, and **reconstruct per-block param name/cty vecs** from the parse arenas (the same reconstruction the C `megatu_param_list` does, but as vecs not a C string). Then a native recipe (mirror `build-sv0-megatu-native.sh`) → `build/sv0-megatu-vm-native` writing `build/vm/<stem>.sv0b`, wire the `SV0_VM_BYTECODE_EMITTER` contract, and D2 byte-exact convergence vs golden (use `scripts/sv0b-hexdiff.sh`).
