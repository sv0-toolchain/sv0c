...
| VM backend | `sml/backend/vm/` | `lib/vm_codegen.sv0` (layout, opcodes, pool, builtins map, **fixed width_of_cty** (proper layout lookup), **enum field helpers**, **layout_lookup**), `lib/bytecode.sv0` (50 opcodes, sizes, LE helpers, file layout) | **`emitValue`/`emitExpr`/`emitInstr`/`emitBlock`** + byte-vector encode/decode |
