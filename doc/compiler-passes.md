...
| E0448  | Assignment to a binding that was not introduced with **`let mut`** (immutable **`let`**, including assigning to **`p.f`** when **`p`** is not **`let mut`**). |
| E0449  | Assignment **left-hand side** is not a **simple name** or **one-level field** **`p.f`** in this slice, or the **base** of **`p.f`** is not **struct**-typed. |
| E0450  | **Compound assignment** (`+=`, …): operator class is not **arithmetic** in this slice (defensive; the parser’s assign-op set is arithmetic/bit-shift today). |
...
- **`no_alias`:** Emits **`sv0_no_alias`** (pointer inequality; approximation only). See [`sv0_runtime.h`](../runtime/sv0_runtime.h). **VM:** arguments are **`Ir.VAddrOf`** for **`&param`**; codegen pushes each binding’s **local base slot** as **`i32`**, then **`CALL_BUILTIN` 1** (interpreter compares for inequality and leaves **`bool`** for **`CONTRACT_CHECK`**).
...
**Block parsing:** A `while` / `for` / `loop` / `if` / `match`, or a nested `{ ... }` **block expression** used as a statement, may omit **`;`** after the closing **`}`** before the next statement (see **`exprEndsWithBracedBlock`** in `parser.sml` and **`sv0doc/grammar/sv0.ebnf`** `braced_ctrl_stmt`). Other expression statements still require **`;`** (e.g. `foo();`, `x + 1;`).
