...
4. **Env (name resolution)** — `sml/name_resolution/env.*` → `lib/env_core.sv0` (bounded module-value register + lookup); `lib/env_scope_core.sv0` (`enterScope` / `bindLocal` / `exitScope` slice, two frames × two locals); `lib/resolver_value_core.sv0` (three-slot `lookup_value` / path check); `lib/resolver_arity_core.sv0` (`registerFnArity` / `lookupFnArity` / call-arity check).
5. **AST** — `sml/ast/*`
6. **Parser** — `sml/parser/*`
