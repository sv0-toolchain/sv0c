...
| `sv0c/lexer/`, `sv0c/parser/`, … | **sv0** pipeline modules (same names as under `sml/`); parser seeds include `expr_entry_core`, `item_entry_core`, `stmt_entry_core`, `assign_lhs_core`, `assign_binop_core`, `expr_rhs_stub_core`, `try_assign_stmt_core` (see **§6**) |
