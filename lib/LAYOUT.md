...
5. **AST** — `sml/ast/*`
6. **Parser** — `sml/parser/*` → `parser/expr_entry_core.sv0` (**parsePrimaryExpr** dispatch + lexer bridges); `parser/item_entry_core.sv0` (**parseItem** top-level dispatch + **`tag_kw_fn`** bridge); `parser/stmt_entry_core.sv0` (**tryStmt** first-token order + **`tag_kw_let`** / assert stand-in + assign fallback arm); `parser/assign_lhs_core.sv0` (**parseAssignTarget** shapes + **`tryParseAssignLHS`** entry / assign-tail; uses **`tag_delim_dot`**/**`tag_delim_eq`** **18**/**19**)
