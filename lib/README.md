...
Two-level numeric model for assignment **RHS** (not full **`parseExpr`**): **`*` (22)** binds tighter than **`+` (20)**; atoms **40** / **73**; optional unary **`-` (21)** at **t0** (**`parseUnaryExpr`**-style) before the primary. **`pratt_from_atom`** implements the prior six-token **expr** + **`;`** shape; **`pratt_rhs_ok_with_semi`** delegates to it or shifts after one **21**. Fits **` - lit ;`**, **` - lit + lit ;`**, **` - lit * lit ;`**; **` - lit + lit * lit ;`** needs a seventh slot for **`;`** (rejected in this stub). **`try_assign_stmt_core`** duplicates both helpers.
...
**tryAssignStmt** in **`parser.sml`**: after **`tryParseAssignLHS`**, **`assign_op_follows_lhs`** (**`isAssignTok`**), then **`rhs_stub_ok_with_semi`** for RHS + **`;`**. Covers atom, **`ident`**, **`1 + 2`**-style RHS (via **`try_assign_id_op_rhs_stub`**), **`*x = …`** with atom RHS and **`try_assign_deref_op_rhs_stub`** for **`*x = 1 + 2`**, **field** atom RHS plus **`try_assign_field_op_rhs_stub`** for **`a.b = 1 + 2`**, **index** (**`ident [ int_lit ]`**, tags **16** / **17**) via **`try_assign_index_op_rhs_stub`** for **`a[i] = 1 + 2`**, **`+=`** with binop RHS, **`try_assign_*_op_pratt_rhs_stub`** (six-token **`pratt_rhs_ok_with_semi`** with **`pratt_from_atom`**, optional unary **21**, duplicated from **`expr_pratt_stub_core`** — includes **`x = -1;`**, **`+= -1*2;`** on deref LHS), and rejects **missing `;`**, **`==` (24)**, truncated **`+`**, non-deref where **`STAR`** was expected, **`EQ` where DOT was expected** between field path segments, **DOT where LBRACKET was expected** after **`ident`**, truncated Pratt RHS (including unary **` - lit + lit * lit`** without a seventh **`;`** slot). Extend when real **`parseExpr`** is transliterated.
