...
**tryAssignStmt** in **`parser.sml`**: after **`tryParseAssignLHS`**, **`assign_op_follows_lhs`** (**`isAssignTok`**), then **`rhs_stub_ok_with_semi`** for RHS + **`;`**. Covers atom, **`ident`**, **`1 + 2`**-style RHS (via **`try_assign_id_op_rhs_stub`**), **`*x = …`** with atom RHS and **`try_assign_deref_op_rhs_stub`** for **`*x = 1 + 2`**, **field** atom RHS plus **`try_assign_field_op_rhs_stub`** for **`a.b = 1 + 2`**, **index** (**`ident [ int_lit ]`**, tags **16** / **17**) via **`try_assign_index_op_rhs_stub`** for **`a[i] = 1 + 2`**, **`+=`** with binop RHS, **`try_assign_id_op_pratt_rhs_stub`** for **`x = 1 + 2 * 3`** / **`+= 1 * 2 + 3`** (six-token **`pratt_rhs_ok_with_semi`**, duplicated from **`expr_pratt_stub_core`**), and rejects **missing `;`**, **`==` (24)**, truncated **`+`**, non-deref where **`STAR`** was expected, **`EQ` where DOT was expected** between field path segments, **DOT where LBRACKET was expected** after **`ident`**, truncated Pratt RHS. Extend when real **`parseExpr`** is transliterated.
