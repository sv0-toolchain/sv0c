...
static int struct_packed_count(int v);
static int struct_packed_width(int v);
...
static int field_cat_at(int fields_flat, int start, int idx);
...
static int locate_member(Value v, int env_names, int env_bases, int env_widths, int env_field_starts, int env_fields_flat, const char* source, int starts, int ends, int out);
static int locate_local(int si, int env_widths, int env_field_starts, int env_fields_flat, int out);
static int locate_out_new(void);
...
static int operand_cat(Value v, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int env_fields_flat, const char* source, int starts, int ends);
static int expr_cat(Expr e, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int env_fields_flat, const char* source, int starts, int ends);
...
static int ensure_var_cat(int name, int cat, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int next_slot, const char* source, int starts, int ends);
static int member_arg_width(Value v, int env_names, int env_bases, int env_widths, int env_field_starts, int env_fields_flat, const char* source, int starts, int ends);
...
static int alloc_struct_slot(int x, int loc, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int env_fields_flat, int next_slot);
static int declvar_struct_init(int instrs, int i, int env_names, int env_bases, int env_widths, int env_field_starts, int env_fields_flat, const char* source, int starts, int ends, int out);
static int scan_instr_env(Instr ins, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int next_slot, int structs_names, int structs_field_counts, int enums_names, int enums_widths, int structs_fields_flat, int structs_field_starts, int env_fields_flat, int blabels, int bretcats, const char* source, int starts, int ends);
...
static int emit_fn(int label, int param_names, int param_ctys, int instrs, int structs_names, int structs_field_counts, int enums_names, int enums_widths, int structs_fields_flat, int structs_field_starts, int pool, int blabels, int bretcats, const char* source, int starts, int ends, int fn_names, int ft_entries);
...
static int emit_program(int item_tags, int item_names, int item_field_counts, int item_variant_max_payload, int item_field_bases, int struct_field_name_toks, int struct_field_cats, int block_labels, int block_param_names, int block_param_ctys, int block_instrs, int block_ret_cats, const char* source, int starts, int ends, int out_pool, int out_ft);
...
static int width_of_cty(int cty_handle, int structs_names, int structs_field_counts, int enums_names, int enums_widths, const char* source, int starts, int ends) {
  ...
  while ((si < sn)) {
    ...
    if (_sv0t2) {
      ...
      int _sv0t4 = struct_packed_width(_sv0t3);
      return _sv0t4;
    } else {
    }
    si = (si + 1);
  }
  int _sv0t5 = sv0_vec_len(enums_names);
  int en = _sv0t5;
  int ei = 0;
  while ((ei < en)) {
    int _sv0t6 = sv0_vec_get(enums_names, ei);
    int _sv0t7 = slot_name_eq(_sv0t6, cty_handle, source, starts, ends);
    if (_sv0t7) {
      int _sv0t8 = sv0_vec_get(enums_widths, ei);
      return _sv0t8;
    } else {
    }
    ei = (ei + 1);
...
static int cty_cat_of_handle(int cty_h, const char* source, int starts, int ends) {
  ...
  int _sv0t8 = sv0_string_eq(s, "u32");
  if (_sv0t8) {
    return 4;
  } else {
  }
...
static int index_of_field_range(int fields_flat, int start, int count, int name, const char* source, int starts, int ends) {
  ...
  while ((i < count)) {
    int _sv0t0 = (2 * i);
    int _sv0t1 = (start + _sv0t0);
    int _sv0t2 = sv0_vec_get(fields_flat, _sv0t1);
    int _sv0t3 = slot_name_eq(_sv0t2, name, source, starts, ends);
    if (_sv0t3) {
      return i;
    } else {
    }
...
static int field_cat_at(int fields_flat, int start, int idx) {
  ...
  int _sv0t1 = (start + _sv0t0);
  int p = (_sv0t1 + 1);
  int _sv0t2 = sv0_vec_len(fields_flat);
  if ((p >= _sv0t2)) {
    return 0;
  } else {
  }
  int _sv0t3 = sv0_vec_get(fields_flat, p);
  int _sv0t4 = (_sv0t3 & 15);
  return _sv0t4;
}
...
static int struct_width_rec(int k, int counts_raw, int fields_flat, int field_starts, int depth) {
  ...
  int _sv0t0 = sv0_vec_len(field_starts);
  if ((k >= _sv0t0)) {
    return 1;
  } else {
  }
  int _sv0t1 = sv0_vec_get(counts_raw, k);
  int _sv0t2 = struct_packed_count(_sv0t1);
  int cnt = _sv0t2;
  if ((depth > 12)) {
    return cnt;
  } else {
  }
  int _sv0t3 = sv0_vec_get(field_starts, k);
  int base = _sv0t3;
  int w = 0;
  int j = 0;
  while ((j < cnt)) {
...
static int struct_run_pairs(int k, int counts_raw, int fields_flat, int field_starts, int depth) {
  ...
  while ((j < cnt)) {
    ...
    if ((sty >= 0)) {
      int _sv0t9 = (n + 1);
      int _sv0t10 = (depth + 1);
      int _sv0t11 = struct_run_pairs(sty, counts_raw, fields_flat, field_starts, _sv0t10);
      n = (_sv0t9 + _sv0t11);
    } else {
    }
    j = (j + 1);
...
static int struct_run_write(int k, int counts_raw, int fields_flat, int field_starts, int out, int depth) {
  ...
  while ((j < cnt)) {
    int _sv0t4 = (2 * j);
    int _sv0t5 = (base + _sv0t4);
    int _sv0t6 = sv0_vec_get(fields_flat, _sv0t5);
    int name = _sv0t6;
    int _sv0t7 = (2 * j);
    int _sv0t8 = (base + _sv0t7);
    int _sv0t9 = (_sv0t8 + 1);
    int _sv0t10 = sv0_vec_get(fields_flat, _sv0t9);
    int raw = _sv0t10;
...
static int struct_widths_pack(int counts, int fields_flat, int field_starts) {
  ...
  k = 0;
  while ((k < n)) {
    int _sv0t3 = sv0_vec_get(counts, k);
    int c = _sv0t3;
    int _sv0t4 = sv0_vec_get(ws, k);
    if ((_sv0t4 != c)) {
      int _sv0t5 = sv0_vec_get(ws, k);
      int _sv0t6 = (_sv0t5 << 12);
      int _sv0t7 = (c | _sv0t6);
      sv0_vec_set(counts, k, _sv0t7);
    } else {
    }
    k = (k + 1);
  }
...
static int record_field_layout(int cty, int structs_names, int structs_field_counts, int struct_fields_flat, int struct_field_starts, const char* source, int starts, int ends, int env_fields_flat) {
  ...
  while ((k < ns)) {
    ...
    if (_sv0t2) {
      ...
      int _sv0t4 = struct_has_nested(k, structs_field_counts, struct_fields_flat, struct_field_starts);
      if (_sv0t4) {
        int _sv0t5 = struct_run_pairs(k, structs_field_counts, struct_fields_flat, struct_field_starts, 0);
        sv0_vec_push(env_fields_flat, _sv0t5);
        int _sv0t6 = run_hdr_magic();
      ...
      int _sv0t9 = sv0_vec_len(env_fields_flat);
      int fstart = _sv0t9;
      int _sv0t10 = sv0_vec_get(structs_field_counts, k);
      int _sv0t11 = struct_packed_count(_sv0t10);
      int fcount = _sv0t11;
      int _sv0t12 = sv0_vec_get(struct_field_starts, k);
      int sbase = _sv0t12;
      int j = 0;
      while ((j < fcount)) {
        int _sv0t13 = (2 * j);
        int _sv0t14 = (sbase + _sv0t13);
        int _sv0t15 = sv0_vec_get(struct_fields_flat, _sv0t14);
        sv0_vec_push(env_fields_flat, _sv0t15);
        int _sv0t16 = (2 * j);
        int _sv0t17 = (sbase + _sv0t16);
        int _sv0t18 = (_sv0t17 + 1);
        int _sv0t19 = sv0_vec_get(struct_fields_flat, _sv0t18);
        int _sv0t20 = (_sv0t19 & 15);
        sv0_vec_push(env_fields_flat, _sv0t20);
        j = (j + 1);
      }
      return fstart;
    } else {
...
static int run_copy(int flat, int rs, int t) {
  sv0_vec_push(flat, t);
  int _sv0t0 = run_hdr_magic();
  sv0_vec_push(flat, _sv0t0);
  int _sv0t1 = sv0_vec_len(flat);
  int nstart = _sv0t1;
  int i = 0;
  while (1) {
    int _sv0t2 = (2 * t);
    ...
    int _sv0t3 = (rs + i);
    int _sv0t4 = sv0_vec_get(flat, _sv0t3);
...
static int struct_fields_build(int item_tags, int item_field_counts, int item_field_bases, int struct_field_name_toks, int struct_field_cats, int out_fields_flat, int out_field_starts) {
  ...
  while ((i < n)) {
    ...
    if ((_sv0t1 == 1)) {
      ...
      while ((k < fc)) {
        ...
        int fcat = 0;
        int _sv0t7 = (base + k);
        int _sv0t8 = sv0_vec_len(struct_field_cats);
        if ((_sv0t7 < _sv0t8)) {
          int _sv0t9 = (base + k);
          int _sv0t10 = sv0_vec_get(struct_field_cats, _sv0t9);
          fcat = _sv0t10;
        } else {
        }
        sv0_vec_push(out_fields_flat, fcat);
        k = (k + 1);
      }
    } else {
    }
  ...
  int _sv0t11 = sv0_vec_len(out_field_starts);
  return _sv0t11;
}
...
static int binop_tag_to_insn_typed(int tag, int cat) {
  ...
  int _sv0t6 = binop_tag_to_insn(tag);
  return _sv0t6;
}
...
static int locate_member(Value v, int env_names, int env_bases, int env_widths, int env_field_starts, int env_fields_flat, const char* source, int starts, int ends, int out) {
  ...
  if ((v.tag == 3)) {
    int x = v.p0;
    int _sv0t39 = lookup_slot(env_names, env_bases, env_widths, x, source, starts, ends);
    int si = _sv0t39;
    if ((si < 0)) {
      int _sv0t40 = (0 - 1);
      return _sv0t40;
    ...
    int _sv0t41 = locate_local(si, env_widths, env_field_starts, env_fields_flat, out);
    return _sv0t41;
    _sv0t0 = 0;
  } else {
    if ((v.tag == 7)) {
      int x = v.p0;
      int _sv0t36 = lookup_slot(env_names, env_bases, env_widths, x, source, starts, ends);
      int si2 = _sv0t36;
      if ((si2 < 0)) {
        int _sv0t37 = (0 - 1);
        return _sv0t37;
      ...
      int _sv0t38 = locate_local(si2, env_widths, env_field_starts, env_fields_flat, out);
      return _sv0t38;
      _sv0t0 = 0;
    } else {
      if ((v.tag == 6)) {
        ...
        _sv0t3.p1 = _sv0t6;
        Value inner;
        inner.tag = (_sv0t3).tag;
        inner.p0 = (_sv0t3).p0;
        inner.p1 = (_sv0t3).p1;
        int _sv0t7 = locate_member(inner, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends, out);
        int li = _sv0t7;
        if ((li < 0)) {
          int _sv0t8 = (0 - 1);
          return _sv0t8;
        ...
        int off0 = _sv0t9;
        int _sv0t10 = sv0_vec_get(out, 2);
        int rs = _sv0t10;
        int _sv0t11 = sv0_vec_get(out, 3);
        int t = _sv0t11;
        int p = 0;
        int acc = 0;
        while ((p < t)) {
          ...
          int e = (rs + _sv0t12);
          int _sv0t13 = (e + 1);
          int _sv0t14 = sv0_vec_len(env_fields_flat);
          if ((_sv0t13 >= _sv0t14)) {
            int _sv0t15 = (0 - 1);
            return _sv0t15;
          } else {
          }
          int _sv0t16 = sv0_vec_get(env_fields_flat, e);
          int name = _sv0t16;
          int _sv0t17 = (e + 1);
          int _sv0t18 = sv0_vec_get(env_fields_flat, _sv0t17);
          int raw = _sv0t18;
          int ew = (raw >> 5);
          if ((ew < 1)) {
            ew = 1;
          ...
          int _sv0t19 = (raw >> 4);
          int hs = (_sv0t19 & 1);
          int _sv0t20 = slot_name_eq(name, f, source, starts, ends);
          if (_sv0t20) {
            int _sv0t21 = (off0 + acc);
            sv0_vec_set(out, 1, _sv0t21);
            sv0_vec_set(out, 4, ew);
            int _sv0t22 = (raw & 15);
            sv0_vec_set(out, 5, _sv0t22);
            sv0_vec_set(out, 6, hs);
            if ((hs == 1)) {
              int _sv0t23 = (e + 3);
              int _sv0t24 = sv0_vec_len(env_fields_flat);
              if ((_sv0t23 >= _sv0t24)) {
                int _sv0t25 = (0 - 1);
                return _sv0t25;
              } else {
              ...
              int _sv0t26 = (e + 4);
              sv0_vec_set(out, 2, _sv0t26);
              int _sv0t27 = (e + 2);
              int _sv0t28 = sv0_vec_get(env_fields_flat, _sv0t27);
              sv0_vec_set(out, 3, _sv0t28);
            } else {
              sv0_vec_set(out, 2, e);
              sv0_vec_set(out, 3, 0);
            }
          ...
          if ((hs == 1)) {
            int _sv0t29 = (e + 3);
            int _sv0t30 = sv0_vec_len(env_fields_flat);
            if ((_sv0t29 >= _sv0t30)) {
              int _sv0t31 = (0 - 1);
              return _sv0t31;
            } else {
            ...
            int _sv0t32 = (p + 2);
            int _sv0t33 = (e + 2);
            int _sv0t34 = sv0_vec_get(env_fields_flat, _sv0t33);
            p = (_sv0t32 + _sv0t34);
          } else {
            p = (p + 1);
          }
...
static int locate_local(int si, int env_widths, int env_field_starts, int env_fields_flat, int out) {
  int _sv0t0 = slot_width(env_widths, si);
  int w = _sv0t0;
  int fs = 0;
  int _sv0t1 = sv0_vec_len(env_field_starts);
  if ((si < _sv0t1)) {
    int _sv0t2 = sv0_vec_get(env_field_starts, si);
    fs = _sv0t2;
  } else {
  }
  int t = w;
  if ((fs >= 2)) {
    ...
    if ((_sv0t3 < _sv0t4)) {
      int _sv0t5 = (fs - 1);
      int _sv0t6 = sv0_vec_get(env_fields_flat, _sv0t5);
      int _sv0t7 = run_hdr_magic();
      if ((_sv0t6 == _sv0t7)) {
        int _sv0t8 = (fs - 2);
        int _sv0t9 = sv0_vec_get(env_fields_flat, _sv0t8);
        t = _sv0t9;
      } else {
      }
    } else {
  ...
  sv0_vec_set(out, 0, si);
  sv0_vec_set(out, 1, 0);
  sv0_vec_set(out, 2, fs);
  sv0_vec_set(out, 3, t);
  sv0_vec_set(out, 4, w);
  sv0_vec_set(out, 5, 0);
  sv0_vec_set(out, 6, 1);
...
static int member_cat(int bv, int f, int env_names, int env_bases, int env_widths, int env_field_starts, int env_fields_flat, const char* source, int starts, int ends) {
  ...
  mv.p0 = bv;
  mv.p1 = f;
  int _sv0t0 = locate_out_new();
  int lo = _sv0t0;
  int _sv0t1 = locate_member(mv, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends, lo);
  int r = _sv0t1;
  if ((r < 0)) {
    return 0;
  } else {
  }
  int _sv0t2 = sv0_vec_get(lo, 5);
  return _sv0t2;
}
...
static int operand_cat(Value v, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int env_fields_flat, const char* source, int starts, int ends) {
  ...
  if ((v.tag == 8)) {
    ...
    if ((v.tag == 9)) {
      ...
      if ((v.tag == 3)) {
        int x = v.p0;
        int _sv0t2 = lookup_slot(env_names, env_bases, env_widths, x, source, starts, ends);
        int si = _sv0t2;
        int _sv0t3 = slot_cat(env_cats, si);
        return _sv0t3;
        _sv0t0 = 0;
      } else {
        if ((v.tag == 0)) {
              ...
              if ((v.tag == 4)) {
                ...
                if ((v.tag == 5)) {
                  ...
                  if ((v.tag == 6)) {
                    int bv = v.p0;
                    int f = v.p1;
                    int _sv0t1 = member_cat(bv, f, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends);
                    return _sv0t1;
                    _sv0t0 = 0;
                  } else {
                    if ((v.tag == 7)) {
...
static int expr_cat(Expr e, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int env_fields_flat, const char* source, int starts, int ends) {
  ...
  if ((e.tag == 0)) {
    int bv = e.p0;
    Value _sv0t20;
    int _sv0t21 = sv0_box_load(bv, 0);
    _sv0t20.tag = _sv0t21;
    int _sv0t22 = sv0_box_load(bv, 1);
    _sv0t20.p0 = _sv0t22;
    int _sv0t23 = sv0_box_load(bv, 2);
    _sv0t20.p1 = _sv0t23;
    Value lv;
    lv.tag = (_sv0t20).tag;
    lv.p0 = (_sv0t20).p0;
    lv.p1 = (_sv0t20).p1;
    int _sv0t24 = operand_cat(lv, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, source, starts, ends);
    return _sv0t24;
    _sv0t0 = 0;
  } else {
    if ((e.tag == 1)) {
      int x = e.p0;
      int _sv0t18 = lookup_slot(env_names, env_bases, env_widths, x, source, starts, ends);
      int si = _sv0t18;
      int _sv0t19 = slot_cat(env_cats, si);
      return _sv0t19;
      _sv0t0 = 0;
    } else {
      if ((e.tag == 2)) {
        ...
        Value _sv0t7;
        int _sv0t8 = sv0_box_load(bv1, 0);
        _sv0t7.tag = _sv0t8;
        int _sv0t9 = sv0_box_load(bv1, 1);
        _sv0t7.p0 = _sv0t9;
        int _sv0t10 = sv0_box_load(bv1, 2);
        _sv0t7.p1 = _sv0t10;
        Value v1;
        v1.tag = (_sv0t7).tag;
        v1.p0 = (_sv0t7).p0;
        v1.p1 = (_sv0t7).p1;
        Value _sv0t11;
        int _sv0t12 = sv0_box_load(bv2, 0);
        _sv0t11.tag = _sv0t12;
        int _sv0t13 = sv0_box_load(bv2, 1);
        _sv0t11.p0 = _sv0t13;
        int _sv0t14 = sv0_box_load(bv2, 2);
        _sv0t11.p1 = _sv0t14;
        Value v2;
        v2.tag = (_sv0t11).tag;
        v2.p0 = (_sv0t11).p0;
        v2.p1 = (_sv0t11).p1;
        int _sv0t15 = operand_cat(v1, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, source, starts, ends);
        int c1 = _sv0t15;
        ...
        if ((e.tag == 3)) {
          ...
          uv.p0 = (_sv0t2).p0;
          uv.p1 = (_sv0t2).p1;
          int _sv0t6 = operand_cat(uv, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, source, starts, ends);
          return _sv0t6;
          _sv0t0 = 0;
        } else {
          if ((e.tag == 4)) {
  ...
  if ((v.tag == 0)) {
    ...
    if ((v.tag == 1)) {
      ...
      if ((v.tag == 2)) {
        ...
        if ((v.tag == 5)) {
          int s = v.p0;
          int _sv0t48 = pool_add(pool, s, source, starts, ends);
          int idx = _sv0t48;
          sv0_vec_push(out, 8);
          ...
          if ((v.tag == 4)) {
            ...
            if ((v.tag == 3)) {
              int x = v.p0;
              int _sv0t42 = lookup_slot(env_names, env_bases, env_widths, x, source, starts, ends);
              int si = _sv0t42;
              if ((si < 0)) {
                int _sv0t43 = (0 - 1);
                return _sv0t43;
              ...
              int _sv0t44 = slot_base(env_bases, si);
              int base = _sv0t44;
              int _sv0t45 = slot_width(env_widths, si);
              int width = _sv0t45;
              int k = 0;
              while ((k < width)) {
                sv0_vec_push(out, 96);
              ...
              if ((v.tag == 6)) {
                ...
                int f = v.p1;
                if ((f >= 0)) {
                  int _sv0t17 = locate_out_new();
                  int lo = _sv0t17;
                  int _sv0t18 = locate_member(v, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends, lo);
                  int lr = _sv0t18;
                  if ((lr >= 0)) {
                    int _sv0t19 = sv0_vec_get(lo, 0);
                    int _sv0t20 = slot_base(env_bases, _sv0t19);
                    int lbase = _sv0t20;
                    int _sv0t21 = sv0_vec_get(lo, 4);
                    int lw = _sv0t21;
                    int lk = 0;
                    while ((lk < lw)) {
                      sv0_vec_push(out, 96);
                      int _sv0t22 = sv0_vec_get(lo, 1);
                      int _sv0t23 = (lbase + _sv0t22);
                      int _sv0t24 = (_sv0t23 + lk);
                      sv0_vec_push(out, _sv0t24);
                      lk = (lk + 1);
                    }
                    int _sv0t25 = (lw * 2);
                ...
                Value _sv0t26;
                int _sv0t27 = sv0_box_load(bv, 0);
                _sv0t26.tag = _sv0t27;
                int _sv0t28 = sv0_box_load(bv, 1);
                _sv0t26.p0 = _sv0t28;
                int _sv0t29 = sv0_box_load(bv, 2);
                _sv0t26.p1 = _sv0t29;
                Value base_v;
                base_v.tag = (_sv0t26).tag;
                base_v.p0 = (_sv0t26).p0;
                base_v.p1 = (_sv0t26).p1;
                int _sv0t30;
                if ((base_v.tag == 3)) {
                  int x = base_v.p0;
                  int _sv0t40 = emit_member_load_from_var(x, f, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends, out);
                  return _sv0t40;
                  _sv0t30 = 0;
                } else {
                  if ((base_v.tag == 0)) {
                ...
                if ((v.tag == 7)) {
                  int x = v.p0;
                  int _sv0t14 = lookup_slot(env_names, env_bases, env_widths, x, source, starts, ends);
                  int si = _sv0t14;
                  if ((si < 0)) {
                    int _sv0t15 = (0 - 1);
                    return _sv0t15;
                  ...
                  int _sv0t16 = slot_base(env_bases, si);
                  int base = _sv0t16;
                  sv0_vec_push(out, 4);
                  ...
                  if ((v.tag == 8)) {
                    int h = v.p0;
                    int _sv0t2 = slot_handle_in_range(h, starts);
                    if ((_sv0t2 != 1)) {
                      int _sv0t3 = (0 - 1);
                      return _sv0t3;
                    } else {
                    }
                    const char* _sv0t4 = handle_to_str(h, source, starts, ends);
                    const char* fs;
                    fs = _sv0t4;
                    int _sv0t5 = sv0_vec_new();
                    int fdg = _sv0t5;
                    ...
                    int fmt = _sv0t6;
                    int _sv0t7 = parse_f64_literal(fs, fdg, fmt);
                    if ((_sv0t7 != 0)) {
                      int _sv0t8 = (0 - 1);
                      return _sv0t8;
                    } else {
                    ...
                    sv0_vec_push(out, 6);
                    int _sv0t9 = sv0_vec_get(fmt, 0);
                    int _sv0t10 = sv0_vec_get(fmt, 1);
                    int _sv0t11 = f64_bits_from_parts(_sv0t9, fdg, _sv0t10, out);
                    int fnb = _sv0t11;
                    if ((fnb < 0)) {
                      int _sv0t12 = (0 - 1);
                      return _sv0t12;
...
static int emit_expr(Expr e, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int env_fields_flat, int pool, const char* source, int starts, int ends, int out) {
  ...
  if ((e.tag == 0)) {
    int bv = e.p0;
    Value _sv0t64;
    int _sv0t65 = sv0_box_load(bv, 0);
    _sv0t64.tag = _sv0t65;
    int _sv0t66 = sv0_box_load(bv, 1);
    _sv0t64.p0 = _sv0t66;
    int _sv0t67 = sv0_box_load(bv, 2);
    _sv0t64.p1 = _sv0t67;
    Value v;
    v.tag = (_sv0t64).tag;
    v.p0 = (_sv0t64).p0;
    v.p1 = (_sv0t64).p1;
    int _sv0t68 = emit_value(v, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
    return _sv0t68;
    _sv0t0 = 0;
  } else {
    if ((e.tag == 1)) {
      int x = e.p0;
      int _sv0t58 = lookup_slot(env_names, env_bases, env_widths, x, source, starts, ends);
      int si = _sv0t58;
      if ((si < 0)) {
        int _sv0t59 = (0 - 1);
        return _sv0t59;
      ...
      if ((e.tag == 2)) {
        ...
        int bv2 = e.p2;
        Value _sv0t35;
        int _sv0t36 = sv0_box_load(bv1, 0);
        _sv0t35.tag = _sv0t36;
        int _sv0t37 = sv0_box_load(bv1, 1);
        _sv0t35.p0 = _sv0t37;
        int _sv0t38 = sv0_box_load(bv1, 2);
        _sv0t35.p1 = _sv0t38;
        Value v1;
        v1.tag = (_sv0t35).tag;
        v1.p0 = (_sv0t35).p0;
        v1.p1 = (_sv0t35).p1;
        Value _sv0t39;
        int _sv0t40 = sv0_box_load(bv2, 0);
        _sv0t39.tag = _sv0t40;
        int _sv0t41 = sv0_box_load(bv2, 1);
        _sv0t39.p0 = _sv0t41;
        int _sv0t42 = sv0_box_load(bv2, 2);
        _sv0t39.p1 = _sv0t42;
        Value v2;
        v2.tag = (_sv0t39).tag;
        v2.p0 = (_sv0t39).p0;
        v2.p1 = (_sv0t39).p1;
        int _sv0t43 = operand_cat(v1, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, source, starts, ends);
        int c1 = _sv0t43;
        int _sv0t44 = operand_cat(v2, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, source, starts, ends);
        int c2 = _sv0t44;
        int _sv0t45 = binop_emit_cat(op_h, c1, c2, v1, v2);
        int bcat = _sv0t45;
        int _sv0t46 = emit_value(v1, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
        int n1 = _sv0t46;
        if ((n1 < 0)) {
          int _sv0t47 = (0 - 1);
          return _sv0t47;
        ...
        int _sv0t48 = u32_operand_needs_zext(c1, bcat, op_h);
        if (_sv0t48) {
          int _sv0t49 = emit_u32_zext(out);
          n1 = (n1 + _sv0t49);
        } else {
        }
        int _sv0t50 = emit_value(v2, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
        int n2 = _sv0t50;
        if ((n2 < 0)) {
          int _sv0t51 = (0 - 1);
          return _sv0t51;
        ...
        int _sv0t52 = u32_operand_needs_zext(c2, bcat, op_h);
        if (_sv0t52) {
          int _sv0t53 = emit_u32_zext(out);
          n2 = (n2 + _sv0t53);
        } else {
        }
        int _sv0t54 = binop_tag_to_insn_typed(op_h, bcat);
        int opc = _sv0t54;
        if ((opc < 0)) {
          int _sv0t55 = (0 - 1);
          return _sv0t55;
        ...
        if ((e.tag == 3)) {
          ...
          v.p0 = (_sv0t26).p0;
          v.p1 = (_sv0t26).p1;
          int _sv0t30 = operand_cat(v, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, source, starts, ends);
          int c = _sv0t30;
          int _sv0t31 = emit_value(v, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
          int n = _sv0t31;
          if ((n < 0)) {
            int _sv0t32 = (0 - 1);
            return _sv0t32;
          ...
          int _sv0t33 = unop_tag_to_insn_typed(op_h, c);
          int opc = _sv0t33;
          if ((opc >= 0)) {
            sv0_vec_push(out, opc);
            int _sv0t34 = (n + 1);
          ...
          if ((e.tag == 4)) {
            ...
            if ((f >= 0)) {
              ...
              fv.p0 = bv;
              fv.p1 = f;
              int _sv0t1 = locate_out_new();
              int flo = _sv0t1;
              int _sv0t2 = locate_member(fv, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends, flo);
              int flr = _sv0t2;
              if ((flr >= 0)) {
                int _sv0t3 = sv0_vec_get(flo, 0);
                int _sv0t4 = slot_base(env_bases, _sv0t3);
                int fbase = _sv0t4;
                int _sv0t5 = sv0_vec_get(flo, 4);
                int fw = _sv0t5;
                int fk = 0;
                while ((fk < fw)) {
                  sv0_vec_push(out, 96);
                  int _sv0t6 = sv0_vec_get(flo, 1);
                  int _sv0t7 = (fbase + _sv0t6);
                  int _sv0t8 = (_sv0t7 + fk);
            ...
            _sv0t10.p1 = _sv0t13;
            Value base_v;
            base_v.tag = (_sv0t10).tag;
            base_v.p0 = (_sv0t10).p0;
            base_v.p1 = (_sv0t10).p1;
            int _sv0t14;
            if ((base_v.tag == 3)) {
              int x = base_v.p0;
              int _sv0t24 = emit_member_load_from_var(x, f, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends, out);
              return _sv0t24;
              _sv0t14 = 0;
            } else {
              if ((base_v.tag == 0)) {
    ...
    if ((ins.tag == 1)) {
      ...
      if ((ins.tag == 2)) {
        ...
        if ((ins.tag == 3)) {
          ...
          int be = ins.p1;
          Expr _sv0t164;
          int _sv0t165 = sv0_box_load(be, 0);
          _sv0t164.tag = _sv0t165;
          int _sv0t166 = sv0_box_load(be, 1);
          _sv0t164.p0 = _sv0t166;
          int _sv0t167 = sv0_box_load(be, 2);
          _sv0t164.p1 = _sv0t167;
          int _sv0t168 = sv0_box_load(be, 3);
          _sv0t164.p2 = _sv0t168;
          Expr e;
          e.tag = (_sv0t164).tag;
          e.p0 = (_sv0t164).p0;
          e.p1 = (_sv0t164).p1;
          e.p2 = (_sv0t164).p2;
          int _sv0t169 = emit_expr(e, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
          int n = _sv0t169;
          if ((n < 0)) {
            int _sv0t170 = (0 - 1);
            return _sv0t170;
          ...
          int _sv0t171 = lookup_slot(env_names, env_bases, env_widths, x, source, starts, ends);
          int si = _sv0t171;
          if ((si < 0)) {
            int _sv0t172 = (0 - 1);
            return _sv0t172;
          ...
          int _sv0t173 = slot_base(env_bases, si);
          int base = _sv0t173;
          int _sv0t174 = slot_width(env_widths, si);
          int width = _sv0t174;
          int k = 0;
          while ((k < width)) {
            sv0_vec_push(out, 97);
            int _sv0t175 = (base + width);
            int _sv0t176 = (_sv0t175 - 1);
            int _sv0t177 = (_sv0t176 - k);
            sv0_vec_push(out, _sv0t177);
            k = (k + 1);
          }
          int _sv0t178 = (width * 2);
          int _sv0t179 = (n + _sv0t178);
          return _sv0t179;
          _sv0t0 = 0;
        } else {
          if ((ins.tag == 4)) {
            ...
            int be = ins.p1;
            Expr _sv0t148;
            int _sv0t149 = sv0_box_load(be, 0);
            _sv0t148.tag = _sv0t149;
            int _sv0t150 = sv0_box_load(be, 1);
            _sv0t148.p0 = _sv0t150;
            int _sv0t151 = sv0_box_load(be, 2);
            _sv0t148.p1 = _sv0t151;
            int _sv0t152 = sv0_box_load(be, 3);
            _sv0t148.p2 = _sv0t152;
            Expr e;
            e.tag = (_sv0t148).tag;
            e.p0 = (_sv0t148).p0;
            e.p1 = (_sv0t148).p1;
            e.p2 = (_sv0t148).p2;
            int _sv0t153 = emit_expr(e, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
            int n = _sv0t153;
            if ((n < 0)) {
              int _sv0t154 = (0 - 1);
              return _sv0t154;
            ...
            int _sv0t155 = lookup_slot(env_names, env_bases, env_widths, x, source, starts, ends);
            int si = _sv0t155;
            if ((si < 0)) {
              int _sv0t156 = (0 - 1);
              return _sv0t156;
            ...
            int _sv0t157 = slot_base(env_bases, si);
            int base = _sv0t157;
            int _sv0t158 = slot_width(env_widths, si);
            int width = _sv0t158;
            int k = 0;
            while ((k < width)) {
              sv0_vec_push(out, 97);
              int _sv0t159 = (base + width);
              int _sv0t160 = (_sv0t159 - 1);
              int _sv0t161 = (_sv0t160 - k);
              sv0_vec_push(out, _sv0t161);
              k = (k + 1);
            }
            int _sv0t162 = (width * 2);
            int _sv0t163 = (n + _sv0t162);
            return _sv0t163;
            _sv0t0 = 0;
          } else {
            if ((ins.tag == 5)) {
              ...
              int be = ins.p2;
              Expr _sv0t119;
              int _sv0t120 = sv0_box_load(be, 0);
              _sv0t119.tag = _sv0t120;
              int _sv0t121 = sv0_box_load(be, 1);
              _sv0t119.p0 = _sv0t121;
              int _sv0t122 = sv0_box_load(be, 2);
              _sv0t119.p1 = _sv0t122;
              int _sv0t123 = sv0_box_load(be, 3);
              _sv0t119.p2 = _sv0t123;
              Expr e;
              e.tag = (_sv0t119).tag;
              e.p0 = (_sv0t119).p0;
              e.p1 = (_sv0t119).p1;
              e.p2 = (_sv0t119).p2;
              int _sv0t124 = emit_expr(e, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
              int ne = _sv0t124;
              if ((ne < 0)) {
                int _sv0t125 = (0 - 1);
                return _sv0t125;
              ...
              int _sv0t126 = lookup_slot(env_names, env_bases, env_widths, bh, source, starts, ends);
              int si = _sv0t126;
              if ((si < 0)) {
                int _sv0t127 = (0 - 1);
                return _sv0t127;
              ...
              int _sv0t128 = slot_base(env_bases, si);
              int b0 = _sv0t128;
              int _sv0t129 = slot_width(env_widths, si);
              int w = _sv0t129;
              if ((fh >= 0)) {
                int _sv0t130 = locate_out_new();
                int slo = _sv0t130;
                Value sm;
                Value _sv0t131;
                _sv0t131.tag = 3;
                ...
                sm.p0 = _sv0t132;
                sm.p1 = fh;
                int _sv0t133 = locate_member(sm, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends, slo);
                int slr = _sv0t133;
                if ((slr >= 0)) {
                  int _sv0t134 = sv0_vec_get(slo, 4);
                  int sw = _sv0t134;
                  int _sv0t135 = sv0_vec_get(slo, 1);
                  int sb = (b0 + _sv0t135);
                  int sk = 0;
                  while ((sk < sw)) {
                    sv0_vec_push(out, 97);
                    int _sv0t136 = (sb + sw);
                    int _sv0t137 = (_sv0t136 - 1);
                    int _sv0t138 = (_sv0t137 - sk);
                    sv0_vec_push(out, _sv0t138);
                    sk = (sk + 1);
                  }
                  int _sv0t139 = (sw * 2);
                  int _sv0t140 = (ne + _sv0t139);
                  return _sv0t140;
                } else {
                }
              } else {
              }
              int _sv0t141 = member_offset_from_field(w, fh);
              int off = _sv0t141;
              if ((fh >= 0)) {
                int _sv0t142 = sv0_vec_len(env_field_starts);
                if ((si < _sv0t142)) {
                  int _sv0t143 = sv0_vec_get(env_field_starts, si);
                  int fs = _sv0t143;
                  int _sv0t144 = index_of_field_range(env_fields_flat, fs, w, fh, source, starts, ends);
                  off = _sv0t144;
                } else {
                }
              } else {
              ...
              if ((ins.tag == 6)) {
                ...
                int el = ins.p2;
                Expr _sv0t97;
                int _sv0t98 = sv0_box_load(be, 0);
                _sv0t97.tag = _sv0t98;
                int _sv0t99 = sv0_box_load(be, 1);
                _sv0t97.p0 = _sv0t99;
                int _sv0t100 = sv0_box_load(be, 2);
                _sv0t97.p1 = _sv0t100;
                int _sv0t101 = sv0_box_load(be, 3);
                _sv0t97.p2 = _sv0t101;
                Expr cond;
                cond.tag = (_sv0t97).tag;
                cond.p0 = (_sv0t97).p0;
                cond.p1 = (_sv0t97).p1;
                cond.p2 = (_sv0t97).p2;
                int _sv0t102 = emit_expr(cond, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
                int nc = _sv0t102;
                if ((nc < 0)) {
                  int _sv0t103 = (0 - 1);
                  return _sv0t103;
                ...
                int then_out = _sv0t104;
                int _sv0t105 = emit_instrs(th, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, pool, source, starts, ends, fn_names, then_out);
                int nt = _sv0t105;
                if ((nt < 0)) {
                  int _sv0t106 = (0 - 1);
                  return _sv0t106;
                ...
                int else_out = _sv0t107;
                int _sv0t108 = emit_instrs(el, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, pool, source, starts, ends, fn_names, else_out);
                int ne = _sv0t108;
                if ((ne < 0)) {
                  int _sv0t109 = (0 - 1);
                  return _sv0t109;
                ...
                int _sv0t110 = flat_byte_size(then_out);
                int _sv0t111 = insn_size(112);
                int off_else = (_sv0t110 + _sv0t111);
                int _sv0t112 = flat_byte_size(else_out);
                int off_end = _sv0t112;
                sv0_vec_push(out, 114);
                sv0_vec_push(out, off_else);
                int _sv0t113 = vec_append(out, then_out);
                int at = _sv0t113;
                sv0_vec_push(out, 112);
                sv0_vec_push(out, off_end);
                int _sv0t114 = vec_append(out, else_out);
                int ae = _sv0t114;
                int _sv0t115 = (nc + 2);
                int _sv0t116 = (_sv0t115 + at);
                int _sv0t117 = (_sv0t116 + 2);
                int _sv0t118 = (_sv0t117 + ae);
                return _sv0t118;
                _sv0t0 = 0;
              } else {
                if ((ins.tag == 7)) {
                  ...
                  int body = ins.p1;
                  Expr _sv0t71;
                  int _sv0t72 = sv0_box_load(be, 0);
                  _sv0t71.tag = _sv0t72;
                  int _sv0t73 = sv0_box_load(be, 1);
                  _sv0t71.p0 = _sv0t73;
                  int _sv0t74 = sv0_box_load(be, 2);
                  _sv0t71.p1 = _sv0t74;
                  int _sv0t75 = sv0_box_load(be, 3);
                  _sv0t71.p2 = _sv0t75;
                  Expr cond;
                  cond.tag = (_sv0t71).tag;
                  cond.p0 = (_sv0t71).p0;
                  cond.p1 = (_sv0t71).p1;
                  cond.p2 = (_sv0t71).p2;
                  int _sv0t76 = sv0_vec_new();
                  int cond_out = _sv0t76;
                  int _sv0t77 = emit_expr(cond, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, pool, source, starts, ends, cond_out);
                  int nc = _sv0t77;
                  if ((nc < 0)) {
                    int _sv0t78 = (0 - 1);
                    return _sv0t78;
                  ...
                  int _sv0t79 = prepool_loop_seq(body, 0, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, pool, source, starts, ends, fn_names);
                  int _sv0t80 = sv0_vec_new();
                  int body_out = _sv0t80;
                  int _sv0t81 = emit_instrs(body, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, pool, source, starts, ends, fn_names, body_out);
                  int nb = _sv0t81;
                  if ((nb < 0)) {
                    int _sv0t82 = (0 - 1);
                    return _sv0t82;
                  ...
                  int _sv0t83 = loop_exit_sentinel();
                  sv0_vec_push(body_out, _sv0t83);
                  int _sv0t84 = flat_byte_size(cond_out);
                  int cond_bytes = _sv0t84;
                  int _sv0t85 = insn_size(114);
                  int jif_bytes = _sv0t85;
                  int _sv0t86 = flat_byte_size(body_out);
                  int body_bytes = _sv0t86;
                  int forward = body_bytes;
                  int _sv0t87 = (cond_bytes + jif_bytes);
                  int _sv0t88 = (_sv0t87 + body_bytes);
                  int back = (0 - _sv0t88);
                  int _sv0t89 = replace_loop_exit_jump(body_out, back);
                  int _sv0t90 = sv0_vec_new();
                  int loop_out = _sv0t90;
                  int _sv0t91 = vec_append(loop_out, cond_out);
                  sv0_vec_push(loop_out, 114);
                  sv0_vec_push(loop_out, forward);
                  int _sv0t92 = vec_append(loop_out, body_out);
                  int _sv0t93 = flat_byte_size(loop_out);
                  int total_bytes = _sv0t93;
                  int _sv0t94 = patch_continue_jumps(loop_out, 0, 0);
                  int _sv0t95 = patch_break_jumps(loop_out, 0, total_bytes);
                  int _sv0t96 = vec_append(out, loop_out);
                  int added = _sv0t96;
                  return added;
                  _sv0t0 = 0;
                } else {
                  if ((ins.tag == 8)) {
                    int body = ins.p0;
                    int _sv0t70 = emit_instrs(body, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, pool, source, starts, ends, fn_names, out);
                    return _sv0t70;
                    _sv0t0 = 0;
                  } else {
                    if ((ins.tag == 9)) {
                      sv0_vec_push(out, 112);
                      int _sv0t69 = loop_break_sentinel();
                      sv0_vec_push(out, _sv0t69);
                      return 2;
                      _sv0t0 = 0;
                    } else {
                      if ((ins.tag == 10)) {
                        sv0_vec_push(out, 112);
                        int _sv0t68 = loop_continue_sentinel();
                        sv0_vec_push(out, _sv0t68);
                        return 2;
                        _sv0t0 = 0;
                      } else {
                        if ((ins.tag == 11)) {
                          ...
                          if ((fn_h < 0)) {
                            ...
                            const char* _sv0t30 = handle_to_str(fn_h, source, starts, ends);
                            const char* fn_name;
                            fn_name = _sv0t30;
                            int _sv0t31 = builtin_id(fn_name);
                            bid = _sv0t31;
                          }
                          if ((bid == 12)) {
                            int _sv0t32 = emit_box_new(dst_h, args, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
                            return _sv0t32;
                          } else {
                          }
                          int _sv0t33 = sv0_vec_len(args);
                          int nargs_vec = _sv0t33;
                          int total = 0;
                          if ((bid >= 0)) {
                            int ai = 0;
                            while ((ai < nargs_vec)) {
                              int _sv0t34 = sv0_vec_get(args, ai);
                              int bv = _sv0t34;
                              Value _sv0t35;
                              int _sv0t36 = sv0_box_load(bv, 0);
                              _sv0t35.tag = _sv0t36;
                              int _sv0t37 = sv0_box_load(bv, 1);
                              _sv0t35.p0 = _sv0t37;
                              int _sv0t38 = sv0_box_load(bv, 2);
                              _sv0t35.p1 = _sv0t38;
                              Value v;
                              v.tag = (_sv0t35).tag;
                              v.p0 = (_sv0t35).p0;
                              v.p1 = (_sv0t35).p1;
                              int _sv0t39 = emit_value(v, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
                              int nv = _sv0t39;
                              if ((nv < 0)) {
                                int _sv0t40 = (0 - 1);
                                return _sv0t40;
                            ...
                            total = (total + 2);
                            if ((dst_h != 0)) {
                              int _sv0t41 = lookup_slot(env_names, env_bases, env_widths, dst_h, source, starts, ends);
                              int si = _sv0t41;
                              if ((si < 0)) {
                                int _sv0t42 = (0 - 1);
                                return _sv0t42;
                          ...
                          int ai = 0;
                          while ((ai < nargs_vec)) {
                            int _sv0t49 = sv0_vec_get(args, ai);
                            int bv = _sv0t49;
                            Value _sv0t50;
                            int _sv0t51 = sv0_box_load(bv, 0);
                            _sv0t50.tag = _sv0t51;
                            int _sv0t52 = sv0_box_load(bv, 1);
                            _sv0t50.p0 = _sv0t52;
                            int _sv0t53 = sv0_box_load(bv, 2);
                            _sv0t50.p1 = _sv0t53;
                            Value v;
                            v.tag = (_sv0t50).tag;
                            v.p0 = (_sv0t50).p0;
                            v.p1 = (_sv0t50).p1;
                            int _sv0t54 = emit_ret_width(v, env_names, env_bases, env_widths, source, starts, ends);
                            int w = _sv0t54;
                            int _sv0t55 = member_arg_width(v, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends);
                            int mw = _sv0t55;
                            if ((mw >= 0)) {
                              w = mw;
                            } else {
                            ...
                            int _sv0t56 = emit_value(v, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
                            int nv = _sv0t56;
                            if ((nv < 0)) {
                              int _sv0t57 = (0 - 1);
                              return _sv0t57;
                          ...
                          int _sv0t58 = index_of_fn(fn_h, fn_names, source, starts, ends);
                          int fix = _sv0t58;
                          if ((fix < 0)) {
                            int _sv0t59 = (0 - 1);
                            return _sv0t59;
                          ...
                          total = (total + 3);
                          if ((dst_h != 0)) {
                            int _sv0t60 = lookup_slot(env_names, env_bases, env_widths, dst_h, source, starts, ends);
                            int si = _sv0t60;
                            if ((si < 0)) {
                              int _sv0t61 = (0 - 1);
                              return _sv0t61;
                            ...
                            int _sv0t62 = slot_base(env_bases, si);
                            int base = _sv0t62;
                            int _sv0t63 = slot_width(env_widths, si);
                            int width = _sv0t63;
                            int k = 0;
                            while ((k < width)) {
                              sv0_vec_push(out, 97);
                              int _sv0t64 = (base + width);
                              int _sv0t65 = (_sv0t64 - 1);
                              int _sv0t66 = (_sv0t65 - k);
                          ...
                          if ((ins.tag == 12)) {
                            ...
                            int fn_h = ins.p1;
                            Expr _sv0t20;
                            int _sv0t21 = sv0_box_load(be, 0);
                            _sv0t20.tag = _sv0t21;
                            int _sv0t22 = sv0_box_load(be, 1);
                            _sv0t20.p0 = _sv0t22;
                            int _sv0t23 = sv0_box_load(be, 2);
                            _sv0t20.p1 = _sv0t23;
                            int _sv0t24 = sv0_box_load(be, 3);
                            _sv0t20.p2 = _sv0t24;
                            Expr e;
                            e.tag = (_sv0t20).tag;
                            e.p0 = (_sv0t20).p0;
                            e.p1 = (_sv0t20).p1;
                            e.p2 = (_sv0t20).p2;
                            int _sv0t25 = emit_expr(e, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
                            int ne = _sv0t25;
                            if ((ne < 0)) {
                              int _sv0t26 = (0 - 1);
                              return _sv0t26;
                            ...
                            int _sv0t27 = contract_pool_key(fn_h, 0);
                            int key = _sv0t27;
                            int _sv0t28 = pool_add(pool, key, source, starts, ends);
                            int pidx = _sv0t28;
                            ...
                            if ((ins.tag == 13)) {
                              ...
                              int fn_h = ins.p1;
                              Expr _sv0t10;
                              int _sv0t11 = sv0_box_load(be, 0);
                              _sv0t10.tag = _sv0t11;
                              int _sv0t12 = sv0_box_load(be, 1);
                              _sv0t10.p0 = _sv0t12;
                              int _sv0t13 = sv0_box_load(be, 2);
                              _sv0t10.p1 = _sv0t13;
                              int _sv0t14 = sv0_box_load(be, 3);
                              _sv0t10.p2 = _sv0t14;
                              Expr e;
                              e.tag = (_sv0t10).tag;
                              e.p0 = (_sv0t10).p0;
                              e.p1 = (_sv0t10).p1;
                              e.p2 = (_sv0t10).p2;
                              int _sv0t15 = emit_expr(e, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
                              int ne = _sv0t15;
                              if ((ne < 0)) {
                                int _sv0t16 = (0 - 1);
                                return _sv0t16;
                              ...
                              int _sv0t17 = contract_pool_key(fn_h, 1);
                              int key = _sv0t17;
                              int _sv0t18 = pool_add(pool, key, source, starts, ends);
                              int pidx = _sv0t18;
                              sv0_vec_push(out, 160);
                              sv0_vec_push(out, pidx);
                              int _sv0t19 = (ne + 2);
                              ...
                              if ((ins.tag == 14)) {
                                ...
                                if ((ins.tag == 15)) {
                                  ...
                                  int w = _sv0t5;
                                  int _sv0t6 = member_arg_width(v, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends);
                                  int mw = _sv0t6;
                                  if ((mw >= 0)) {
                                    w = mw;
                                  } else {
                                  }
                                  int _sv0t7 = emit_value(v, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, pool, source, starts, ends, out);
                                  int n = _sv0t7;
                                  if ((n < 0)) {
                                    int _sv0t8 = (0 - 1);
                                    return _sv0t8;
...
static int ensure_var_int(int name, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int next_slot, const char* source, int starts, int ends) {
  int _sv0t0 = ensure_var_cat(name, 0, env_names, env_bases, env_widths, env_cats, env_field_starts, next_slot, source, starts, ends);
  return _sv0t0;
}
...
static int ensure_var_cat(int name, int cat, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int next_slot, const char* source, int starts, int ends) {
  ...
  if ((si >= 0)) {
    if ((cat != 0)) {
      int _sv0t1 = sv0_vec_len(env_cats);
      if ((si < _sv0t1)) {
        int _sv0t2 = sv0_vec_get(env_cats, si);
        if ((_sv0t2 == 0)) {
          sv0_vec_set(env_cats, si, cat);
        } else {
        }
      } else {
      }
  ...
  sv0_vec_push(env_cats, cat);
  sv0_vec_push(env_field_starts, 0);
  int _sv0t3 = (next_slot + 1);
  return _sv0t3;
}
...
static int member_arg_width(Value v, int env_names, int env_bases, int env_widths, int env_field_starts, int env_fields_flat, const char* source, int starts, int ends) {
  ...
  if ((v.tag == 6)) {
    ...
    int _sv0t3 = locate_out_new();
    int lo = _sv0t3;
    int _sv0t4 = locate_member(v, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends, lo);
    int r = _sv0t4;
    if ((r < 0)) {
      int _sv0t5 = (0 - 1);
      return _sv0t5;
...
static int alloc_struct_slot(int x, int loc, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int env_fields_flat, int next_slot) {
  int _sv0t0 = sv0_vec_get(loc, 4);
  int w = _sv0t0;
  sv0_vec_push(env_names, x);
  sv0_vec_push(env_bases, next_slot);
  sv0_vec_push(env_widths, w);
  sv0_vec_push(env_cats, 0);
  int _sv0t1 = sv0_vec_get(loc, 2);
  int _sv0t2 = sv0_vec_get(loc, 3);
  int _sv0t3 = run_copy(env_fields_flat, _sv0t1, _sv0t2);
  sv0_vec_push(env_field_starts, _sv0t3);
  int _sv0t4 = (next_slot + w);
  return _sv0t4;
}

static int declvar_struct_init(int instrs, int i, int env_names, int env_bases, int env_widths, int env_field_starts, int env_fields_flat, const char* source, int starts, int ends, int out) {
  int _sv0t0 = (i + 1);
  int _sv0t1 = sv0_vec_len(instrs);
  if ((_sv0t0 >= _sv0t1)) {
    int _sv0t2 = (0 - 1);
    return _sv0t2;
  } else {
  }
  int _sv0t3 = sv0_vec_get(instrs, i);
  int bh = _sv0t3;
  Instr _sv0t4;
  int _sv0t5 = sv0_box_load(bh, 0);
  _sv0t4.tag = _sv0t5;
  int _sv0t6 = sv0_box_load(bh, 1);
  _sv0t4.p0 = _sv0t6;
  int _sv0t7 = sv0_box_load(bh, 2);
  _sv0t4.p1 = _sv0t7;
  int _sv0t8 = sv0_box_load(bh, 3);
  _sv0t4.p2 = _sv0t8;
  int _sv0t9 = sv0_box_load(bh, 4);
  _sv0t4.p3 = _sv0t9;
  Instr ins;
  ins.tag = (_sv0t4).tag;
  ins.p0 = (_sv0t4).p0;
  ...
  if ((ins.tag == 1)) {
    ...
    int nb = _sv0t13;
    Instr _sv0t14;
    int _sv0t15 = sv0_box_load(nb, 0);
    _sv0t14.tag = _sv0t15;
    int _sv0t16 = sv0_box_load(nb, 1);
    _sv0t14.p0 = _sv0t16;
    int _sv0t17 = sv0_box_load(nb, 2);
    _sv0t14.p1 = _sv0t17;
    int _sv0t18 = sv0_box_load(nb, 3);
    _sv0t14.p2 = _sv0t18;
    int _sv0t19 = sv0_box_load(nb, 4);
    _sv0t14.p3 = _sv0t19;
    Instr nx;
    nx.tag = (_sv0t14).tag;
    nx.p0 = (_sv0t14).p0;
    ...
    if ((nx.tag == 4)) {
      ...
      Expr _sv0t23;
      int _sv0t24 = sv0_box_load(be, 0);
      _sv0t23.tag = _sv0t24;
      int _sv0t25 = sv0_box_load(be, 1);
      _sv0t23.p0 = _sv0t25;
      int _sv0t26 = sv0_box_load(be, 2);
      _sv0t23.p1 = _sv0t26;
      int _sv0t27 = sv0_box_load(be, 3);
      _sv0t23.p2 = _sv0t27;
      Expr e;
      e.tag = (_sv0t23).tag;
      e.p0 = (_sv0t23).p0;
      e.p1 = (_sv0t23).p1;
      e.p2 = (_sv0t23).p2;
      int _sv0t28 = expr_struct_locate(e, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends, out);
      int r = _sv0t28;
      if ((r < 0)) {
        int _sv0t29 = (0 - 1);
        return _sv0t29;
...
static int scan_instr_env(Instr ins, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int next_slot, int structs_names, int structs_field_counts, int enums_names, int enums_widths, int structs_fields_flat, int structs_field_starts, int env_fields_flat, int blabels, int bretcats, const char* source, int starts, int ends) {
  ...
  if ((ins.tag == 1)) {
    int x = ins.p0;
    int _sv0t41 = ensure_var_int(x, env_names, env_bases, env_widths, env_cats, env_field_starts, next_slot, source, starts, ends);
    return _sv0t41;
    _sv0t0 = 0;
  } else {
    if ((ins.tag == 2)) {
      int cty = ins.p0;
      int x = ins.p1;
      int _sv0t37 = width_of_cty(cty, structs_names, structs_field_counts, enums_names, enums_widths, source, starts, ends);
      int w = _sv0t37;
      sv0_vec_push(env_names, x);
      sv0_vec_push(env_bases, next_slot);
      sv0_vec_push(env_widths, w);
      ...
      if ((ins.tag == 3)) {
        ...
        ae.p1 = (_sv0t26).p1;
        ae.p2 = (_sv0t26).p2;
        int _sv0t31 = locate_out_new();
        int aso = _sv0t31;
        int _sv0t32 = expr_struct_locate(ae, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends, aso);
        if ((_sv0t32 >= 0)) {
          int _sv0t33 = lookup_slot(env_names, env_bases, env_widths, x, source, starts, ends);
          if ((_sv0t33 < 0)) {
            int _sv0t34 = alloc_struct_slot(x, aso, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, next_slot);
            return _sv0t34;
          } else {
          }
          return next_slot;
        ...
        int _sv0t35 = expr_cat(ae, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, source, starts, ends);
        int acat = _sv0t35;
        int _sv0t36 = ensure_var_cat(x, acat, env_names, env_bases, env_widths, env_cats, env_field_starts, next_slot, source, starts, ends);
        return _sv0t36;
        _sv0t0 = 0;
      } else {
        if ((ins.tag == 11)) {
          ...
          int _sv0t14 = width_of_cty(rty, structs_names, structs_field_counts, enums_names, enums_widths, source, starts, ends);
          int w0 = _sv0t14;
          int _sv0t15;
          int _sv0t16;
          if ((w0 < 1)) {
          ...
          sv0_vec_push(env_bases, next_slot);
          sv0_vec_push(env_widths, w);
          int _sv0t17 = cty_cat_of_handle(rty, source, starts, ends);
          int rcat = _sv0t17;
          if ((fn_h < 0)) {
            int _sv0t18 = (0 - fn_h);
            if ((_sv0t18 == 39)) {
          ...
          if ((ins.tag == 6)) {
            ...
            int th = ins.p1;
            int el = ins.p2;
            int _sv0t12 = scan_instrs_env(th, env_names, env_bases, env_widths, env_cats, env_field_starts, next_slot, structs_names, structs_field_counts, enums_names, enums_widths, structs_fields_flat, structs_field_starts, env_fields_flat, blabels, bretcats, source, starts, ends);
            int ns1 = _sv0t12;
            int _sv0t13 = scan_instrs_env(el, env_names, env_bases, env_widths, env_cats, env_field_starts, ns1, structs_names, structs_field_counts, enums_names, enums_widths, structs_fields_flat, structs_field_starts, env_fields_flat, blabels, bretcats, source, starts, ends);
            return _sv0t13;
            _sv0t0 = 0;
          } else {
            if ((ins.tag == 7)) {
              int be = ins.p0;
              int body = ins.p1;
              int _sv0t11 = scan_instrs_env(body, env_names, env_bases, env_widths, env_cats, env_field_starts, next_slot, structs_names, structs_field_counts, enums_names, enums_widths, structs_fields_flat, structs_field_starts, env_fields_flat, blabels, bretcats, source, starts, ends);
              return _sv0t11;
              _sv0t0 = 0;
            } else {
              if ((ins.tag == 8)) {
                int body = ins.p0;
                int _sv0t10 = scan_instrs_env(body, env_names, env_bases, env_widths, env_cats, env_field_starts, next_slot, structs_names, structs_field_counts, enums_names, enums_widths, structs_fields_flat, structs_field_starts, env_fields_flat, blabels, bretcats, source, starts, ends);
                return _sv0t10;
                _sv0t0 = 0;
              } else {
                if ((ins.tag == 0)) {
                  ...
                  if ((ins.tag == 4)) {
                    ...
                    int be = ins.p1;
                    Expr _sv0t1;
                    int _sv0t2 = sv0_box_load(be, 0);
                    _sv0t1.tag = _sv0t2;
                    int _sv0t3 = sv0_box_load(be, 1);
                    _sv0t1.p0 = _sv0t3;
                    int _sv0t4 = sv0_box_load(be, 2);
                    _sv0t1.p1 = _sv0t4;
                    int _sv0t5 = sv0_box_load(be, 3);
                    _sv0t1.p2 = _sv0t5;
                    Expr se;
                    se.tag = (_sv0t1).tag;
                    se.p0 = (_sv0t1).p0;
                    se.p1 = (_sv0t1).p1;
                    se.p2 = (_sv0t1).p2;
                    int _sv0t6 = expr_cat(se, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, source, starts, ends);
                    int scat = _sv0t6;
                    if ((scat != 0)) {
                      int _sv0t7 = lookup_slot(env_names, env_bases, env_widths, x, source, starts, ends);
                      int ssi = _sv0t7;
                      if ((ssi >= 0)) {
                        int _sv0t8 = sv0_vec_len(env_cats);
                        if ((ssi < _sv0t8)) {
                          int _sv0t9 = sv0_vec_get(env_cats, ssi);
                          if ((_sv0t9 == 0)) {
                            sv0_vec_set(env_cats, ssi, scat);
                          } else {
                          }
                        } else {
                        }
...
static int scan_instrs_env(int instrs, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int next_slot, int structs_names, int structs_field_counts, int enums_names, int enums_widths, int structs_fields_flat, int structs_field_starts, int env_fields_flat, int blabels, int bretcats, const char* source, int starts, int ends) {
  ...
  while ((i < n)) {
    int _sv0t1 = locate_out_new();
    int dso = _sv0t1;
    int _sv0t2 = declvar_struct_init(instrs, i, env_names, env_bases, env_widths, env_field_starts, env_fields_flat, source, starts, ends, dso);
    int dvx = _sv0t2;
    if ((dvx >= 0)) {
      int _sv0t3 = lookup_slot(env_names, env_bases, env_widths, dvx, source, starts, ends);
      if ((_sv0t3 < 0)) {
        int _sv0t4 = alloc_struct_slot(dvx, dso, env_names, env_bases, env_widths, env_cats, env_field_starts, env_fields_flat, ns);
        ns = _sv0t4;
      } else {
      }
      i = (i + 1);
    ...
    int _sv0t5 = sv0_vec_get(instrs, i);
    int bh = _sv0t5;
    Instr _sv0t6;
    int _sv0t7 = sv0_box_load(bh, 0);
    _sv0t6.tag = _sv0t7;
    int _sv0t8 = sv0_box_load(bh, 1);
    _sv0t6.p0 = _sv0t8;
    int _sv0t9 = sv0_box_load(bh, 2);
    _sv0t6.p1 = _sv0t9;
    int _sv0t10 = sv0_box_load(bh, 3);
    _sv0t6.p2 = _sv0t10;
    int _sv0t11 = sv0_box_load(bh, 4);
    _sv0t6.p3 = _sv0t11;
    Instr ins;
    ins.tag = (_sv0t6).tag;
    ins.p0 = (_sv0t6).p0;
    ...
    ins.p2 = (_sv0t6).p2;
    ins.p3 = (_sv0t6).p3;
    int _sv0t12 = scan_instr_env(ins, env_names, env_bases, env_widths, env_cats, env_field_starts, ns, structs_names, structs_field_counts, enums_names, enums_widths, structs_fields_flat, structs_field_starts, env_fields_flat, blabels, bretcats, source, starts, ends);
    ns = _sv0t12;
    i = (i + 1);
  }
  return ns;
...
static int local_count_and_env(int param_names, int param_ctys, int instrs, int env_names, int env_bases, int env_widths, int env_cats, int env_field_starts, int structs_names, int structs_field_counts, int enums_names, int enums_widths, int structs_fields_flat, int structs_field_starts, int env_fields_flat, int blabels, int bretcats, const char* source, int starts, int ends) {
  ...
  int _sv0t6 = scan_instrs_env(instrs, env_names, env_bases, env_widths, env_cats, env_field_starts, ns, structs_names, structs_field_counts, enums_names, enums_widths, structs_fields_flat, structs_field_starts, env_fields_flat, blabels, bretcats, source, starts, ends);
  return _sv0t6;
}
...
static int emit_program(int item_tags, int item_names, int item_field_counts, int item_variant_max_payload, int item_field_bases, int struct_field_name_toks, int struct_field_cats, int block_labels, int block_param_names, int block_param_ctys, int block_instrs, int block_ret_cats, const char* source, int starts, int ends, int out_pool, int out_ft) {
  ...
  int fn_names = _sv0t12;
  int fi = 0;
  while ((fi < fc)) {
    int _sv0t13 = sv0_vec_get(func_order, fi);
    int bi = _sv0t13;
    int _sv0t14 = sv0_vec_get(block_labels, bi);
    sv0_vec_push(fn_names, _sv0t14);
    fi = (fi + 1);
  }
  fi = 0;
  while ((fi < fc)) {
    int _sv0t15 = sv0_vec_get(func_order, fi);
    int bi2 = _sv0t15;
    int _sv0t16 = sv0_vec_get(block_param_names, bi2);
    int pn = _sv0t16;
    int _sv0t17 = sv0_vec_get(block_param_ctys, bi2);
    int pc = _sv0t17;
    int _sv0t18 = sv0_vec_get(block_instrs, bi2);
    int ins = _sv0t18;
    int _sv0t19 = sv0_vec_get(block_labels, bi2);
    int lab = _sv0t19;
    int _sv0t20 = emit_fn(lab, pn, pc, ins, sn, sfc, enn, enw, sff, sfs, out_pool, block_labels, block_ret_cats, source, starts, ends, fn_names, out_ft);
    int r = _sv0t20;
    if ((r < 0)) {
      int _sv0t21 = (0 - 1);
      return _sv0t21;
...
static int test_emit_value(void) {
  ...
  int _sv0t107 = emit_value(_sv0t104, en, eb, ew, _sv0t105, _sv0t106, pool, fsrc, fst, fen, out12);
  int r11 = _sv0t107;
  if ((r11 != 3)) {
    return 34;
  } else {
  }
  int _sv0t108 = sv0_vec_get(out12, 0);
  if ((_sv0t108 != 6)) {
    return 35;
  } else {
  }
  int _sv0t109 = sv0_vec_get(out12, 1);
  if ((_sv0t109 != 0)) {
    return 36;
  } else {
  }
  int _sv0t110 = sv0_vec_get(out12, 2);
  if ((_sv0t110 != 1073217536)) {
    return 37;
  } else {
  }
  return 0;
...
static int test_emit_expr(void) {
  ...
  int _sv0t113 = emit_expr(flit, enf, ebf, ewf, ecf, _sv0t111, _sv0t112, pool, fls, flst, flen, out12);
  int r12 = _sv0t113;
  if ((r12 != 7)) {
    return 35;
  } else {
  }
  int _sv0t114 = sv0_vec_get(out12, 0);
  if ((_sv0t114 != 6)) {
    return 36;
  } else {
  }
  int _sv0t115 = sv0_vec_get(out12, 2);
  if ((_sv0t115 != 1074003968)) {
    return 37;
  } else {
  }
  int _sv0t116 = sv0_vec_get(out12, 3);
  if ((_sv0t116 != 6)) {
    return 38;
  } else {
  }
  int _sv0t117 = sv0_vec_get(out12, 6);
  if ((_sv0t117 != 48)) {
    return 39;
  } else {
  }
...
static int test_emit_fn(void) {
  ...
  int _sv0t16 = sv0_vec_new();
  int _sv0t17 = sv0_vec_new();
  int _sv0t18 = emit_fn(42, pn0, pc0, ins0, sn, sfc, enn, enw, _sv0t14, _sv0t15, pool, _sv0t16, _sv0t17, source, starts, ends, fns, ft);
  int r0 = _sv0t18;
  if ((r0 != 0)) {
    return 1;
  } else {
  }
  int _sv0t19 = sv0_vec_len(ft);
  if ((_sv0t19 != 4)) {
    return 2;
  } else {
  }
  int _sv0t20 = sv0_vec_get(ft, 0);
  int ni0 = _sv0t20;
  if ((ni0 != 0)) {
    return 3;
  } else {
  }
  int _sv0t21 = sv0_vec_get(ft, 1);
  if ((_sv0t21 != 0)) {
    return 4;
  } else {
  }
  int _sv0t22 = sv0_vec_get(ft, 2);
  if ((_sv0t22 != 0)) {
    return 5;
  } else {
  }
  int _sv0t23 = sv0_vec_get(ft, 3);
  int code0 = _sv0t23;
  int _sv0t24 = sv0_vec_len(code0);
  if ((_sv0t24 != 2)) {
    return 6;
  } else {
  }
  int _sv0t25 = sv0_vec_get(code0, 0);
  if ((_sv0t25 != 118)) {
    return 7;
  } else {
  }
  int _sv0t26 = sv0_vec_get(code0, 1);
  if ((_sv0t26 != 0)) {
    return 8;
  } else {
  }
  ...
  sv0_box_store(_sv0t35, 0, _sv0t30.tag);
  sv0_box_store(_sv0t35, 1, _sv0t30.p0);
  sv0_box_store(_sv0t35, 2, _sv0t30.p1);
  sv0_box_store(_sv0t35, 3, _sv0t30.p2);
  sv0_box_store(_sv0t35, 4, _sv0t30.p3);
  ...
  sv0_vec_push(ins1, _sv0t39);
  int _sv0t40 = sv0_vec_new();
  int _sv0t41 = sv0_vec_new();
  int _sv0t42 = sv0_vec_new();
  int _sv0t43 = sv0_vec_new();
  int _sv0t44 = emit_fn(43, pn1, pc1, ins1, sn, sfc, enn, enw, _sv0t40, _sv0t41, pool, _sv0t42, _sv0t43, source, starts, ends, fns, ft);
  int r1 = _sv0t44;
  if ((r1 != 0)) {
    return 9;
  } else {
  }
  int _sv0t45 = sv0_vec_len(ft);
  if ((_sv0t45 != 8)) {
    return 10;
  } else {
  }
  int _sv0t46 = sv0_vec_get(ft, 5);
  if ((_sv0t46 != 1)) {
    return 11;
  } else {
  }
  int _sv0t47 = sv0_vec_get(ft, 6);
  int lc1 = _sv0t47;
  if ((lc1 != 2)) {
    return 12;
  } else {
  }
  int _sv0t48 = sv0_vec_get(ft, 7);
  int code1 = _sv0t48;
  int _sv0t49 = sv0_vec_len(code1);
  if ((_sv0t49 < 2)) {
    return 13;
  } else {
  }
  int _sv0t50 = pool_size(pool);
  if ((_sv0t50 != 2)) {
    return 14;
  } else {
  }
  ...
  sv0_box_store(_sv0t57, 1, _sv0t56.p0);
  sv0_box_store(_sv0t57, 2, _sv0t56.p1);
  sv0_box_store(_sv0t57, 3, _sv0t56.p2);
  sv0_box_store(_sv0t57, 4, _sv0t56.p3);
  sv0_vec_push(ins2, _sv0t57);
  ...
  int _sv0t63 = sv0_vec_new();
  int _sv0t64 = emit_fn(44, pn2, pc2, ins2, sn3, sfc3, enn, enw, _sv0t60, _sv0t61, pool2, _sv0t62, _sv0t63, source, starts, ends, fns, ft2);
  int r2 = _sv0t64;
  if ((r2 != 0)) {
    return 15;
  } else {
  }
  int _sv0t65 = sv0_vec_get(ft2, 1);
  if ((_sv0t65 != 3)) {
    return 16;
  } else {
  }
  int _sv0t66 = sv0_vec_get(ft2, 2);
  if ((_sv0t66 != 3)) {
    return 17;
  } else {
  }
  ...
  sv0_box_store(_sv0t71, 3, _sv0t70.p2);
  sv0_box_store(_sv0t71, 4, _sv0t70.p3);
  sv0_vec_push(ins3, _sv0t71);
  int _sv0t72 = sv0_vec_new();
  int _sv0t73 = sv0_vec_new();
  int _sv0t74 = sv0_vec_new();
  int _sv0t75 = sv0_vec_new();
  int _sv0t76 = emit_fn(45, pn3, pc3, ins3, sn, sfc, enn, enw, _sv0t72, _sv0t73, pool2, _sv0t74, _sv0t75, source, starts, ends, fns, ft2);
  int r3 = _sv0t76;
  if ((r3 != 0)) {
    return 18;
  } else {
  }
  int _sv0t77 = sv0_vec_len(ft2);
  if ((_sv0t77 != 8)) {
    return 19;
  } else {
  }
  int _sv0t78 = sv0_vec_get(ft2, 4);
  int _sv0t79 = sv0_vec_get(ft2, 0);
  int _sv0t80 = (_sv0t79 + 1);
  if ((_sv0t78 != _sv0t80)) {
    return 20;
  } else {
  }
...
static int test_local_count_and_env(void) {
  ...
  int _sv0t17 = sv0_vec_new();
  int _sv0t18 = sv0_vec_new();
  int _sv0t19 = local_count_and_env(pn0, pc0, ins0, en0, eb0, ew0, ec0, efs0, sn, sfc, enn, enw, _sv0t12, _sv0t13, _sv0t14, _sv0t15, _sv0t16, "", _sv0t17, _sv0t18);
  int lc0 = _sv0t19;
  if ((lc0 != 0)) {
    return 1;
  } else {
  }
  int _sv0t20 = sv0_vec_len(en0);
  if ((_sv0t20 != 0)) {
    return 2;
  } else {
  }
  ...
  int _sv0t34 = sv0_vec_new();
  int _sv0t35 = sv0_vec_new();
  int _sv0t36 = local_count_and_env(pn1, pc1, ins1, en1, eb1, ew1, ec1, efs1, sn, sfc, enn, enw, _sv0t29, _sv0t30, _sv0t31, _sv0t32, _sv0t33, "", _sv0t34, _sv0t35);
  int lc1 = _sv0t36;
  if ((lc1 != 2)) {
    return 3;
  } else {
  }
  int _sv0t37 = sv0_vec_len(en1);
  if ((_sv0t37 != 2)) {
    return 4;
  } else {
  }
  int _sv0t38 = sv0_vec_get(en1, 0);
  if ((_sv0t38 != 10)) {
    return 5;
  } else {
  }
  int _sv0t39 = sv0_vec_get(eb1, 0);
  if ((_sv0t39 != 0)) {
    return 6;
  } else {
  }
  int _sv0t40 = sv0_vec_get(ew1, 0);
  if ((_sv0t40 != 1)) {
    return 7;
  } else {
  }
  int _sv0t41 = sv0_vec_get(en1, 1);
  if ((_sv0t41 != 20)) {
    return 8;
  } else {
  }
  int _sv0t42 = sv0_vec_get(eb1, 1);
  if ((_sv0t42 != 1)) {
    return 9;
  } else {
  }
  ...
  int _sv0t58 = sv0_vec_new();
  int _sv0t59 = sv0_vec_new();
  int _sv0t60 = local_count_and_env(pn2, pc2, ins2, en2, eb2, ew2, ec2, efs2, sn2, sfc2, enn, enw, _sv0t53, _sv0t54, _sv0t55, _sv0t56, _sv0t57, "", _sv0t58, _sv0t59);
  int lc2 = _sv0t60;
  if ((lc2 != 3)) {
    return 10;
  } else {
  }
  int _sv0t61 = sv0_vec_get(ew2, 0);
  if ((_sv0t61 != 3)) {
    return 11;
  } else {
  }
  int _sv0t62 = sv0_vec_get(eb2, 0);
  if ((_sv0t62 != 0)) {
    return 12;
  } else {
  }
  ...
  _sv0t71.tag = 1;
  _sv0t71.p0 = 40;
  int _sv0t72 = sv0_box_alloc(5);
  sv0_box_store(_sv0t72, 0, _sv0t71.tag);
  sv0_box_store(_sv0t72, 1, _sv0t71.p0);
  sv0_box_store(_sv0t72, 2, _sv0t71.p1);
  sv0_box_store(_sv0t72, 3, _sv0t71.p2);
  sv0_box_store(_sv0t72, 4, _sv0t71.p3);
  sv0_vec_push(ins3, _sv0t72);
  int _sv0t73 = sv0_vec_new();
  int _sv0t74 = sv0_vec_new();
  int _sv0t75 = sv0_vec_new();
  int _sv0t76 = sv0_vec_new();
  ...
  int _sv0t80 = local_count_and_env(pn3, pc3, ins3, en3, eb3, ew3, ec3, efs3, sn, sfc, enn, enw, _sv0t73, _sv0t74, _sv0t75, _sv0t76, _sv0t77, "", _sv0t78, _sv0t79);
  int lc3 = _sv0t80;
  if ((lc3 != 1)) {
    return 13;
  } else {
  }
  int _sv0t81 = sv0_vec_get(en3, 0);
  if ((_sv0t81 != 40)) {
    return 14;
  } else {
  }
  int _sv0t82 = sv0_vec_get(eb3, 0);
  if ((_sv0t82 != 0)) {
    return 15;
  } else {
  }
  int _sv0t83 = sv0_vec_get(ew3, 0);
  if ((_sv0t83 != 1)) {
    return 16;
  } else {
  }
  ...
  sv0_vec_push(ins4, _sv0t93);
  Instr _sv0t94;
  _sv0t94.tag = 1;
  _sv0t94.p0 = 50;
  int _sv0t95 = sv0_box_alloc(5);
  sv0_box_store(_sv0t95, 0, _sv0t94.tag);
  sv0_box_store(_sv0t95, 1, _sv0t94.p0);
  sv0_box_store(_sv0t95, 2, _sv0t94.p1);
  sv0_box_store(_sv0t95, 3, _sv0t94.p2);
  sv0_box_store(_sv0t95, 4, _sv0t94.p3);
  sv0_vec_push(ins4, _sv0t95);
  int _sv0t96 = sv0_vec_new();
  int _sv0t97 = sv0_vec_new();
  int _sv0t98 = sv0_vec_new();
  ...
  int _sv0t101 = sv0_vec_new();
  int _sv0t102 = sv0_vec_new();
  int _sv0t103 = local_count_and_env(pn4, pc4, ins4, en4, eb4, ew4, ec4, efs4, sn, sfc, enn, enw, _sv0t96, _sv0t97, _sv0t98, _sv0t99, _sv0t100, "", _sv0t101, _sv0t102);
  int lc4 = _sv0t103;
  if ((lc4 != 1)) {
    return 17;
  } else {
  }
  int _sv0t104 = sv0_vec_len(en4);
  if ((_sv0t104 != 1)) {
    return 18;
  } else {
  }
  ...
  _sv0t113.tag = 3;
  _sv0t113.p0 = 60;
  _sv0t113.p1 = _sv0t117;
  int _sv0t118 = sv0_box_alloc(5);
  sv0_box_store(_sv0t118, 0, _sv0t113.tag);
  sv0_box_store(_sv0t118, 1, _sv0t113.p0);
  sv0_box_store(_sv0t118, 2, _sv0t113.p1);
  sv0_box_store(_sv0t118, 3, _sv0t113.p2);
  sv0_box_store(_sv0t118, 4, _sv0t113.p3);
  sv0_vec_push(ins5, _sv0t118);
  ...
  int _sv0t125 = sv0_vec_new();
  int _sv0t126 = local_count_and_env(pn5, pc5, ins5, en5, eb5, ew5, ec5, efs5, sn, sfc, enn, enw, _sv0t119, _sv0t120, _sv0t121, _sv0t122, _sv0t123, "", _sv0t124, _sv0t125);
  int lc5 = _sv0t126;
  if ((lc5 != 1)) {
    return 19;
  } else {
  }
  int _sv0t127 = sv0_vec_get(en5, 0);
  if ((_sv0t127 != 60)) {
    return 20;
  } else {
  }
  ...
  sv0_box_store(_sv0t138, 4, _sv0t137.p3);
  sv0_vec_push(ins6, _sv0t138);
  ...
  int _sv0t146 = local_count_and_env(pn6, pc6, ins6, en6, eb6, ew6, ec6, efs6, sn, sfc, enn, enw, _sv0t139, _sv0t140, _sv0t141, _sv0t142, _sv0t143, "", _sv0t144, _sv0t145);
  int lc6 = _sv0t146;
  if ((lc6 != 1)) {
    return 21;
  } else {
  }
  int _sv0t147 = sv0_vec_get(en6, 0);
  if ((_sv0t147 != 70)) {
    return 22;
  } else {
  }
  ...
  int _sv0t166 = local_count_and_env(pn7, pc7, ins7, en7, eb7, ew7, ec7, efs7, sn, sfc, enn, enw, _sv0t159, _sv0t160, _sv0t161, _sv0t162, _sv0t163, "", _sv0t164, _sv0t165);
  int lc7 = _sv0t166;
  if ((lc7 != 0)) {
    return 23;
  } else {
  }
  int _sv0t167 = sv0_vec_len(en7);
  if ((_sv0t167 != 0)) {
    return 24;
  } else {
  }
  ...
  sv0_box_store(_sv0t178, 0, _sv0t177.tag);
  sv0_box_store(_sv0t178, 1, _sv0t177.p0);
  sv0_box_store(_sv0t178, 2, _sv0t177.p1);
  sv0_box_store(_sv0t178, 3, _sv0t177.p2);
  sv0_box_store(_sv0t178, 4, _sv0t177.p3);
  sv0_vec_push(ins8, _sv0t178);
  int _sv0t179 = sv0_vec_new();
  int _sv0t180 = sv0_vec_new();
  int _sv0t181 = sv0_vec_new();
  int _sv0t182 = sv0_vec_new();
  int _sv0t183 = sv0_vec_new();
  int _sv0t184 = sv0_vec_new();
  int _sv0t185 = sv0_vec_new();
  int _sv0t186 = local_count_and_env(pn8, pc8, ins8, en8, eb8, ew8, ec8, efs8, sn2, sfc2, enn, enw, _sv0t179, _sv0t180, _sv0t181, _sv0t182, _sv0t183, "", _sv0t184, _sv0t185);
  int lc8 = _sv0t186;
  if ((lc8 != 3)) {
    return 25;
  } else {
  }
  int _sv0t187 = sv0_vec_get(ew8, 0);
  if ((_sv0t187 != 3)) {
    return 26;
  } else {
  }
  ...
  sv0_box_store(_sv0t199, 3, _sv0t198.p2);
  sv0_box_store(_sv0t199, 4, _sv0t198.p3);
  sv0_vec_push(th9, _sv0t199);
  Instr _sv0t200;
  _sv0t200.tag = 1;
  _sv0t200.p0 = 91;
  ...
  sv0_box_store(_sv0t201, 4, _sv0t200.p3);
  sv0_vec_push(el9, _sv0t201);
  Instr _sv0t202;
  Expr _sv0t203;
  Value _sv0t204;
  _sv0t204.tag = 1;
  int _sv0t205 = sv0_box_alloc(3);
  sv0_box_store(_sv0t205, 0, _sv0t204.tag);
  sv0_box_store(_sv0t205, 1, _sv0t204.p0);
  sv0_box_store(_sv0t205, 2, _sv0t204.p1);
  _sv0t203.tag = 0;
  _sv0t203.p0 = _sv0t205;
  int _sv0t206 = sv0_box_alloc(4);
  sv0_box_store(_sv0t206, 0, _sv0t203.tag);
  sv0_box_store(_sv0t206, 1, _sv0t203.p0);
  sv0_box_store(_sv0t206, 2, _sv0t203.p1);
  sv0_box_store(_sv0t206, 3, _sv0t203.p2);
  _sv0t202.tag = 6;
  _sv0t202.p0 = _sv0t206;
  _sv0t202.p1 = th9;
  _sv0t202.p2 = el9;
  int _sv0t207 = sv0_box_alloc(5);
  sv0_box_store(_sv0t207, 0, _sv0t202.tag);
  sv0_box_store(_sv0t207, 1, _sv0t202.p0);
  sv0_box_store(_sv0t207, 2, _sv0t202.p1);
  sv0_box_store(_sv0t207, 3, _sv0t202.p2);
  sv0_box_store(_sv0t207, 4, _sv0t202.p3);
  sv0_vec_push(ins9, _sv0t207);
  int _sv0t208 = sv0_vec_new();
  int _sv0t209 = sv0_vec_new();
  int _sv0t210 = sv0_vec_new();
  int _sv0t211 = sv0_vec_new();
  int _sv0t212 = sv0_vec_new();
  int _sv0t213 = sv0_vec_new();
  int _sv0t214 = sv0_vec_new();
  int _sv0t215 = local_count_and_env(pn9, pc9, ins9, en9, eb9, ew9, ec9, efs9, sn, sfc, enn, enw, _sv0t208, _sv0t209, _sv0t210, _sv0t211, _sv0t212, "", _sv0t213, _sv0t214);
  int lc9 = _sv0t215;
  if ((lc9 != 2)) {
    return 27;
  } else {
  }
  int _sv0t216 = sv0_vec_get(en9, 0);
  if ((_sv0t216 != 90)) {
    return 28;
  } else {
  }
  int _sv0t217 = sv0_vec_get(en9, 1);
  if ((_sv0t217 != 91)) {
    return 29;
  } else {
  }
  ...
  int eb10 = _sv0t219;
  int _sv0t220 = sv0_vec_new();
  int ew10 = _sv0t220;
  int _sv0t221 = sv0_vec_new();
  int ec10 = _sv0t221;
  ...
  sv0_box_store(_sv0t228, 0, _sv0t227.tag);
  sv0_box_store(_sv0t228, 1, _sv0t227.p0);
  sv0_box_store(_sv0t228, 2, _sv0t227.p1);
  sv0_box_store(_sv0t228, 3, _sv0t227.p2);
  sv0_box_store(_sv0t228, 4, _sv0t227.p3);
  sv0_vec_push(wbod, _sv0t228);
  Instr _sv0t229;
  Expr _sv0t230;
  Value _sv0t231;
  _sv0t231.tag = 1;
  int _sv0t232 = sv0_box_alloc(3);
  sv0_box_store(_sv0t232, 0, _sv0t231.tag);
  sv0_box_store(_sv0t232, 1, _sv0t231.p0);
  sv0_box_store(_sv0t232, 2, _sv0t231.p1);
  _sv0t230.tag = 0;
  _sv0t230.p0 = _sv0t232;
  int _sv0t233 = sv0_box_alloc(4);
  sv0_box_store(_sv0t233, 0, _sv0t230.tag);
  sv0_box_store(_sv0t233, 1, _sv0t230.p0);
  sv0_box_store(_sv0t233, 2, _sv0t230.p1);
  sv0_box_store(_sv0t233, 3, _sv0t230.p2);
  _sv0t229.tag = 7;
  _sv0t229.p0 = _sv0t233;
  _sv0t229.p1 = wbod;
  int _sv0t234 = sv0_box_alloc(5);
  sv0_box_store(_sv0t234, 0, _sv0t229.tag);
  sv0_box_store(_sv0t234, 1, _sv0t229.p0);
  sv0_box_store(_sv0t234, 2, _sv0t229.p1);
  sv0_box_store(_sv0t234, 3, _sv0t229.p2);
  sv0_box_store(_sv0t234, 4, _sv0t229.p3);
  sv0_vec_push(ins10, _sv0t234);
  int _sv0t235 = sv0_vec_new();
  int _sv0t236 = sv0_vec_new();
  int _sv0t237 = sv0_vec_new();
  int _sv0t238 = sv0_vec_new();
  int _sv0t239 = sv0_vec_new();
  int _sv0t240 = sv0_vec_new();
  int _sv0t241 = sv0_vec_new();
  int _sv0t242 = local_count_and_env(pn10, pc10, ins10, en10, eb10, ew10, ec10, efs10, sn, sfc, enn, enw, _sv0t235, _sv0t236, _sv0t237, _sv0t238, _sv0t239, "", _sv0t240, _sv0t241);
  int lc10 = _sv0t242;
  if ((lc10 != 1)) {
    return 30;
  } else {
  }
  int _sv0t243 = sv0_vec_get(en10, 0);
  if ((_sv0t243 != 95)) {
    return 31;
  } else {
  }
  ...
  sv0_vec_push(ins11, _sv0t255);
  int _sv0t256 = sv0_vec_new();
  int _sv0t257 = sv0_vec_new();
  int _sv0t258 = sv0_vec_new();
  int _sv0t259 = sv0_vec_new();
  int _sv0t260 = sv0_vec_new();
  int _sv0t261 = sv0_vec_new();
  int _sv0t262 = sv0_vec_new();
  int _sv0t263 = local_count_and_env(pn11, pc11, ins11, en11, eb11, ew11, ec11, efs11, sn, sfc, enn, enw, _sv0t256, _sv0t257, _sv0t258, _sv0t259, _sv0t260, "", _sv0t261, _sv0t262);
  int lc11 = _sv0t263;
  if ((lc11 != 3)) {
    return 32;
  } else {
  }
  int _sv0t264 = sv0_vec_len(en11);
  if ((_sv0t264 != 3)) {
    return 33;
  } else {
  }
  int _sv0t265 = sv0_vec_get(eb11, 0);
  if ((_sv0t265 != 0)) {
    return 34;
  } else {
  }
  int _sv0t266 = sv0_vec_get(eb11, 1);
  if ((_sv0t266 != 1)) {
    return 35;
  } else {
  }
  int _sv0t267 = sv0_vec_get(eb11, 2);
  if ((_sv0t267 != 2)) {
    return 36;
  } else {
  }
  ...
  int en12 = _sv0t268;
  int _sv0t269 = sv0_vec_new();
  int eb12 = _sv0t269;
  int _sv0t270 = sv0_vec_new();
  int ew12 = _sv0t270;
  int _sv0t271 = sv0_vec_new();
  int ec12 = _sv0t271;
  int _sv0t272 = sv0_vec_new();
  int efs12 = _sv0t272;
  int _sv0t273 = sv0_vec_new();
  int pn12 = _sv0t273;
  int _sv0t274 = sv0_vec_new();
  int pc12 = _sv0t274;
  int _sv0t275 = sv0_vec_new();
  int ins12 = _sv0t275;
  Instr _sv0t276;
  _sv0t276.tag = 0;
  int _sv0t277 = sv0_box_alloc(5);
  sv0_box_store(_sv0t277, 0, _sv0t276.tag);
  sv0_box_store(_sv0t277, 1, _sv0t276.p0);
  sv0_box_store(_sv0t277, 2, _sv0t276.p1);
  sv0_box_store(_sv0t277, 3, _sv0t276.p2);
  sv0_box_store(_sv0t277, 4, _sv0t276.p3);
  sv0_vec_push(ins12, _sv0t277);
  Instr _sv0t278;
  _sv0t278.tag = 9;
  int _sv0t279 = sv0_box_alloc(5);
  sv0_box_store(_sv0t279, 0, _sv0t278.tag);
  sv0_box_store(_sv0t279, 1, _sv0t278.p0);
  sv0_box_store(_sv0t279, 2, _sv0t278.p1);
  sv0_box_store(_sv0t279, 3, _sv0t278.p2);
  sv0_box_store(_sv0t279, 4, _sv0t278.p3);
  sv0_vec_push(ins12, _sv0t279);
  Instr _sv0t280;
  _sv0t280.tag = 10;
  int _sv0t281 = sv0_box_alloc(5);
  sv0_box_store(_sv0t281, 0, _sv0t280.tag);
  sv0_box_store(_sv0t281, 1, _sv0t280.p0);
  sv0_box_store(_sv0t281, 2, _sv0t280.p1);
  sv0_box_store(_sv0t281, 3, _sv0t280.p2);
  sv0_box_store(_sv0t281, 4, _sv0t280.p3);
  sv0_vec_push(ins12, _sv0t281);
  Instr _sv0t282;
  _sv0t282.tag = 14;
  int _sv0t283 = sv0_box_alloc(5);
  sv0_box_store(_sv0t283, 0, _sv0t282.tag);
  sv0_box_store(_sv0t283, 1, _sv0t282.p0);
  sv0_box_store(_sv0t283, 2, _sv0t282.p1);
  sv0_box_store(_sv0t283, 3, _sv0t282.p2);
  sv0_box_store(_sv0t283, 4, _sv0t282.p3);
  sv0_vec_push(ins12, _sv0t283);
  int _sv0t284 = sv0_vec_new();
  int _sv0t285 = sv0_vec_new();
  int _sv0t286 = sv0_vec_new();
  int _sv0t287 = sv0_vec_new();
  int _sv0t288 = sv0_vec_new();
  int _sv0t289 = sv0_vec_new();
  int _sv0t290 = sv0_vec_new();
  int _sv0t291 = local_count_and_env(pn12, pc12, ins12, en12, eb12, ew12, ec12, efs12, sn, sfc, enn, enw, _sv0t284, _sv0t285, _sv0t286, _sv0t287, _sv0t288, "", _sv0t289, _sv0t290);
  int lc12 = _sv0t291;
  if ((lc12 != 0)) {
    return 37;
  } else {
  }
  int _sv0t292 = sv0_vec_len(en12);
  if ((_sv0t292 != 0)) {
    return 38;
  } else {
  }
...
static int emit_program_1fn(const char* source, int starts, int ends, int lab, int pn, int pc, int ins, int pool, int ft) {
  ...
  int _sv0t10 = sv0_vec_new();
  int _sv0t11 = sv0_vec_new();
  int _sv0t12 = emit_program(it, in2, ifc, ivp, _sv0t8, _sv0t9, _sv0t10, bl, bpn, bpc, bi, _sv0t11, source, starts, ends, pool, ft);
  return _sv0t12;
}
...
static int test_typed_opcode_select(void) {
  ...
  int _sv0t20 = binop_tag_to_insn_typed(16, 2);
  if ((_sv0t20 != 40)) {
    return 34;
  } else {
  }
  int _sv0t21 = binop_tag_to_insn_typed(17, 2);
  if ((_sv0t21 != 41)) {
    return 35;
  } else {
  }
  int _sv0t22 = binop_tag_to_insn_typed(16, 3);
  if ((_sv0t22 != 40)) {
    return 36;
  } else {
  }
  int _sv0t23 = binop_tag_to_insn_typed(17, 3);
  if ((_sv0t23 != 42)) {
    return 37;
  } else {
  }
  int _sv0t24 = binop_tag_to_insn_typed(13, 2);
  if ((_sv0t24 != 43)) {
    return 38;
  } else {
  }
  int _sv0t25 = unop_tag_to_insn_typed(0, 0);
  if ((_sv0t25 != 21)) {
    return 40;
  } else {
  }
  int _sv0t26 = unop_tag_to_insn_typed(0, 1);
  if ((_sv0t26 != 52)) {
    return 41;
  } else {
  }
  int _sv0t27 = unop_tag_to_insn_typed(0, 2);
  if ((_sv0t27 != 37)) {
    return 42;
  } else {
  }
  int _sv0t28 = unop_tag_to_insn_typed(1, 1);
  if ((_sv0t28 != 82)) {
    return 43;
  } else {
  }
  ...
  sv0_vec_push(ec, 2);
  int _sv0t30 = (0 - 1);
  int _sv0t31 = slot_cat(ec, _sv0t30);
  if ((_sv0t31 != 0)) {
    return 50;
  } else {
  }
  int _sv0t32 = slot_cat(ec, 1);
  if ((_sv0t32 != 1)) {
    return 51;
  } else {
  }
  int _sv0t33 = slot_cat(ec, 2);
  if ((_sv0t33 != 2)) {
    return 52;
  } else {
  }
  int _sv0t34 = slot_cat(ec, 9);
  if ((_sv0t34 != 0)) {
    return 53;
  } else {
  }
  ...
  int _sv0t41 = sv0_vec_new();
  int _sv0t42 = sv0_vec_new();
  int _sv0t43 = operand_cat(_sv0t38, en, eb, ew, ec, _sv0t39, _sv0t40, "", _sv0t41, _sv0t42);
  if ((_sv0t43 != 1)) {
    return 60;
  } else {
  }
  ...
  int _sv0t47 = sv0_vec_new();
  int _sv0t48 = sv0_vec_new();
  int _sv0t49 = operand_cat(_sv0t44, en, eb, ew, ec, _sv0t45, _sv0t46, "", _sv0t47, _sv0t48);
  if ((_sv0t49 != 2)) {
    return 61;
  } else {
  }
  ...
  int _sv0t53 = sv0_vec_new();
  int _sv0t54 = sv0_vec_new();
  int _sv0t55 = operand_cat(_sv0t50, en, eb, ew, ec, _sv0t51, _sv0t52, "", _sv0t53, _sv0t54);
  if ((_sv0t55 != 0)) {
    return 62;
  } else {
  }
  ...
  int _sv0t60 = sv0_vec_new();
  int _sv0t61 = sv0_vec_new();
  int _sv0t62 = operand_cat(_sv0t57, en, eb, ew, ec2, _sv0t58, _sv0t59, "", _sv0t60, _sv0t61);
  if ((_sv0t62 != 1)) {
    return 63;
  } else {
  }
  ...
  mv_missing.tag = 6;
  mv_missing.p0 = _sv0t74;
  mv_missing.p1 = 99;
  int _sv0t75 = sv0_vec_new();
  int _sv0t76 = sv0_vec_new();
  int _sv0t77 = operand_cat(mv7, en, eb, ew3, ec3, fst, ffs, "", _sv0t75, _sv0t76);
  if ((_sv0t77 != 1)) {
    return 64;
  } else {
  }
  int _sv0t78 = sv0_vec_new();
  int _sv0t79 = sv0_vec_new();
  int _sv0t80 = operand_cat(mv8, en, eb, ew3, ec3, fst, ffs, "", _sv0t78, _sv0t79);
  if ((_sv0t80 != 3)) {
    return 65;
  } else {
  }
  ...
  int _sv0t83 = operand_cat(mv9, en, eb, ew3, ec3, fst, ffs, "", _sv0t81, _sv0t82);
  if ((_sv0t83 != 0)) {
    return 66;
  } else {
  }
  int _sv0t84 = sv0_vec_new();
  int _sv0t85 = sv0_vec_new();
  int _sv0t86 = operand_cat(mv_missing, en, eb, ew3, ec3, fst, ffs, "", _sv0t84, _sv0t85);
  if ((_sv0t86 != 0)) {
    return 67;
  } else {
  }
  ...
  int _sv0t89 = sv0_vec_new();
  int _sv0t90 = sv0_vec_new();
  int _sv0t91 = operand_cat(mv7, en, eb, ew3, ec3, _sv0t87, _sv0t88, "", _sv0t89, _sv0t90);
  if ((_sv0t91 != 0)) {
    return 68;
  } else {
  }
  int _sv0t92 = field_cat_at(ffs, 0, 1);
  if ((_sv0t92 != 3)) {
    return 69;
  } else {
  }
  int _sv0t93 = field_cat_at(ffs, 0, 5);
  if ((_sv0t93 != 0)) {
    return 70;
  } else {
  }
  int _sv0t94 = combine_cat(4, 0);
  if ((_sv0t94 != 4)) {
    return 71;
  } else {
  }
  int _sv0t95 = combine_cat(0, 4);
  if ((_sv0t95 != 4)) {
    return 72;
  } else {
  }
  int _sv0t96 = combine_cat(4, 3);
  if ((_sv0t96 != 3)) {
    return 73;
  } else {
  }
  int _sv0t97 = combine_cat(4, 2);
  if ((_sv0t97 != 2)) {
    return 74;
  } else {
  }
  int _sv0t98 = combine_cat(4, 1);
  if ((_sv0t98 != 1)) {
    return 75;
  } else {
  }
  int _sv0t99 = binop_tag_to_insn_typed(3, 4);
  if ((_sv0t99 != 38)) {
    return 76;
  } else {
  }
  int _sv0t100 = binop_tag_to_insn_typed(4, 4);
  if ((_sv0t100 != 39)) {
    return 77;
  } else {
  }
  int _sv0t101 = binop_tag_to_insn_typed(10, 4);
  if ((_sv0t101 != 73)) {
    return 78;
  } else {
  }
  int _sv0t102 = binop_tag_to_insn_typed(17, 4);
  if ((_sv0t102 != 42)) {
    return 79;
  } else {
  }
  int _sv0t103 = binop_tag_to_insn_typed(1, 4);
  int _sv0t104 = binop_tag_to_insn(1);
  if ((_sv0t103 != _sv0t104)) {
    return 80;
  } else {
  }
  int _sv0t105 = binop_tag_to_insn_typed(13, 2);
  if ((_sv0t105 != 43)) {
    return 102;
  } else {
  }
  int _sv0t106 = binop_tag_to_insn_typed(14, 3);
  if ((_sv0t106 != 44)) {
    return 103;
  } else {
  }
  int _sv0t107 = binop_tag_to_insn_typed(15, 2);
  if ((_sv0t107 != 45)) {
    return 104;
  } else {
  }
  int _sv0t108 = binop_tag_to_insn_typed(13, 0);
  int _sv0t109 = binop_tag_to_insn(13);
  if ((_sv0t108 != _sv0t109)) {
    return 105;
  } else {
  }
  int _sv0t110 = binop_tag_to_insn_typed(13, 4);
  int _sv0t111 = binop_tag_to_insn(13);
  if ((_sv0t110 != _sv0t111)) {
    return 106;
  } else {
  }
  int _sv0t112 = unop_tag_to_insn_typed(0, 4);
  if ((_sv0t112 != 21)) {
    return 81;
  } else {
  }
  int _sv0t113 = u32_operand_needs_zext(4, 4, 10);
  if ((_sv0t113 != 1)) {
    return 82;
  } else {
  }
  int _sv0t114 = u32_operand_needs_zext(4, 4, 3);
  if ((_sv0t114 != 1)) {
    return 83;
  } else {
  }
  int _sv0t115 = u32_operand_needs_zext(4, 4, 0);
  if ((_sv0t115 != 0)) {
    return 84;
  } else {
  }
  int _sv0t116 = u32_operand_needs_zext(4, 4, 16);
  if ((_sv0t116 != 0)) {
    return 85;
  } else {
  }
  int _sv0t117 = u32_operand_needs_zext(4, 3, 0);
  if ((_sv0t117 != 1)) {
    return 86;
  } else {
  }
  int _sv0t118 = u32_operand_needs_zext(4, 1, 0);
  if ((_sv0t118 != 1)) {
    return 87;
  } else {
  }
  int _sv0t119 = u32_operand_needs_zext(0, 4, 10);
  if ((_sv0t119 != 0)) {
    return 88;
  } else {
  }
  int _sv0t120 = u32_operand_needs_zext(3, 3, 10);
  if ((_sv0t120 != 0)) {
    return 89;
  } else {
  }
  ...
  _sv0t121.tag = 9;
  _sv0t121.p0 = 0;
  int _sv0t122 = value_is_wide_literal(_sv0t121);
  if ((_sv0t122 != 1)) {
    return 95;
  } else {
  }
  ...
  _sv0t123.tag = 0;
  _sv0t123.p0 = 7;
  int _sv0t124 = value_is_wide_literal(_sv0t123);
  if ((_sv0t124 != 0)) {
    return 96;
  } else {
  }
  ...
  _sv0t126.tag = 9;
  _sv0t126.p0 = 0;
  int _sv0t127 = binop_emit_cat(13, 4, 2, _sv0t125, _sv0t126);
  if ((_sv0t127 != 4)) {
    return 97;
  } else {
  }
  ...
  _sv0t129.tag = 3;
  _sv0t129.p0 = 1;
  int _sv0t130 = binop_emit_cat(15, 2, 4, _sv0t128, _sv0t129);
  if ((_sv0t130 != 4)) {
    return 98;
  } else {
  }
  ...
  _sv0t132.tag = 3;
  _sv0t132.p0 = 2;
  int _sv0t133 = binop_emit_cat(13, 4, 2, _sv0t131, _sv0t132);
  if ((_sv0t133 != 2)) {
    return 99;
  } else {
  }
  ...
  _sv0t135.tag = 9;
  _sv0t135.p0 = 0;
  int _sv0t136 = binop_emit_cat(0, 4, 2, _sv0t134, _sv0t135);
  if ((_sv0t136 != 2)) {
    return 100;
  } else {
  }
  ...
  _sv0t138.tag = 9;
  _sv0t138.p0 = 0;
  int _sv0t139 = binop_emit_cat(16, 4, 2, _sv0t137, _sv0t138);
  if ((_sv0t139 != 2)) {
    return 101;
  } else {
  }
  int _sv0t140 = sv0_vec_new();
  int wout = _sv0t140;
  int _sv0t141 = emit_u32_zext(wout);
  if ((_sv0t141 != 4)) {
    return 90;
  } else {
  }
  int _sv0t142 = sv0_vec_len(wout);
  if ((_sv0t142 != 4)) {
    return 91;
  } else {
  }
  int _sv0t143 = sv0_vec_get(wout, 0);
  if ((_sv0t143 != 5)) {
    return 92;
  } else {
  }
  int _sv0t144 = sv0_vec_get(wout, 2);
  if ((_sv0t144 != 1)) {
    return 93;
  } else {
  }
  int _sv0t145 = sv0_vec_get(wout, 3);
  if ((_sv0t145 != 39)) {
    return 94;
  } else {
  }
...
static int nested_struct_tables(int sn, int sfc, int flat, int fstarts) {
  ...
  sv0_vec_push(sfc, 2);
  sv0_vec_push(sfc, 2);
  sv0_vec_push(flat, 101);
  sv0_vec_push(flat, 0);
  sv0_vec_push(flat, 102);
  sv0_vec_push(flat, 1);
  sv0_vec_push(flat, 201);
  sv0_vec_push(flat, 16);
  sv0_vec_push(flat, 202);
  sv0_vec_push(flat, 0);
  sv0_vec_push(fstarts, 0);
  sv0_vec_push(fstarts, 4);
  return 0;
}
...
static int test_nested_struct_layout(void) {
  ...
  int _sv0t11 = sv0_vec_new();
  int _sv0t12 = sv0_vec_new();
  int _sv0t13 = width_of_cty(20, sn, sfc, en, ew, "", _sv0t11, _sv0t12);
  if ((_sv0t13 != 3)) {
    return 2;
  } else {
  }
  int _sv0t14 = sv0_vec_get(sfc, 1);
  int _sv0t15 = struct_packed_count(_sv0t14);
  if ((_sv0t15 != 2)) {
    return 3;
  } else {
  }
  ...
  int _sv0t20 = sv0_vec_len(env_flat);
  if ((_sv0t20 != 4)) {
    return 5;
  } else {
  }
  ...
  int _sv0t24 = (fs_t - 2);
  int _sv0t25 = sv0_vec_get(env_flat, _sv0t24);
  if ((_sv0t25 != 5)) {
    return 7;
  } else {
  }
  int _sv0t26 = (fs_t - 1);
  int _sv0t27 = sv0_vec_get(env_flat, _sv0t26);
  int _sv0t28 = run_hdr_magic();
  if ((_sv0t27 != _sv0t28)) {
    return 8;
  } else {
  }
  int _sv0t29 = sv0_vec_get(env_flat, fs_t);
  if ((_sv0t29 != 201)) {
    return 9;
  } else {
  }
  int _sv0t30 = (fs_t + 1);
  int _sv0t31 = sv0_vec_get(env_flat, _sv0t30);
  int _sv0t32 = (0 | 16);
  int _sv0t33 = (2 << 5);
  int _sv0t34 = (_sv0t32 | _sv0t33);
  if ((_sv0t31 != _sv0t34)) {
    return 10;
  } else {
  }
  int _sv0t35 = (fs_t + 2);
  int _sv0t36 = sv0_vec_get(env_flat, _sv0t35);
  if ((_sv0t36 != 2)) {
    return 11;
  } else {
  }
  int _sv0t37 = (fs_t + 4);
  int _sv0t38 = sv0_vec_get(env_flat, _sv0t37);
  if ((_sv0t38 != 101)) {
    return 12;
  } else {
  }
  int _sv0t39 = (fs_t + 8);
  int _sv0t40 = sv0_vec_get(env_flat, _sv0t39);
  if ((_sv0t40 != 202)) {
    return 13;
  } else {
  }
  ...
  int widths = _sv0t43;
  int _sv0t44 = sv0_vec_new();
  int fstarts_env = _sv0t44;
  sv0_vec_push(names, 500);
  sv0_vec_push(bases, 0);
  sv0_vec_push(widths, 3);
  sv0_vec_push(fstarts_env, fs_t);
  int _sv0t45 = locate_out_new();
  int lo = _sv0t45;
  Value tb;
  Value _sv0t46;
  Value _sv0t47;
  ...
  int _sv0t50 = sv0_vec_new();
  int _sv0t51 = sv0_vec_new();
  int _sv0t52 = locate_member(tb, names, bases, widths, fstarts_env, env_flat, "", _sv0t50, _sv0t51, lo);
  if ((_sv0t52 < 0)) {
    return 14;
  } else {
  }
  int _sv0t53 = sv0_vec_get(lo, 1);
  if ((_sv0t53 != 1)) {
    return 15;
  } else {
  }
  int _sv0t54 = sv0_vec_get(lo, 4);
  if ((_sv0t54 != 1)) {
    return 16;
  } else {
  }
  int _sv0t55 = sv0_vec_get(lo, 5);
  if ((_sv0t55 != 1)) {
    return 17;
  } else {
  }
  ...
  int _sv0t58 = sv0_vec_new();
  int _sv0t59 = sv0_vec_new();
  int _sv0t60 = locate_member(tk, names, bases, widths, fstarts_env, env_flat, "", _sv0t58, _sv0t59, lo);
  if ((_sv0t60 < 0)) {
    return 18;
  } else {
  }
  int _sv0t61 = sv0_vec_get(lo, 1);
  if ((_sv0t61 != 2)) {
    return 19;
  } else {
  }
  ...
  int _sv0t64 = sv0_vec_new();
  int _sv0t65 = sv0_vec_new();
  int _sv0t66 = locate_member(ts, names, bases, widths, fstarts_env, env_flat, "", _sv0t64, _sv0t65, lo);
  if ((_sv0t66 < 0)) {
    return 20;
  } else {
  }
  int _sv0t67 = sv0_vec_get(lo, 4);
  if ((_sv0t67 != 2)) {
    return 21;
  } else {
  }
  int _sv0t68 = sv0_vec_get(lo, 6);
  if ((_sv0t68 != 1)) {
    return 22;
  } else {
  }
  int _sv0t69 = sv0_vec_get(lo, 3);
  if ((_sv0t69 != 2)) {
    return 23;
  } else {
  }
...
int main(void) {
  ...
  int _sv0t87 = test_nested_struct_layout();
  int r45 = _sv0t87;
  if ((r45 != 0)) {
    int _sv0t88 = (800 + r45);
    return _sv0t88;
