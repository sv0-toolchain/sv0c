...
static int lower_field_chain(int et, int ed1, int ed2, int pp, int lhs, int chain);
static int lower_nested_field_store(int out_instrs, int ctr, int base_tok, int chain, int op_tag, Value rhs);
...
static const char* lower_emit_all_enum_tds(int item_tags, int item_names, int item_field_counts, int item_vmax, const char* source, int starts, int ends, int item_d3, int evpv, int evptn);
...
static int test_lower_field_chain(void);
...
static Value lower_tag_match(int et, int ed1, int ed2, int ed3, int ed4, int pp, int tok_tags, int idx, int ctr, int out_instrs, int enum_names, int enum_tag_offsets, int enum_tag_counts, int enum_tags_flat, int fn_ctx, int builtin_map, int lit_sf_names, const char* source, int starts, int ends, int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int enum_vnames_flat, int enum_max_payloads, int item_fn_row, int match_env_inherit, int import_aliases) {
  ...
  int sct = _sv0t10;
  if ((sct < 0)) {
    int _sv0t11 = lower_match_pat_type_tok(et, ed1, ed2, ed4, pp, arms_first, arm_count, arms_sidecar);
    sct = _sv0t11;
  } else {
  }
  int _sv0t12 = bind_scrut_var(et, ed1, ed2, ed3, ed4, pp, tok_tags, scrut_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env, sct, source, starts, ends, import_aliases);
  int scrut_var = _sv0t12;
  int _sv0t13 = sv0_vec_get(et, scrut_idx);
  int st_scr = _sv0t13;
  if ((st_scr == 1)) {
    int _sv0t14 = sv0_vec_get(ed1, scrut_idx);
    int pps_s2 = _sv0t14;
    int _sv0t15 = sv0_vec_get(ed2, scrut_idx);
    int ppc_s2 = _sv0t15;
    if ((ppc_s2 == 1)) {
      int _sv0t16 = sv0_vec_get(pp, pps_s2);
      int nh2 = _sv0t16;
      int _sv0t17 = lower_match_env_append_scrut_pair(match_env, nh2, sct);
      int dps2 = _sv0t17;
    } else {
    }
  } else {
  ...
  dv.p0 = out;
  int _sv0t19 = sv0_box_alloc(5);
  sv0_box_store(_sv0t19, 0, dv.tag);
  sv0_box_store(_sv0t19, 1, dv.p0);
  sv0_box_store(_sv0t19, 2, dv.p1);
  sv0_box_store(_sv0t19, 3, dv.p2);
  sv0_box_store(_sv0t19, 4, dv.p3);
  sv0_vec_push(out_instrs, _sv0t19);
  int _sv0t20 = lower_vm_target();
  if ((_sv0t20 == 0)) {
    Instr zero_init;
    Expr _sv0t21;
    Value _sv0t22;
    ...
    zero_init.tag = 4;
    zero_init.p0 = out;
    zero_init.p1 = _sv0t24;
    int _sv0t25 = sv0_box_alloc(5);
    sv0_box_store(_sv0t25, 0, zero_init.tag);
    sv0_box_store(_sv0t25, 1, zero_init.p0);
    sv0_box_store(_sv0t25, 2, zero_init.p1);
    sv0_box_store(_sv0t25, 3, zero_init.p2);
    sv0_box_store(_sv0t25, 4, zero_init.p3);
    sv0_vec_push(out_instrs, _sv0t25);
  } else {
  }
...
static int lower_match_pat_type_tok(int et, int ed1, int ed2, int ed4, int pp, int arms_first, int arm_count, int arms_sidecar) {
  int ai = 0;
  while ((ai < arm_count)) {
    int _sv0t0 = block_stmt_index(pp, arms_first, arms_sidecar, ai);
    int arm_idx = _sv0t0;
    if ((arm_idx >= 0)) {
      int _sv0t1 = sv0_vec_get(et, arm_idx);
      if ((_sv0t1 == 29)) {
        int _sv0t2 = sv0_vec_get(ed1, arm_idx);
        int pt = _sv0t2;
        if ((pt == 4)) {
          int _sv0t3 = sv0_vec_get(ed2, arm_idx);
          int pa = _sv0t3;
          int _sv0t4 = sv0_vec_get(et, pa);
          if ((_sv0t4 == 30)) {
            int _sv0t5 = sv0_vec_get(ed4, pa);
            int k = _sv0t5;
            int _sv0t6 = sv0_vec_get(ed2, pa);
            int segs = _sv0t6;
            int _sv0t7 = sv0_vec_get(ed1, pa);
            int ps = _sv0t7;
            if ((k == 4)) {
              if ((segs >= 1)) {
                int _sv0t8 = sv0_vec_get(pp, ps);
                return _sv0t8;
              } else {
              }
            } else {
        ...
        if ((pt == 5)) {
          int _sv0t9 = sv0_vec_get(ed2, arm_idx);
          int pa5 = _sv0t9;
          int _sv0t10 = sv0_vec_get(et, pa5);
          if ((_sv0t10 == 30)) {
            int _sv0t11 = sv0_vec_get(ed4, pa5);
            int k5 = _sv0t11;
            int _sv0t12 = sv0_vec_get(ed2, pa5);
            int segs5 = _sv0t12;
            int _sv0t13 = sv0_vec_get(ed1, pa5);
            int ps5 = _sv0t13;
            if ((k5 == 5)) {
              if ((segs5 >= 2)) {
                int _sv0t14 = sv0_vec_get(pp, ps5);
                return _sv0t14;
              } else {
              }
            } else {
...
static int lower_field_chain(int et, int ed1, int ed2, int pp, int lhs, int chain) {
  ...
  int _sv0t3 = sv0_vec_get(et, lhs);
  if ((_sv0t3 != 6)) {
    int _sv0t4 = (0 - 1);
    return _sv0t4;
  } else {
  }
  int _sv0t5 = sv0_vec_get(ed1, lhs);
  int obj = _sv0t5;
  if ((obj < 0)) {
    int _sv0t6 = (0 - 1);
    return _sv0t6;
  ...
  int _sv0t11 = sv0_vec_new();
  int rev = _sv0t11;
  int _sv0t12 = sv0_vec_get(ed2, lhs);
  sv0_vec_push(rev, _sv0t12);
  int cur = obj;
  int guard = 0;
  while ((guard < 64)) {
    int _sv0t13 = sv0_vec_get(et, cur);
    if ((_sv0t13 == 6)) {
      int _sv0t14 = sv0_vec_get(ed2, cur);
      sv0_vec_push(rev, _sv0t14);
      int _sv0t15 = sv0_vec_get(ed1, cur);
      cur = _sv0t15;
      if ((cur < 0)) {
        int _sv0t16 = (0 - 1);
        return _sv0t16;
  ...
  int _sv0t19 = sv0_vec_get(et, cur);
  if ((_sv0t19 != 1)) {
    int _sv0t20 = (0 - 1);
    return _sv0t20;
  } else {
  }
  int _sv0t21 = sv0_vec_get(ed1, cur);
  int pps = _sv0t21;
  if ((pps < 0)) {
    int _sv0t22 = (0 - 1);
    return _sv0t22;
  ...
  int _sv0t23 = sv0_vec_len(pp);
  if ((pps >= _sv0t23)) {
    int _sv0t24 = (0 - 1);
    return _sv0t24;
  } else {
  }
  int _sv0t25 = sv0_vec_len(rev);
  int r = (_sv0t25 - 1);
  while ((r >= 0)) {
    int _sv0t26 = sv0_vec_get(rev, r);
    sv0_vec_push(chain, _sv0t26);
    r = (r - 1);
  }
  int _sv0t27 = sv0_vec_get(pp, pps);
  return _sv0t27;
}

static int lower_nested_field_store(int out_instrs, int ctr, int base_tok, int chain, int op_tag, Value rhs) {
  int _sv0t0 = sv0_vec_len(chain);
  int n = _sv0t0;
  int _sv0t1 = sv0_vec_new();
  int temps = _sv0t1;
  int owner = base_tok;
  ...
  while (1) {
    ...
    int _sv0t6 = sv0_vec_get(chain, i);
    mem.tag = 6;
    mem.p0 = _sv0t5;
    mem.p1 = _sv0t6;
    Instr _sv0t7;
    Expr _sv0t8 = value_to_expr(mem);
    int _sv0t9 = sv0_box_alloc(4);
    sv0_box_store(_sv0t9, 0, _sv0t8.tag);
    sv0_box_store(_sv0t9, 1, _sv0t8.p0);
    sv0_box_store(_sv0t9, 2, _sv0t8.p1);
    ...
    sv0_box_store(_sv0t10, 4, _sv0t7.p3);
    sv0_vec_push(out_instrs, _sv0t10);
    sv0_vec_push(temps, t);
    owner = t;
    i = (i + 1);
  }
  int _sv0t12 = (n - 1);
  int _sv0t13 = sv0_vec_get(chain, _sv0t12);
  int fnl = _sv0t13;
  Expr _sv0t14 = value_to_expr(rhs);
  Expr e;
  e = _sv0t14;
  if ((op_tag >= 0)) {
    Value cur;
    ...
    sv0_box_store(_sv0t18, 0, cur.tag);
    sv0_box_store(_sv0t18, 1, cur.p0);
    sv0_box_store(_sv0t18, 2, cur.p1);
    int _sv0t19 = sv0_box_alloc(3);
    sv0_box_store(_sv0t19, 0, rhs.tag);
    sv0_box_store(_sv0t19, 1, rhs.p0);
    sv0_box_store(_sv0t19, 2, rhs.p1);
    _sv0t17.tag = 2;
    _sv0t17.p0 = op_tag;
    _sv0t17.p1 = _sv0t18;
    _sv0t17.p2 = _sv0t19;
  ...
  Instr _sv0t20;
  int _sv0t21 = sv0_box_alloc(4);
  sv0_box_store(_sv0t21, 0, e.tag);
  sv0_box_store(_sv0t21, 1, e.p0);
  sv0_box_store(_sv0t21, 2, e.p1);
  sv0_box_store(_sv0t21, 3, e.p2);
  _sv0t20.tag = 5;
  _sv0t20.p0 = owner;
  _sv0t20.p1 = fnl;
  _sv0t20.p2 = _sv0t21;
  int _sv0t22 = sv0_box_alloc(5);
  sv0_box_store(_sv0t22, 0, _sv0t20.tag);
  sv0_box_store(_sv0t22, 1, _sv0t20.p0);
  sv0_box_store(_sv0t22, 2, _sv0t20.p1);
  sv0_box_store(_sv0t22, 3, _sv0t20.p2);
  sv0_box_store(_sv0t22, 4, _sv0t20.p3);
  ...
  while ((w >= 0)) {
    ...
    if ((w > 0)) {
      int _sv0t23 = (w - 1);
      int _sv0t24 = sv0_vec_get(temps, _sv0t23);
      up = _sv0t24;
    } else {
    }
    Expr wb;
    int _sv0t25 = sv0_vec_get(temps, w);
    wb.tag = 1;
    wb.p0 = _sv0t25;
    Instr _sv0t26;
    int _sv0t27 = sv0_vec_get(chain, w);
    int _sv0t28 = sv0_box_alloc(4);
    sv0_box_store(_sv0t28, 0, wb.tag);
    sv0_box_store(_sv0t28, 1, wb.p0);
    sv0_box_store(_sv0t28, 2, wb.p1);
    sv0_box_store(_sv0t28, 3, wb.p2);
    _sv0t26.tag = 5;
    _sv0t26.p0 = up;
    _sv0t26.p1 = _sv0t27;
    _sv0t26.p2 = _sv0t28;
...
static const char* collect_typedefs_str(int item_tags, int item_names, int item_field_counts, int item_vmax, int sf_names, int sf_types, const char* source, int starts, int ends, int sn, int en, int item_d3, int evpv, int evptn) {
  ...
  int ci = 0;
  while ((ci < n)) {
    int _sv0t4 = sv0_vec_get(item_tags, ci);
    if ((_sv0t4 == 1)) {
      int cj = 0;
      while (1) {
        int _sv0t5 = sv0_vec_get(item_field_counts, ci);
        int _sv0t11 = (cj < _sv0t5);
        if ((!_sv0t11)) {
          break;
        } else {
        }
        int _sv0t6 = sv0_vec_get(sfbase, ci);
        int _sv0t7 = (_sv0t6 + cj);
        int _sv0t8 = sv0_vec_get(sf_types, _sv0t7);
        int cth = _sv0t8;
        if ((cth >= 0)) {
          const char* _sv0t9 = handle_to_str(cth, source, starts, ends);
          int _sv0t10 = lower_name_in_handle_list(_sv0t9, en, source, starts, ends);
          if (_sv0t10) {
            enums_first = 1;
          } else {
          }
  ...
  enum_r = "";
  if (enums_first) {
    const char* _sv0t12 = lower_emit_all_enum_tds(item_tags, item_names, item_field_counts, item_vmax, source, starts, ends, item_d3, evpv, evptn);
    enum_r = _sv0t12;
    const char* _sv0t13 = sv0_string_concat(r, enum_r);
    r = _sv0t13;
  } else {
  }
  int _sv0t14 = sv0_vec_new();
  ...
  while (progress) {
    ...
    i = 0;
    while ((i < n)) {
      int _sv0t15 = sv0_vec_get(item_tags, i);
      if ((_sv0t15 == 1)) {
        int _sv0t16 = sv0_vec_get(done, i);
        if ((_sv0t16 == 0)) {
          int ready = 1;
          int fj = 0;
          while (1) {
            int _sv0t17 = sv0_vec_get(item_field_counts, i);
            int _sv0t27 = (fj < _sv0t17);
            if ((!_sv0t27)) {
              break;
            } else {
            }
            int _sv0t18 = sv0_vec_get(sfbase, i);
            int _sv0t19 = (_sv0t18 + fj);
            int _sv0t20 = sv0_vec_get(sf_types, _sv0t19);
            int fth = _sv0t20;
            if ((fth >= 0)) {
              const char* _sv0t21 = handle_to_str(fth, source, starts, ends);
              const char* fname;
              fname = _sv0t21;
              int k2 = 0;
              while ((k2 < n)) {
                if ((k2 != i)) {
                  int _sv0t22 = sv0_vec_get(item_tags, k2);
                  if ((_sv0t22 == 1)) {
                    int _sv0t23 = sv0_vec_get(done, k2);
                    if ((_sv0t23 == 0)) {
                      int _sv0t24 = sv0_vec_get(item_names, k2);
                      const char* _sv0t25 = handle_to_str(_sv0t24, source, starts, ends);
                      int _sv0t26 = sv0_string_eq(_sv0t25, fname);
                      if (_sv0t26) {
                        ready = 0;
                      } else {
                      }
          ...
          if (ready) {
            int _sv0t28 = sv0_vec_get(item_names, i);
            int _sv0t29 = sv0_vec_get(sfbase, i);
            int _sv0t30 = sv0_vec_get(item_field_counts, i);
            const char* _sv0t31 = emit_struct_td(_sv0t28, sf_names, sf_types, _sv0t29, _sv0t30, source, starts, ends, sn, en);
            const char* td;
            td = _sv0t31;
            const char* _sv0t32 = sv0_string_concat(r, td);
            r = _sv0t32;
            sv0_vec_set(done, i, 1);
            progress = 1;
          } else {
          }
        } else {
  ...
  while ((i < n)) {
    ...
    if ((_sv0t34 == 1)) {
      int _sv0t35 = sv0_vec_get(done, i);
      if ((_sv0t35 == 0)) {
        int _sv0t36 = sv0_vec_get(item_names, i);
        int _sv0t37 = sv0_vec_get(sfbase, i);
        int _sv0t38 = sv0_vec_get(item_field_counts, i);
        const char* _sv0t39 = emit_struct_td(_sv0t36, sf_names, sf_types, _sv0t37, _sv0t38, source, starts, ends, sn, en);
        const char* tdc;
        tdc = _sv0t39;
        const char* _sv0t40 = sv0_string_concat(r, tdc);
        r = _sv0t40;
      } else {
      }
    } else {
  ...
  const char* _sv0t41 = lower_emit_all_enum_tds(item_tags, item_names, item_field_counts, item_vmax, source, starts, ends, item_d3, evpv, evptn);
  const char* _sv0t42 = sv0_string_concat(r, _sv0t41);
  return _sv0t42;
}
...
static const char* lower_emit_all_enum_tds(int item_tags, int item_names, int item_field_counts, int item_vmax, const char* source, int starts, int ends, int item_d3, int evpv, int evptn) {
  ...
  int i = 0;
  int _sv0t1 = sv0_vec_new();
  int payload_offset_by_variant = _sv0t1;
  int _sv0t2 = sv0_vec_len(evpv);
  int vn = _sv0t2;
  int running = 0;
  int vi = 0;
  while ((vi < vn)) {
    sv0_vec_push(payload_offset_by_variant, running);
    int _sv0t3 = sv0_vec_get(evpv, vi);
    running = (running + _sv0t3);
    vi = (vi + 1);
  }
  i = 0;
  while ((i < n)) {
    ...
    if ((_sv0t4 == 2)) {
      int _sv0t5 = sv0_vec_get(item_vmax, i);
      int mp = _sv0t5;
      int _sv0t6 = sv0_vec_get(item_d3, i);
      int _sv0t7 = sv0_vec_get(item_field_counts, i);
      int _sv0t8 = enum_slot_ctys_for_item(_sv0t6, _sv0t7, mp, evpv, evptn, payload_offset_by_variant, source, starts, ends);
      int slot_cats = _sv0t8;
      int _sv0t9 = sv0_vec_get(item_names, i);
      const char* _sv0t10 = emit_enum_td(_sv0t9, mp, slot_cats, source, starts, ends);
      const char* td2;
      td2 = _sv0t10;
      const char* _sv0t11 = sv0_string_concat(r, td2);
      r = _sv0t11;
    } else {
    }
    i = (i + 1);
...
static int test_lower_field_chain(void) {
  ...
  sv0_vec_push(et, 6);
  sv0_vec_push(ed1, 0);
  sv0_vec_push(ed2, 601);
  sv0_vec_push(et, 6);
  sv0_vec_push(ed1, 1);
  sv0_vec_push(ed2, 602);
  int _sv0t4 = sv0_vec_new();
  int chain = _sv0t4;
  int _sv0t5 = lower_field_chain(et, ed1, ed2, pp, 2, chain);
  if ((_sv0t5 != 500)) {
    return 1;
  } else {
  }
  int _sv0t6 = sv0_vec_len(chain);
  if ((_sv0t6 != 2)) {
    return 2;
  } else {
  }
  int _sv0t7 = sv0_vec_get(chain, 0);
  if ((_sv0t7 != 601)) {
    return 3;
  } else {
  }
  int _sv0t8 = sv0_vec_get(chain, 1);
  if ((_sv0t8 != 602)) {
    return 4;
  } else {
  }
  int _sv0t9 = sv0_vec_new();
  int one = _sv0t9;
  int _sv0t10 = lower_field_chain(et, ed1, ed2, pp, 1, one);
  int _sv0t11 = (0 - 1);
  if ((_sv0t10 != _sv0t11)) {
    return 5;
  } else {
  }
  int _sv0t12 = sv0_vec_len(one);
  if ((_sv0t12 != 0)) {
    return 6;
  } else {
  }
  int _sv0t13 = lower_field_chain(et, ed1, ed2, pp, 0, one);
  int _sv0t14 = (0 - 1);
  if ((_sv0t13 != _sv0t14)) {
    return 7;
  } else {
  ...
  _sv0t18.tag = 0;
  _sv0t18.p0 = 7;
  int _sv0t19 = lower_nested_field_store(out, ctr, 500, chain, _sv0t17, _sv0t18);
  int _s = _sv0t19;
  int _sv0t20 = sv0_vec_len(out);
  if ((_sv0t20 != 3)) {
    return 8;
  } else {
  }
...
int main(void) {
  ...
  int _sv0t181 = test_lower_field_chain();
  int rfc = _sv0t181;
  if ((rfc != 0)) {
    int _sv0t182 = (1500 + rfc);
    return _sv0t182;
  } else {
  }
  return 0;
