...
static Value lower_tag_struct(int et, int ed1, int ed2, int ed3, int ed4, int pp, int tok_tags, int idx, int ctr, int out_instrs, int enum_names, int enum_tag_offsets, int enum_tag_counts, int enum_tags_flat, int fn_ctx, int builtin_map, int lit_sf_names, const char* source, int starts, int ends, int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int enum_vnames_flat, int enum_max_payloads, int item_fn_row, int match_env_inherit, int import_aliases);
...
static int lower_vecf_tag_at(int tok_tags, int i);
static int lower_vecf_text_is(const char* source, int starts, int ends, int t, const char* want);
static int lower_vecf_same_text(const char* source, int starts, int ends, int a, int b);
static int lower_vecf_type_kind(int tok_tags, const char* source, int starts, int ends, int p);
static int lower_vecf_lookup(int names, int kinds, const char* source, int starts, int ends, int name_tok);
static int lower_vecf_collect_struct(int tok_tags, const char* source, int starts, int ends, int i, int fnames, int fkinds);
static int lower_vecf_params_open(int tok_tags, int i);
static int lower_vecf_close_paren(int tok_tags, int open);
static int lower_vecf_fn_end(int tok_tags, int i);
static int lower_vecf_collect_params(int tok_tags, const char* source, int starts, int ends, int i, int names, int kinds);
static int lower_vecf_collect_lets(int tok_tags, const char* source, int starts, int ends, int lo, int hi, int rnames, int rkinds, int names, int kinds, int map);
static int lower_vecf_param_kinds(int tok_tags, const char* source, int starts, int ends, int i, int out);
static int lower_vecf_fn_param_kind(int pfn_names, int pfn_off, int pfn_cnt, int pkinds, const char* source, int starts, int ends, int name_tok, int argi);
static int lower_vecf_word_type(const char* source, int starts, int ends, int t);
static int lower_vecf_evidence_at(int tok_tags, const char* source, int starts, int ends, int lo, int p, int fnames, int fkinds, int pfn_names, int pfn_off, int pfn_cnt, int pkinds, int retkind, int out);
static int lower_vecf_field_evidence(int fnames, int fkinds, const char* source, int starts, int ends, int field_tok, int out);
static int lower_vecf_resolve_fresh(int tok_tags, const char* source, int starts, int ends, int lo, int body_lo, int hi, int names, int kinds, int fnames, int fkinds, int pfn_names, int pfn_off, int pfn_cnt, int pkinds, int retkind);
static int lower_vecf_accessor_kind(const char* source, int starts, int ends, int t);
static int lower_vecf_site_map(int tok_tags, const char* source, int starts, int ends);
static int lower_index_base_tok(int et, int ed1, int ed2, int pp, int ob);
static int lower_index_bid(int builtin_map, int tok, int plain, int f64_bid);
static int lower_array_lit_is_f64(int et, int ed1, int ed2, int ed3, int pp, int tok_tags, int builtin_map, int idx);
static int lower_vecf_apply(int bm, int sites, int tok_tags);
...
static int lower_ret_ty_head_from_name(const char* cname, int item_tags, int item_names, int item_d2, int tok_tags, const char* source, int starts, int ends);
...
static int lower_mcall_ret_user_ty_tok(int et, int ed2, int mcall_idx, int item_tags, int item_names, int item_d2, int tok_tags, const char* source, int starts, int ends);
...
static int is_wide_primitive_ty_name(const char* name);
...
static int enum_variant_def_tok_lookup_str(int enum_names, int enum_tags_flat, int enum_tag_offsets, int enum_tag_counts, int enum_name, int variant_name, const char* source, int starts, int ends);
...
static int enum_max_payload_lookup_str(int enum_names, int enum_max_payloads, int name, const char* source, int starts, int ends);
...
static const char* enum_slot_category_to_cty(int cat);
static const char* emit_enum_td(int name_h, int max_payload, int slot_cats, const char* source, int starts, int ends);
...
static int enum_payload_ty_name_to_category(const char* name);
static int enum_slot_ctys_for_item(int vbase, int vcount, int max_payload, int evpv, int evptn, int payload_offset_by_variant, const char* source, int starts, int ends);
static const char* collect_typedefs_str(int item_tags, int item_names, int item_field_counts, int item_vmax, int sf_names, int sf_types, const char* source, int starts, int ends, int sn, int en, int item_d3, int evpv, int evptn);
...
static int test_lower_vecf_lookup_kinds(void);
static int lower_vecf_test_lex(const char* src, int tags, int st, int en);
static int lower_vecf_test_site(const char* src, const char* want, int occ, int mode, int expect);
static int test_lower_vecf_site_table(void);
...
static Value lower_lit_to_ir_value(int lit_tag, int tok_pos, int tok_tags, const char* source, int starts, int ends) {
  if ((lit_tag == 0)) {
    int _sv0t0 = decimal_lit_fits_i32(source, starts, ends, tok_pos);
    if (_sv0t0) {
      Value _sv0t1;
      int _sv0t2 = parse_decimal_i32_from_tok(source, starts, ends, tok_pos);
      _sv0t1.tag = 0;
      _sv0t1.p0 = _sv0t2;
      return _sv0t1;
    } else {
  ...
  if ((lit_tag == 5)) {
    int _sv0t4 = sv0_vec_get(tok_tags, tok_pos);
    int tt = _sv0t4;
    if ((tt == 91)) {
      Value _sv0t5;
      _sv0t5.tag = 1;
...
static int decimal_lit_fits_i32(const char* source, int starts, int ends, int tok_pos) {
  ...
  while ((i < 10)) {
    int _sv0t4 = (s + i);
    int _sv0t5 = sv0_string_char_at(source, _sv0t4);
    int c = _sv0t5;
    int _sv0t6 = sv0_string_char_at(i32_max, i);
    int m = _sv0t6;
    if ((c < m)) {
      return 1;
    } else {
  ...
  if ((ct == 1)) {
    ...
    if ((cppc == 1)) {
      ...
      if ((bid == 0)) {
        ...
        if (_sv0t119) {
          int vdst = 0;
          int _sv0t120 = lower_vm_target();
          if ((_sv0t120 == 1)) {
            int _sv0t121 = ctr_fresh(ctr);
            vdst = _sv0t121;
          } else {
          }
          Instr void_call_instr;
          void_call_instr.tag = 11;
          void_call_instr.p0 = vdst;
          void_call_instr.p1 = callee_tok;
          void_call_instr.p2 = arg_vals;
          void_call_instr.p3 = 0;
          int _sv0t122 = sv0_box_alloc(5);
          sv0_box_store(_sv0t122, 0, void_call_instr.tag);
          sv0_box_store(_sv0t122, 1, void_call_instr.p0);
          sv0_box_store(_sv0t122, 2, void_call_instr.p1);
          sv0_box_store(_sv0t122, 3, void_call_instr.p2);
          sv0_box_store(_sv0t122, 4, void_call_instr.p3);
        ...
        int rt_u = 0;
        int _sv0t124 = lower_call_ret_is_string(et, ed1, ed2, pp, idx, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
        if (_sv0t124) {
          int _sv0t125 = lower_string_cty();
          rt_u = _sv0t125;
        } else {
          int _sv0t126 = lower_call_ret_user_ty_tok(et, ed1, ed2, pp, idx, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
          int uty_c = _sv0t126;
          if ((uty_c >= 0)) {
            rt_u = uty_c;
          } else {
        ...
        sv0_box_store(_sv0t127, 0, call_instr.tag);
        sv0_box_store(_sv0t127, 1, call_instr.p0);
        sv0_box_store(_sv0t127, 2, call_instr.p1);
        sv0_box_store(_sv0t127, 3, call_instr.p2);
        sv0_box_store(_sv0t127, 4, call_instr.p3);
      ...
      if ((bid == 14)) {
        if ((args_count >= 1)) {
          int _sv0t129 = sv0_vec_get(arg_vals, 0);
          Value _sv0t130;
          int _sv0t131 = sv0_box_load(_sv0t129, 0);
          _sv0t130.tag = _sv0t131;
          int _sv0t132 = sv0_box_load(_sv0t129, 1);
          _sv0t130.p0 = _sv0t132;
          int _sv0t133 = sv0_box_load(_sv0t129, 2);
          _sv0t130.p1 = _sv0t133;
          Value na_val;
          na_val.tag = (_sv0t130).tag;
          na_val.p0 = (_sv0t130).p0;
          na_val.p1 = (_sv0t130).p1;
          return na_val;
        } else {
        }
        Value _sv0t134;
      ...
      if ((is_void == 1)) {
        ...
        sv0_box_store(_sv0t135, 0, vc.tag);
        sv0_box_store(_sv0t135, 1, vc.p0);
        sv0_box_store(_sv0t135, 2, vc.p1);
        sv0_box_store(_sv0t135, 3, vc.p2);
        sv0_box_store(_sv0t135, 4, vc.p3);
      ...
      sv0_box_store(_sv0t137, 0, bc.tag);
      sv0_box_store(_sv0t137, 1, bc.p0);
      sv0_box_store(_sv0t137, 2, bc.p1);
      sv0_box_store(_sv0t137, 3, bc.p2);
      sv0_box_store(_sv0t137, 4, bc.p3);
      sv0_vec_push(out_instrs, _sv0t137);
      Value _sv0t138;
      _sv0t138.tag = 3;
      _sv0t138.p0 = t;
      return _sv0t138;
    ...
    if ((cppc == 2)) {
      int _sv0t139 = sv0_vec_get(pp, cpps);
      int en_tok = _sv0t139;
      int _sv0t140 = (cpps + 1);
      int _sv0t141 = sv0_vec_get(pp, _sv0t140);
      int vn_tok = _sv0t141;
      int _sv0t142 = enum_tag_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, en_tok, vn_tok, source, starts, ends);
      int k = _sv0t142;
      int t = ec_t;
      Expr tag_e;
      Value _sv0t143;
      ...
      Instr sf_tag;
      int _sv0t145 = (0 - 1);
      int _sv0t146 = sv0_box_alloc(4);
      sv0_box_store(_sv0t146, 0, tag_e.tag);
      sv0_box_store(_sv0t146, 1, tag_e.p0);
      sv0_box_store(_sv0t146, 2, tag_e.p1);
      sv0_box_store(_sv0t146, 3, tag_e.p2);
      sf_tag.tag = 5;
      sf_tag.p0 = t;
      sf_tag.p1 = _sv0t145;
      sf_tag.p2 = _sv0t146;
      int _sv0t147 = sv0_box_alloc(5);
      sv0_box_store(_sv0t147, 0, sf_tag.tag);
      sv0_box_store(_sv0t147, 1, sf_tag.p0);
      sv0_box_store(_sv0t147, 2, sf_tag.p1);
      sv0_box_store(_sv0t147, 3, sf_tag.p2);
      sv0_box_store(_sv0t147, 4, sf_tag.p3);
      sv0_vec_push(out_instrs, _sv0t147);
      int pi = 0;
      while (1) {
        if ((pi >= args_count)) {
          break;
        ...
        int _sv0t148 = sv0_vec_get(arg_vals, pi);
        Value _sv0t149;
        int _sv0t150 = sv0_box_load(_sv0t148, 0);
        _sv0t149.tag = _sv0t150;
        int _sv0t151 = sv0_box_load(_sv0t148, 1);
        _sv0t149.p0 = _sv0t151;
        int _sv0t152 = sv0_box_load(_sv0t148, 2);
        _sv0t149.p1 = _sv0t152;
        Value pv;
        pv.tag = (_sv0t149).tag;
        pv.p0 = (_sv0t149).p0;
        pv.p1 = (_sv0t149).p1;
        Expr _sv0t153 = value_to_expr(pv);
        Expr pe;
        pe = _sv0t153;
        int _sv0t154 = (0 - 10);
        int field_sentinel = (_sv0t154 - pi);
        Instr sf_p;
        int _sv0t155 = sv0_box_alloc(4);
        sv0_box_store(_sv0t155, 0, pe.tag);
        sv0_box_store(_sv0t155, 1, pe.p0);
        sv0_box_store(_sv0t155, 2, pe.p1);
        sv0_box_store(_sv0t155, 3, pe.p2);
        sf_p.tag = 5;
        sf_p.p0 = t;
        sf_p.p1 = field_sentinel;
        sf_p.p2 = _sv0t155;
        int _sv0t156 = sv0_box_alloc(5);
        sv0_box_store(_sv0t156, 0, sf_p.tag);
        sv0_box_store(_sv0t156, 1, sf_p.p0);
        sv0_box_store(_sv0t156, 2, sf_p.p1);
        sv0_box_store(_sv0t156, 3, sf_p.p2);
        sv0_box_store(_sv0t156, 4, sf_p.p3);
...
static Value lower_tag_struct(int et, int ed1, int ed2, int ed3, int ed4, int pp, int tok_tags, int idx, int ctr, int out_instrs, int enum_names, int enum_tag_offsets, int enum_tag_counts, int enum_tags_flat, int fn_ctx, int builtin_map, int lit_sf_names, const char* source, int starts, int ends, int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int enum_vnames_flat, int enum_max_payloads, int item_fn_row, int match_env_inherit, int import_aliases) {
  ...
  if ((ppc_s == 2)) {
    int _sv0t6 = sv0_vec_get(pp, pps_s);
    int en_tok_s24 = _sv0t6;
    int _sv0t7 = (pps_s + 1);
    int _sv0t8 = sv0_vec_get(pp, _sv0t7);
    int vn_tok_s24 = _sv0t8;
    int _sv0t9;
    int _sv0t10;
    if ((en_tok_s24 >= 0)) {
      int _sv0t12 = sv0_vec_len(starts);
      int _sv0t11;
      if ((en_tok_s24 < _sv0t12)) {
        int _sv0t13;
        if ((vn_tok_s24 >= 0)) {
          int _sv0t14 = sv0_vec_len(starts);
          _sv0t13 = (vn_tok_s24 < _sv0t14);
        } else {
          _sv0t13 = 0;
        }
        _sv0t11 = _sv0t13;
    ...
    _sv0t9 = _sv0t10;
    int toks_ok_s24 = _sv0t9;
    int _sv0t15;
    int _sv0t16;
    if (toks_ok_s24) {
      int _sv0t17 = enum_tag_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, en_tok_s24, vn_tok_s24, source, starts, ends);
      _sv0t16 = _sv0t17;
    } else {
      _sv0t16 = (0 - 2);
    }
    ...
    int k_s24 = _sv0t15;
    if ((k_s24 >= 0)) {
      int _sv0t18 = ctr_fresh(ctr);
      int t_e24 = _sv0t18;
      Instr _sv0t19;
      _sv0t19.tag = 2;
      _sv0t19.p0 = en_tok_s24;
      _sv0t19.p1 = t_e24;
      ...
      sv0_box_store(_sv0t25, 0, tag_e24.tag);
      sv0_box_store(_sv0t25, 1, tag_e24.p0);
      sv0_box_store(_sv0t25, 2, tag_e24.p1);
      sv0_box_store(_sv0t25, 3, tag_e24.p2);
      _sv0t23.tag = 5;
      _sv0t23.p0 = t_e24;
      _sv0t23.p1 = _sv0t24;
      _sv0t23.p2 = _sv0t25;
      int _sv0t26 = sv0_box_alloc(5);
      sv0_box_store(_sv0t26, 0, _sv0t23.tag);
      sv0_box_store(_sv0t26, 1, _sv0t23.p0);
      sv0_box_store(_sv0t26, 2, _sv0t23.p1);
      sv0_box_store(_sv0t26, 3, _sv0t23.p2);
      sv0_box_store(_sv0t26, 4, _sv0t23.p3);
      sv0_vec_push(out_instrs, _sv0t26);
      int _sv0t27 = enum_variant_def_tok_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, en_tok_s24, vn_tok_s24, source, starts, ends);
      int def_tok_e24 = _sv0t27;
      int fi_e24 = 0;
      while ((fi_e24 < fc_s)) {
        int _sv0t28 = (fi_e24 * 2);
        int _sv0t29 = (sf0 + _sv0t28);
        int _sv0t30 = (_sv0t29 + 1);
        int _sv0t31 = sv0_vec_get(lit_sf_names, _sv0t30);
        int ch_e24 = _sv0t31;
        Value _sv0t32 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, ch_e24, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
        Value fv_e24;
        fv_e24 = _sv0t32;
        int _sv0t33 = (fi_e24 * 2);
        int _sv0t34 = (sf0 + _sv0t33);
        int _sv0t35 = sv0_vec_get(lit_sf_names, _sv0t34);
        int fnm_e24 = _sv0t35;
        int pi_e24 = (0 - 1);
        if ((def_tok_e24 >= 0)) {
          int _sv0t36 = lower_pat_struct_field_payload_index(tok_tags, source, starts, ends, def_tok_e24, fc_s, fnm_e24);
          pi_e24 = _sv0t36;
        } else {
        }
        if ((pi_e24 < 0)) {
        ...
        int _sv0t37 = (0 - 10);
        int pay_e24 = (_sv0t37 - pi_e24);
        Instr _sv0t38;
        Expr _sv0t39 = value_to_expr(fv_e24);
        ...
        sv0_box_store(_sv0t40, 3, _sv0t39.p2);
        _sv0t38.tag = 5;
        _sv0t38.p0 = t_e24;
        _sv0t38.p1 = pay_e24;
        _sv0t38.p2 = _sv0t40;
        ...
        sv0_box_store(_sv0t41, 4, _sv0t38.p3);
  ...
  int _sv0t43 = (pps_s + ppc_s);
  int sty_idx = (_sv0t43 - 1);
  int _sv0t44 = sv0_vec_get(pp, sty_idx);
  int sty_tok = _sv0t44;
  int _sv0t45 = ctr_fresh(ctr);
  int t_s = _sv0t45;
  Instr decl_s;
  decl_s.tag = 2;
  decl_s.p0 = sty_tok;
  decl_s.p1 = t_s;
  int _sv0t46 = sv0_box_alloc(5);
  sv0_box_store(_sv0t46, 0, decl_s.tag);
  sv0_box_store(_sv0t46, 1, decl_s.p0);
  sv0_box_store(_sv0t46, 2, decl_s.p1);
  sv0_box_store(_sv0t46, 3, decl_s.p2);
  sv0_box_store(_sv0t46, 4, decl_s.p3);
  sv0_vec_push(out_instrs, _sv0t46);
  int fi_s = 0;
  while ((fi_s < fc_s)) {
    int _sv0t47 = (fi_s * 2);
    int _sv0t48 = (sf0 + _sv0t47);
    int _sv0t49 = (_sv0t48 + 1);
    int _sv0t50 = sv0_vec_get(lit_sf_names, _sv0t49);
    int ch_s = _sv0t50;
    Value _sv0t51 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, ch_s, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value fv_s;
    fv_s = _sv0t51;
    Expr _sv0t52 = value_to_expr(fv_s);
    Expr fe_s;
    fe_s = _sv0t52;
    int _sv0t53 = (fi_s * 2);
    int _sv0t54 = (sf0 + _sv0t53);
    int _sv0t55 = sv0_vec_get(lit_sf_names, _sv0t54);
    int fnm_s = _sv0t55;
    Instr stf_s;
    int _sv0t56 = sv0_box_alloc(4);
    sv0_box_store(_sv0t56, 0, fe_s.tag);
    sv0_box_store(_sv0t56, 1, fe_s.p0);
    sv0_box_store(_sv0t56, 2, fe_s.p1);
    sv0_box_store(_sv0t56, 3, fe_s.p2);
    stf_s.tag = 5;
    stf_s.p0 = t_s;
    stf_s.p1 = fnm_s;
    stf_s.p2 = _sv0t56;
    int _sv0t57 = sv0_box_alloc(5);
    sv0_box_store(_sv0t57, 0, stf_s.tag);
    sv0_box_store(_sv0t57, 1, stf_s.p0);
    sv0_box_store(_sv0t57, 2, stf_s.p1);
    sv0_box_store(_sv0t57, 3, stf_s.p2);
    sv0_box_store(_sv0t57, 4, stf_s.p3);
...
static Value lower_tag_block(int et, int ed1, int ed2, int ed3, int ed4, int pp, int tok_tags, int idx, int ctr, int out_instrs, int enum_names, int enum_tag_offsets, int enum_tag_counts, int enum_tags_flat, int fn_ctx, int builtin_map, int lit_sf_names, const char* source, int starts, int ends, int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int enum_vnames_flat, int enum_max_payloads, int item_fn_row, int match_env_inherit, int import_aliases, int for_effect, int into_dest) {
  ...
  while (1) {
    ...
    if ((s_tag == 27)) {
      ...
      int deferred_ptr_inner = (0 - 1);
      int deferred_ptr_kind = 0;
      if ((init_idx >= 0)) {
        ...
        if ((_sv0t8 == 24)) {
          ...
          if ((sppc == 2)) {
            if ((fc_lb > 0)) {
              int _sv0t12 = sv0_vec_get(pp, spps_lb);
              int en_tok_lb = _sv0t12;
              int _sv0t13 = (spps_lb + 1);
              int _sv0t14 = sv0_vec_get(pp, _sv0t13);
              int vn_tok_lb = _sv0t14;
              int _sv0t15;
              int _sv0t16;
              if ((en_tok_lb >= 0)) {
                int _sv0t18 = sv0_vec_len(starts);
                int _sv0t17;
                if ((en_tok_lb < _sv0t18)) {
                  int _sv0t19;
                  if ((vn_tok_lb >= 0)) {
                    int _sv0t20 = sv0_vec_len(starts);
                    _sv0t19 = (vn_tok_lb < _sv0t20);
                  } else {
                    _sv0t19 = 0;
                  }
                  _sv0t17 = _sv0t19;
              ...
              _sv0t15 = _sv0t16;
              int toks_ok_lb2 = _sv0t15;
              int _sv0t21;
              int _sv0t22;
              if (toks_ok_lb2) {
                int _sv0t23 = enum_tag_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, en_tok_lb, vn_tok_lb, source, starts, ends);
                _sv0t22 = _sv0t23;
              } else {
                _sv0t22 = (0 - 2);
              }
              ...
              if ((k_lb2 >= 0)) {
                ...
                sv0_box_store(_sv0t32, 0, _sv0t26.tag);
                sv0_box_store(_sv0t32, 1, _sv0t26.p0);
                sv0_box_store(_sv0t32, 2, _sv0t26.p1);
                sv0_box_store(_sv0t32, 3, _sv0t26.p2);
                sv0_box_store(_sv0t32, 4, _sv0t26.p3);
                sv0_vec_push(out_instrs, _sv0t32);
                int _sv0t33 = enum_variant_def_tok_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, en_tok_lb, vn_tok_lb, source, starts, ends);
                int def_tok_lb = _sv0t33;
                int _sv0t34 = sv0_vec_get(ed4, init_idx);
                int sf0_lb2 = _sv0t34;
                int fi_lb2 = 0;
                while ((fi_lb2 < fc_lb)) {
                  int _sv0t35 = (fi_lb2 * 2);
                  int _sv0t36 = (sf0_lb2 + _sv0t35);
                  int _sv0t37 = (_sv0t36 + 1);
                  int _sv0t38 = sv0_vec_get(lit_sf_names, _sv0t37);
                  int ch_lb2 = _sv0t38;
                  Value _sv0t39 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, ch_lb2, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
                  Value fv_lb2;
                  fv_lb2 = _sv0t39;
                  int _sv0t40 = (fi_lb2 * 2);
                  int _sv0t41 = (sf0_lb2 + _sv0t40);
                  int _sv0t42 = sv0_vec_get(lit_sf_names, _sv0t41);
                  int fnm_lb2 = _sv0t42;
                  int pi_lb2 = (0 - 1);
                  if ((def_tok_lb >= 0)) {
                    int _sv0t43 = lower_pat_struct_field_payload_index(tok_tags, source, starts, ends, def_tok_lb, fc_lb, fnm_lb2);
                    pi_lb2 = _sv0t43;
                  } else {
                  }
                  if ((pi_lb2 < 0)) {
                  ...
                  int _sv0t44 = (0 - 10);
                  int pay_lb2 = (_sv0t44 - pi_lb2);
                  Instr _sv0t45;
                  Expr _sv0t46 = value_to_expr(fv_lb2);
                  ...
                  sv0_box_store(_sv0t48, 4, _sv0t45.p3);
            ...
            if ((sppc >= 1)) {
              if ((fc_lb > 0)) {
                int _sv0t49 = (spps_lb + sppc);
                int _sv0t50 = (_sv0t49 - 1);
                ...
                sv0_vec_push(out_instrs, _sv0t53);
                let_declared = 1;
                ...
                while ((fi_lb < fc_lb)) {
                  int _sv0t55 = (fi_lb * 2);
                  int _sv0t56 = (sf0_lb + _sv0t55);
                  int _sv0t57 = (_sv0t56 + 1);
                  int _sv0t58 = sv0_vec_get(lit_sf_names, _sv0t57);
                  int ch_lb = _sv0t58;
                  Value _sv0t59 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, ch_lb, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
                  Value fv_lb;
                  fv_lb = _sv0t59;
                  int _sv0t60 = (fi_lb * 2);
                  int _sv0t61 = (sf0_lb + _sv0t60);
                  int _sv0t62 = sv0_vec_get(lit_sf_names, _sv0t61);
                  int fnm_lb = _sv0t62;
                  Instr _sv0t63;
                  Expr _sv0t64 = value_to_expr(fv_lb);
                  ...
                  sv0_box_store(_sv0t66, 0, _sv0t63.tag);
                  sv0_box_store(_sv0t66, 1, _sv0t63.p0);
                  sv0_box_store(_sv0t66, 2, _sv0t63.p1);
                  sv0_box_store(_sv0t66, 3, _sv0t63.p2);
                  sv0_box_store(_sv0t66, 4, _sv0t63.p3);
        ...
        int _sv0t67 = sv0_vec_get(et, init_idx);
        if ((_sv0t67 == 1)) {
          int _sv0t68 = sv0_vec_get(ed2, init_idx);
          if ((_sv0t68 == 2)) {
            int _sv0t69 = sv0_vec_get(ed1, init_idx);
            int _sv0t70 = sv0_vec_get(pp, _sv0t69);
            int ety_tok = _sv0t70;
            int _sv0t71 = sv0_vec_get(ed1, init_idx);
            int _sv0t72 = (_sv0t71 + 1);
            int _sv0t73 = sv0_vec_get(pp, _sv0t72);
            int vn_tok_lb = _sv0t73;
            Instr _sv0t74;
            _sv0t74.tag = 2;
            _sv0t74.p0 = ety_tok;
            _sv0t74.p1 = name_tok;
            ...
            sv0_box_store(_sv0t75, 3, _sv0t74.p2);
            sv0_box_store(_sv0t75, 4, _sv0t74.p3);
            sv0_vec_push(out_instrs, _sv0t75);
            int _sv0t76 = enum_tag_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, ety_tok, vn_tok_lb, source, starts, ends);
            int k_lb = _sv0t76;
            Expr tag_elb;
            Value _sv0t77;
            _sv0t77.tag = 0;
            _sv0t77.p0 = k_lb;
            int _sv0t78 = sv0_box_alloc(3);
            sv0_box_store(_sv0t78, 0, _sv0t77.tag);
            sv0_box_store(_sv0t78, 1, _sv0t77.p0);
            sv0_box_store(_sv0t78, 2, _sv0t77.p1);
            tag_elb.tag = 0;
            tag_elb.p0 = _sv0t78;
            Instr _sv0t79;
            int _sv0t80 = (0 - 1);
            int _sv0t81 = sv0_box_alloc(4);
            sv0_box_store(_sv0t81, 0, tag_elb.tag);
            sv0_box_store(_sv0t81, 1, tag_elb.p0);
            sv0_box_store(_sv0t81, 2, tag_elb.p1);
            sv0_box_store(_sv0t81, 3, tag_elb.p2);
            _sv0t79.tag = 5;
            _sv0t79.p0 = name_tok;
            _sv0t79.p1 = _sv0t80;
            _sv0t79.p2 = _sv0t81;
            ...
            sv0_box_store(_sv0t82, 4, _sv0t79.p3);
        ...
        int _sv0t83 = sv0_vec_get(et, init_idx);
        if ((_sv0t83 == 4)) {
          int _sv0t84 = sv0_vec_get(ed1, init_idx);
          int callee_cb = _sv0t84;
          int _sv0t85 = sv0_vec_get(et, callee_cb);
          if ((_sv0t85 == 1)) {
            int _sv0t86 = sv0_vec_get(ed2, callee_cb);
            if ((_sv0t86 == 2)) {
              int _sv0t87 = sv0_vec_get(ed1, callee_cb);
              int _sv0t88 = sv0_vec_get(pp, _sv0t87);
              int en_tok_lb = _sv0t88;
              int _sv0t89 = sv0_vec_get(ed1, callee_cb);
              int _sv0t90 = (_sv0t89 + 1);
              int _sv0t91 = sv0_vec_get(pp, _sv0t90);
              int vnc_lb = _sv0t91;
              Instr _sv0t92;
              _sv0t92.tag = 2;
              _sv0t92.p0 = en_tok_lb;
              _sv0t92.p1 = name_tok;
              ...
              int _sv0t94 = enum_tag_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, en_tok_lb, vnc_lb, source, starts, ends);
              int kc_lb = _sv0t94;
              int _sv0t95 = sv0_vec_get(ed2, init_idx);
              int af_lb = _sv0t95;
              int _sv0t96 = sv0_vec_get(ed3, init_idx);
              int ac_lb = _sv0t96;
              int _sv0t97 = sv0_vec_get(ed4, init_idx);
              int asc_lb = _sv0t97;
              int _sv0t98 = sv0_vec_new();
              int avals_lb = _sv0t98;
              int aic_lb = 0;
              while ((aic_lb < ac_lb)) {
                int _sv0t99 = block_stmt_index(pp, af_lb, asc_lb, aic_lb);
                int aidx_lb = _sv0t99;
                Value _sv0t100 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, aidx_lb, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
                Value avc_lb;
                avc_lb = _sv0t100;
                int _sv0t101 = sv0_box_alloc(3);
                sv0_box_store(_sv0t101, 0, avc_lb.tag);
                sv0_box_store(_sv0t101, 1, avc_lb.p0);
                sv0_box_store(_sv0t101, 2, avc_lb.p1);
                sv0_vec_push(avals_lb, _sv0t101);
                aic_lb = (aic_lb + 1);
              }
              Expr tagc_lb;
              Value _sv0t102;
              _sv0t102.tag = 0;
              _sv0t102.p0 = kc_lb;
              int _sv0t103 = sv0_box_alloc(3);
              sv0_box_store(_sv0t103, 0, _sv0t102.tag);
              sv0_box_store(_sv0t103, 1, _sv0t102.p0);
              sv0_box_store(_sv0t103, 2, _sv0t102.p1);
              tagc_lb.tag = 0;
              tagc_lb.p0 = _sv0t103;
              Instr _sv0t104;
              int _sv0t105 = (0 - 1);
              int _sv0t106 = sv0_box_alloc(4);
              sv0_box_store(_sv0t106, 0, tagc_lb.tag);
              sv0_box_store(_sv0t106, 1, tagc_lb.p0);
              sv0_box_store(_sv0t106, 2, tagc_lb.p1);
              sv0_box_store(_sv0t106, 3, tagc_lb.p2);
              _sv0t104.tag = 5;
              _sv0t104.p0 = name_tok;
              _sv0t104.p1 = _sv0t105;
              _sv0t104.p2 = _sv0t106;
              int _sv0t107 = sv0_box_alloc(5);
              ...
              sv0_box_store(_sv0t107, 4, _sv0t104.p3);
              sv0_vec_push(out_instrs, _sv0t107);
              int pic_lb = 0;
              while ((pic_lb < ac_lb)) {
                int _sv0t108 = sv0_vec_get(avals_lb, pic_lb);
                Value _sv0t109;
                int _sv0t110 = sv0_box_load(_sv0t108, 0);
                _sv0t109.tag = _sv0t110;
                int _sv0t111 = sv0_box_load(_sv0t108, 1);
                _sv0t109.p0 = _sv0t111;
                int _sv0t112 = sv0_box_load(_sv0t108, 2);
                _sv0t109.p1 = _sv0t112;
                Value pvc_lb;
                pvc_lb.tag = (_sv0t109).tag;
                pvc_lb.p0 = (_sv0t109).p0;
                pvc_lb.p1 = (_sv0t109).p1;
                int _sv0t113 = (0 - 10);
                int fsc_lb = (_sv0t113 - pic_lb);
                Instr _sv0t114;
                Expr _sv0t115 = value_to_expr(pvc_lb);
                int _sv0t116 = sv0_box_alloc(4);
                sv0_box_store(_sv0t116, 0, _sv0t115.tag);
                sv0_box_store(_sv0t116, 1, _sv0t115.p0);
                sv0_box_store(_sv0t116, 2, _sv0t115.p1);
                sv0_box_store(_sv0t116, 3, _sv0t115.p2);
                _sv0t114.tag = 5;
                _sv0t114.p0 = name_tok;
                _sv0t114.p1 = fsc_lb;
                _sv0t114.p2 = _sv0t116;
                int _sv0t117 = sv0_box_alloc(5);
                sv0_box_store(_sv0t117, 0, _sv0t114.tag);
                sv0_box_store(_sv0t117, 1, _sv0t114.p0);
                sv0_box_store(_sv0t117, 2, _sv0t114.p1);
                sv0_box_store(_sv0t117, 3, _sv0t114.p2);
                sv0_box_store(_sv0t117, 4, _sv0t114.p3);
        ...
        int _sv0t118 = lower_expr_yields_string(et, ed1, ed2, ed3, pp, init_idx, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
        if (_sv0t118) {
          int _sv0t119 = lower_string_cty();
          deferred_ty = _sv0t119;
          has_deferred_decl = 1;
          let_declared = 1;
        } else {
        }
        if ((let_declared == 0)) {
          int _sv0t120 = lower_call_ret_is_string(et, ed1, ed2, pp, init_idx, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
          if (_sv0t120) {
            Instr decl_scb;
            int _sv0t121 = lower_string_cty();
            decl_scb.tag = 2;
            decl_scb.p0 = _sv0t121;
            decl_scb.p1 = name_tok;
            int _sv0t122 = sv0_box_alloc(5);
            sv0_box_store(_sv0t122, 0, decl_scb.tag);
            sv0_box_store(_sv0t122, 1, decl_scb.p0);
            sv0_box_store(_sv0t122, 2, decl_scb.p1);
            sv0_box_store(_sv0t122, 3, decl_scb.p2);
            sv0_box_store(_sv0t122, 4, decl_scb.p3);
            sv0_vec_push(out_instrs, _sv0t122);
            let_declared = 1;
          } else {
          }
        ...
        if ((let_declared == 0)) {
          int _sv0t123 = lower_init_ident_is_string_param(et, ed1, ed2, pp, init_idx, item_tags, item_names, item_d3, tok_tags, source, starts, ends, item_fn_row);
          if (_sv0t123) {
            Instr decl_spb;
            int _sv0t124 = lower_string_cty();
            decl_spb.tag = 2;
            decl_spb.p0 = _sv0t124;
            decl_spb.p1 = name_tok;
            int _sv0t125 = sv0_box_alloc(5);
            sv0_box_store(_sv0t125, 0, decl_spb.tag);
            sv0_box_store(_sv0t125, 1, decl_spb.p0);
            sv0_box_store(_sv0t125, 2, decl_spb.p1);
            sv0_box_store(_sv0t125, 3, decl_spb.p2);
            sv0_box_store(_sv0t125, 4, decl_spb.p3);
        ...
        if ((let_declared == 0)) {
          int _sv0t126 = lower_call_ret_user_ty_tok(et, ed1, ed2, pp, init_idx, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
          int uty_lb = _sv0t126;
          if ((uty_lb >= 0)) {
            deferred_ty = uty_lb;
            has_deferred_decl = 1;
        ...
        if ((let_declared == 0)) {
          int _sv0t127 = lower_mcall_ret_user_ty_tok(et, ed2, init_idx, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
          int mty_lb = _sv0t127;
          if ((mty_lb >= 0)) {
            deferred_ty = mty_lb;
            has_deferred_decl = 1;
        ...
        if ((let_declared == 0)) {
          int _sv0t128 = sv0_vec_get(ed2, s_idx);
          int _sv0t129 = lower_typed_box_deref_let(et, ed1, ed2, ed3, ed4, pp, tok_tags, init_idx, name_tok, _sv0t128, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
          int bdb = _sv0t129;
          if ((bdb == 1)) {
            let_declared = 1;
            init_handled = 1;
        ...
        if ((let_declared == 0)) {
          int _sv0t130 = sv0_vec_get(ed2, s_idx);
          int ann_tok_lbp = _sv0t130;
          if ((ann_tok_lbp >= 0)) {
            int _sv0t131 = lower_tok_in_bounds(ann_tok_lbp, starts, ends);
            if (_sv0t131) {
              int _sv0t132 = sv0_vec_get(tok_tags, ann_tok_lbp);
              if ((_sv0t132 == 24)) {
                int _sv0t133 = (ann_tok_lbp + 1);
                int _sv0t134 = sv0_vec_get(tok_tags, _sv0t133);
                int kind_tag_lb = _sv0t134;
                if ((kind_tag_lb == 58)) {
                  deferred_ptr_kind = 1;
                } else {
                }
        ...
        if ((let_declared == 0)) {
          if ((deferred_ptr_kind == 0)) {
            int _sv0t135 = sv0_vec_get(ed2, s_idx);
            int ann_tok_lb = _sv0t135;
            if ((ann_tok_lb >= 0)) {
              int _sv0t136 = lower_tok_in_bounds(ann_tok_lb, starts, ends);
              if (_sv0t136) {
                const char* _sv0t137 = handle_to_str(ann_tok_lb, source, starts, ends);
                const char* ann_name_lb;
                ann_name_lb = _sv0t137;
                int _sv0t138 = is_wide_primitive_ty_name(ann_name_lb);
                if (_sv0t138) {
      ...
      if ((init_idx >= 0)) {
        if ((init_handled == 0)) {
          Expr _sv0t139 = lower_expr_with_instrs(et, ed1, ed2, ed3, ed4, pp, tok_tags, init_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
          Expr e;
          e = _sv0t139;
          if ((deferred_ptr_kind != 0)) {
            Instr _sv0t140;
            _sv0t140.tag = 16;
            _sv0t140.p0 = deferred_ptr_inner;
            _sv0t140.p1 = deferred_ptr_kind;
            _sv0t140.p2 = name_tok;
            int _sv0t141 = sv0_box_alloc(5);
            sv0_box_store(_sv0t141, 0, _sv0t140.tag);
            sv0_box_store(_sv0t141, 1, _sv0t140.p0);
            sv0_box_store(_sv0t141, 2, _sv0t140.p1);
            sv0_box_store(_sv0t141, 3, _sv0t140.p2);
            sv0_box_store(_sv0t141, 4, _sv0t140.p3);
            sv0_vec_push(out_instrs, _sv0t141);
          } else {
            if ((has_deferred_decl == 1)) {
              Instr _sv0t142;
              _sv0t142.tag = 2;
              _sv0t142.p0 = deferred_ty;
              _sv0t142.p1 = name_tok;
              int _sv0t143 = sv0_box_alloc(5);
              sv0_box_store(_sv0t143, 0, _sv0t142.tag);
              sv0_box_store(_sv0t143, 1, _sv0t142.p0);
              sv0_box_store(_sv0t143, 2, _sv0t142.p1);
              sv0_box_store(_sv0t143, 3, _sv0t142.p2);
              sv0_box_store(_sv0t143, 4, _sv0t142.p3);
              sv0_vec_push(out_instrs, _sv0t143);
            } else {
              if ((let_declared == 0)) {
                Instr _sv0t144;
                _sv0t144.tag = 1;
                _sv0t144.p0 = name_tok;
                int _sv0t145 = sv0_box_alloc(5);
                sv0_box_store(_sv0t145, 0, _sv0t144.tag);
                sv0_box_store(_sv0t145, 1, _sv0t144.p0);
                sv0_box_store(_sv0t145, 2, _sv0t144.p1);
                sv0_box_store(_sv0t145, 3, _sv0t144.p2);
                sv0_box_store(_sv0t145, 4, _sv0t144.p3);
          ...
          Instr asgn;
          int _sv0t146 = sv0_box_alloc(4);
          sv0_box_store(_sv0t146, 0, e.tag);
          sv0_box_store(_sv0t146, 1, e.p0);
          sv0_box_store(_sv0t146, 2, e.p1);
          sv0_box_store(_sv0t146, 3, e.p2);
          asgn.tag = 4;
          asgn.p0 = name_tok;
          asgn.p1 = _sv0t146;
          int _sv0t147 = sv0_box_alloc(5);
          sv0_box_store(_sv0t147, 0, asgn.tag);
          sv0_box_store(_sv0t147, 1, asgn.p0);
          sv0_box_store(_sv0t147, 2, asgn.p1);
          sv0_box_store(_sv0t147, 3, asgn.p2);
          sv0_box_store(_sv0t147, 4, asgn.p3);
          sv0_vec_push(out_instrs, _sv0t147);
        } else {
        }
      } else {
        if ((deferred_ptr_kind != 0)) {
          Instr _sv0t148;
          _sv0t148.tag = 16;
          _sv0t148.p0 = deferred_ptr_inner;
          _sv0t148.p1 = deferred_ptr_kind;
          _sv0t148.p2 = name_tok;
          int _sv0t149 = sv0_box_alloc(5);
          sv0_box_store(_sv0t149, 0, _sv0t148.tag);
          sv0_box_store(_sv0t149, 1, _sv0t148.p0);
          sv0_box_store(_sv0t149, 2, _sv0t148.p1);
          sv0_box_store(_sv0t149, 3, _sv0t148.p2);
          sv0_box_store(_sv0t149, 4, _sv0t148.p3);
          sv0_vec_push(out_instrs, _sv0t149);
        } else {
          if ((let_declared == 0)) {
            Instr _sv0t150;
            _sv0t150.tag = 1;
            _sv0t150.p0 = name_tok;
            int _sv0t151 = sv0_box_alloc(5);
            sv0_box_store(_sv0t151, 0, _sv0t150.tag);
            sv0_box_store(_sv0t151, 1, _sv0t150.p0);
            sv0_box_store(_sv0t151, 2, _sv0t150.p1);
            sv0_box_store(_sv0t151, 3, _sv0t150.p2);
            sv0_box_store(_sv0t151, 4, _sv0t150.p3);
            sv0_vec_push(out_instrs, _sv0t151);
          } else {
          }
        }
      }
    ...
    if ((s_tag == 28)) {
      int _sv0t152 = sv0_vec_get(ed1, s_idx);
      int semi_idx = _sv0t152;
      int _sv0t153 = lower_for_effect(et, ed1, ed2, ed3, ed4, pp, tok_tags, semi_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      int de1 = _sv0t153;
    } else {
    }
    si = (si + 1);
  ...
  if ((into_dest >= 0)) {
    if ((tail_idx >= 0)) {
      int _sv0t154 = lower_into_var_instrs(et, ed1, ed2, ed3, ed4, pp, tok_tags, tail_idx, into_dest, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    } else {
      Instr _sv0t155;
      Expr _sv0t156;
      Value _sv0t157;
      _sv0t157.tag = 4;
      int _sv0t158 = sv0_box_alloc(3);
      sv0_box_store(_sv0t158, 0, _sv0t157.tag);
      sv0_box_store(_sv0t158, 1, _sv0t157.p0);
      sv0_box_store(_sv0t158, 2, _sv0t157.p1);
      _sv0t156.tag = 0;
      _sv0t156.p0 = _sv0t158;
      int _sv0t159 = sv0_box_alloc(4);
      sv0_box_store(_sv0t159, 0, _sv0t156.tag);
      sv0_box_store(_sv0t159, 1, _sv0t156.p0);
      sv0_box_store(_sv0t159, 2, _sv0t156.p1);
      sv0_box_store(_sv0t159, 3, _sv0t156.p2);
      _sv0t155.tag = 4;
      _sv0t155.p0 = into_dest;
      _sv0t155.p1 = _sv0t159;
      int _sv0t160 = sv0_box_alloc(5);
      sv0_box_store(_sv0t160, 0, _sv0t155.tag);
      sv0_box_store(_sv0t160, 1, _sv0t155.p0);
      sv0_box_store(_sv0t160, 2, _sv0t155.p1);
      sv0_box_store(_sv0t160, 3, _sv0t155.p2);
      sv0_box_store(_sv0t160, 4, _sv0t155.p3);
      sv0_vec_push(out_instrs, _sv0t160);
    }
    Value _sv0t161;
    _sv0t161.tag = 4;
    return _sv0t161;
  ...
  if ((tail_idx >= 0)) {
    if ((for_effect == 1)) {
      int _sv0t162 = lower_for_effect(et, ed1, ed2, ed3, ed4, pp, tok_tags, tail_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Value _sv0t163;
      _sv0t163.tag = 4;
      return _sv0t163;
    } else {
    }
    Value _sv0t164 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, tail_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    return _sv0t164;
  } else {
  }
  Value _sv0t165;
...
static Value lower_tag_try(int et, int ed1, int ed2, int ed3, int ed4, int pp, int tok_tags, int idx, int ctr, int out_instrs, int enum_names, int enum_tag_offsets, int enum_tag_counts, int enum_tags_flat, int fn_ctx, int builtin_map, int lit_sf_names, const char* source, int starts, int ends, int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int enum_vnames_flat, int enum_max_payloads, int item_fn_row, int match_env_inherit, int import_aliases) {
    ...
    if ((callee_tag == 1)) {
      ...
      if ((cppc == 1)) {
        ...
        while (1) {
          ...
          int _sv0t11 = lower_tok_str_eq(source, starts, ends, fn_name_h, callee_tok);
          if (_sv0t11) {
            int _sv0t12 = (1 + fn_count);
            int _sv0t13 = (_sv0t12 + fi);
            int _sv0t14 = sv0_vec_get(fn_ctx, _sv0t13);
            enum_name = _sv0t14;
          } else {
          }
          fi = (fi + 1);
      ...
      if ((cppc == 2)) {
        int _sv0t15 = sv0_vec_get(ed1, callee_idx);
        int cpps2 = _sv0t15;
        int _sv0t16 = sv0_vec_get(pp, cpps2);
        enum_name = _sv0t16;
      } else {
      }
    } else {
  ...
  if ((inner_tag == 1)) {
    int _sv0t17 = sv0_vec_get(ed1, inner);
    int pps = _sv0t17;
    int _sv0t18 = sv0_vec_get(ed2, inner);
    int ppc = _sv0t18;
    if ((ppc == 2)) {
      int _sv0t19 = sv0_vec_get(pp, pps);
      enum_name = _sv0t19;
    } else {
    }
    if ((ppc == 1)) {
      if ((item_fn_row >= 0)) {
        int _sv0t20 = sv0_vec_len(item_names);
        if ((item_fn_row < _sv0t20)) {
          int _sv0t21 = sv0_vec_get(item_names, item_fn_row);
          int cur_fn_tok = _sv0t21;
          int fj = 0;
          while ((fj < fn_count)) {
            int _sv0t22 = (1 + fj);
            int _sv0t23 = sv0_vec_get(fn_ctx, _sv0t22);
            if ((_sv0t23 == cur_fn_tok)) {
              int _sv0t24 = (1 + fn_count);
              int _sv0t25 = (_sv0t24 + fj);
              int _sv0t26 = sv0_vec_get(fn_ctx, _sv0t25);
              enum_name = _sv0t26;
            } else {
            }
            fj = (fj + 1);
  ...
  int failK = 1;
  Value _sv0t28 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, inner, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
  Value inner_v;
  inner_v = _sv0t28;
  ...
  sv0_box_store(_sv0t30, 0, decl_u.tag);
  sv0_box_store(_sv0t30, 1, decl_u.p0);
  sv0_box_store(_sv0t30, 2, decl_u.p1);
  sv0_box_store(_sv0t30, 3, decl_u.p2);
  sv0_box_store(_sv0t30, 4, decl_u.p3);
  sv0_vec_push(out_instrs, _sv0t30);
  Expr _sv0t31 = value_to_expr(inner_v);
  Expr inner_e;
  inner_e = _sv0t31;
  Instr store_u;
  int _sv0t32 = sv0_box_alloc(4);
  sv0_box_store(_sv0t32, 0, inner_e.tag);
  sv0_box_store(_sv0t32, 1, inner_e.p0);
  sv0_box_store(_sv0t32, 2, inner_e.p1);
  sv0_box_store(_sv0t32, 3, inner_e.p2);
  store_u.tag = 4;
  store_u.p0 = u;
  store_u.p1 = _sv0t32;
  int _sv0t33 = sv0_box_alloc(5);
  sv0_box_store(_sv0t33, 0, store_u.tag);
  sv0_box_store(_sv0t33, 1, store_u.p0);
  sv0_box_store(_sv0t33, 2, store_u.p1);
  sv0_box_store(_sv0t33, 3, store_u.p2);
  sv0_box_store(_sv0t33, 4, store_u.p3);
  sv0_vec_push(out_instrs, _sv0t33);
  ...
  sv0_box_store(_sv0t35, 0, decl_out.tag);
  sv0_box_store(_sv0t35, 1, decl_out.p0);
  sv0_box_store(_sv0t35, 2, decl_out.p1);
  sv0_box_store(_sv0t35, 3, decl_out.p2);
  sv0_box_store(_sv0t35, 4, decl_out.p3);
  ...
  int _sv0t38 = (0 - 1);
  tag_member.tag = 6;
  tag_member.p0 = _sv0t37;
  tag_member.p1 = _sv0t38;
  Value fail_v;
  fail_v.tag = 0;
  fail_v.p0 = failK;
  Expr cond_e;
  int _sv0t39 = sv0_box_alloc(3);
  sv0_box_store(_sv0t39, 0, tag_member.tag);
  sv0_box_store(_sv0t39, 1, tag_member.p0);
  sv0_box_store(_sv0t39, 2, tag_member.p1);
  int _sv0t40 = sv0_box_alloc(3);
  sv0_box_store(_sv0t40, 0, fail_v.tag);
  sv0_box_store(_sv0t40, 1, fail_v.p0);
  sv0_box_store(_sv0t40, 2, fail_v.p1);
  cond_e.tag = 2;
  cond_e.p0 = 11;
  cond_e.p1 = _sv0t39;
  cond_e.p2 = _sv0t40;
  ...
  sv0_box_store(_sv0t44, 0, ret_fail.tag);
  sv0_box_store(_sv0t44, 1, ret_fail.p0);
  sv0_box_store(_sv0t44, 2, ret_fail.p1);
  sv0_box_store(_sv0t44, 3, ret_fail.p2);
  sv0_box_store(_sv0t44, 4, ret_fail.p3);
  sv0_vec_push(then_is, _sv0t44);
  ...
  sv0_box_store(_sv0t46, 0, p0_access.tag);
  sv0_box_store(_sv0t46, 1, p0_access.p0);
  sv0_box_store(_sv0t46, 2, p0_access.p1);
  int _sv0t47 = (0 - 10);
  p0_e.tag = 4;
  p0_e.p0 = _sv0t46;
  p0_e.p1 = _sv0t47;
  ...
  sv0_box_store(_sv0t48, 0, p0_e.tag);
  sv0_box_store(_sv0t48, 1, p0_e.p0);
  sv0_box_store(_sv0t48, 2, p0_e.p1);
  sv0_box_store(_sv0t48, 3, p0_e.p2);
  store_out.tag = 4;
  store_out.p0 = out_t;
  store_out.p1 = _sv0t48;
  int _sv0t49 = sv0_box_alloc(5);
  sv0_box_store(_sv0t49, 0, store_out.tag);
  sv0_box_store(_sv0t49, 1, store_out.p0);
  sv0_box_store(_sv0t49, 2, store_out.p1);
  sv0_box_store(_sv0t49, 3, store_out.p2);
  sv0_box_store(_sv0t49, 4, store_out.p3);
  sv0_vec_push(else_is, _sv0t49);
  Instr if_instr;
  int _sv0t50 = sv0_box_alloc(4);
  sv0_box_store(_sv0t50, 0, cond_e.tag);
  sv0_box_store(_sv0t50, 1, cond_e.p0);
  sv0_box_store(_sv0t50, 2, cond_e.p1);
  sv0_box_store(_sv0t50, 3, cond_e.p2);
  if_instr.tag = 6;
  if_instr.p0 = _sv0t50;
  if_instr.p1 = then_is;
  if_instr.p2 = else_is;
  int _sv0t51 = sv0_box_alloc(5);
  sv0_box_store(_sv0t51, 0, if_instr.tag);
  sv0_box_store(_sv0t51, 1, if_instr.p0);
  sv0_box_store(_sv0t51, 2, if_instr.p1);
  sv0_box_store(_sv0t51, 3, if_instr.p2);
  sv0_box_store(_sv0t51, 4, if_instr.p3);
  sv0_vec_push(out_instrs, _sv0t51);
  Value _sv0t52;
  _sv0t52.tag = 3;
  _sv0t52.p0 = out_t;
  return _sv0t52;
...
static Value lower_expr_to_value(int et, int ed1, int ed2, int ed3, int ed4, int pp, int tok_tags, int idx, int ctr, int out_instrs, int enum_names, int enum_tag_offsets, int enum_tag_counts, int enum_tags_flat, int fn_ctx, int builtin_map, int lit_sf_names, const char* source, int starts, int ends, int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int enum_vnames_flat, int enum_max_payloads, int item_fn_row, int match_env_inherit, int import_aliases) {
  ...
  if ((tag == 2)) {
    ...
    if ((unop_tag == 4)) {
      ...
      if ((ot == 1)) {
        ...
        int oppc = _sv0t25;
        if ((oppc == 1)) {
          int _sv0t26 = lower_vm_target();
          if ((_sv0t26 == 1)) {
            Value _sv0t27;
            int _sv0t28 = sv0_vec_get(pp, opps);
            _sv0t27.tag = 3;
            _sv0t27.p0 = _sv0t28;
            return _sv0t27;
          } else {
          ...
          Value _sv0t29;
          int _sv0t30 = sv0_vec_get(pp, opps);
          _sv0t29.tag = 7;
          _sv0t29.p0 = _sv0t30;
          return _sv0t29;
        } else {
    ...
    if ((unop_tag == 5)) {
      int _sv0t31 = sv0_vec_get(et, operand_idx);
      int ot2 = _sv0t31;
      if ((ot2 == 1)) {
        int _sv0t32 = sv0_vec_get(ed1, operand_idx);
        int opps2 = _sv0t32;
        int _sv0t33 = sv0_vec_get(ed2, operand_idx);
        int oppc2 = _sv0t33;
        if ((oppc2 == 1)) {
          Value _sv0t34;
          int _sv0t35 = sv0_vec_get(pp, opps2);
          _sv0t34.tag = 7;
          _sv0t34.p0 = _sv0t35;
          return _sv0t34;
        } else {
    ...
    if ((unop_tag == 4)) {
      int _sv0t36 = sv0_vec_get(et, operand_idx);
      if ((_sv0t36 == 8)) {
        int _sv0t37 = sv0_vec_get(ed2, operand_idx);
        int _sv0t38 = sv0_vec_get(et, _sv0t37);
        if ((_sv0t38 == 21)) {
          Value _sv0t39 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, operand_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
          return _sv0t39;
        } else {
        }
      } else {
    ...
    if ((unop_tag == 5)) {
      int _sv0t40 = sv0_vec_get(et, operand_idx);
      if ((_sv0t40 == 8)) {
        int _sv0t41 = sv0_vec_get(ed2, operand_idx);
        int _sv0t42 = sv0_vec_get(et, _sv0t41);
        if ((_sv0t42 == 21)) {
          Value _sv0t43 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, operand_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
          return _sv0t43;
        } else {
        }
      } else {
    ...
    Value _sv0t44 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, operand_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value v;
    v = _sv0t44;
    int _sv0t45 = ctr_fresh(ctr);
    int t = _sv0t45;
    Expr expr;
    int _sv0t46 = sv0_box_alloc(3);
    sv0_box_store(_sv0t46, 0, v.tag);
    sv0_box_store(_sv0t46, 1, v.p0);
    sv0_box_store(_sv0t46, 2, v.p1);
    expr.tag = 3;
    expr.p0 = unop_tag;
    expr.p1 = _sv0t46;
    Instr instr;
  ...
  if ((tag == 3)) {
    ...
    int binop_tag = _sv0t50;
    int _sv0t51 = sv0_vec_get(ed2, idx);
    int lhs_idx = _sv0t51;
    int _sv0t52 = sv0_vec_get(ed3, idx);
    int rhs_idx = _sv0t52;
    Value _sv0t53 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, lhs_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value vl;
    vl = _sv0t53;
    Value _sv0t54 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, rhs_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value vr;
    vr = _sv0t54;
    int _sv0t55 = ctr_fresh(ctr);
    int t = _sv0t55;
    ...
    sv0_box_store(_sv0t56, 0, vl.tag);
    sv0_box_store(_sv0t56, 1, vl.p0);
    sv0_box_store(_sv0t56, 2, vl.p1);
    int _sv0t57 = sv0_box_alloc(3);
    sv0_box_store(_sv0t57, 0, vr.tag);
    sv0_box_store(_sv0t57, 1, vr.p0);
    sv0_box_store(_sv0t57, 2, vr.p1);
    expr.tag = 2;
    expr.p0 = binop_tag;
    expr.p1 = _sv0t56;
    expr.p2 = _sv0t57;
    Instr instr;
    int _sv0t58 = sv0_box_alloc(4);
    sv0_box_store(_sv0t58, 0, expr.tag);
    sv0_box_store(_sv0t58, 1, expr.p0);
    sv0_box_store(_sv0t58, 2, expr.p1);
    sv0_box_store(_sv0t58, 3, expr.p2);
    instr.tag = 3;
    instr.p0 = t;
    instr.p1 = _sv0t58;
    int _sv0t59 = sv0_box_alloc(5);
    sv0_box_store(_sv0t59, 0, instr.tag);
    sv0_box_store(_sv0t59, 1, instr.p0);
    sv0_box_store(_sv0t59, 2, instr.p1);
    sv0_box_store(_sv0t59, 3, instr.p2);
    sv0_box_store(_sv0t59, 4, instr.p3);
    sv0_vec_push(out_instrs, _sv0t59);
    Value _sv0t60;
    _sv0t60.tag = 3;
    _sv0t60.p0 = t;
    return _sv0t60;
  ...
  if ((tag == 4)) {
    Value _sv0t61 = lower_tag_call(et, ed1, ed2, ed3, ed4, pp, tok_tags, idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    return _sv0t61;
  } else {
  }
  if ((tag == 6)) {
    int _sv0t62 = sv0_vec_get(ed1, idx);
    int obj_idx = _sv0t62;
    int _sv0t63 = sv0_vec_get(ed2, idx);
    int field_tok = _sv0t63;
    Value _sv0t64 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, obj_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value v1;
    v1 = _sv0t64;
    Value _sv0t65;
    int _sv0t66 = sv0_box_alloc(3);
    sv0_box_store(_sv0t66, 0, v1.tag);
    sv0_box_store(_sv0t66, 1, v1.p0);
    sv0_box_store(_sv0t66, 2, v1.p1);
    _sv0t65.tag = 6;
    _sv0t65.p0 = _sv0t66;
    _sv0t65.p1 = field_tok;
    return _sv0t65;
  } else {
  }
  if ((tag == 7)) {
    int _sv0t67 = sv0_vec_get(ed1, idx);
    int obj_idx7 = _sv0t67;
    int _sv0t68 = sv0_vec_get(ed2, idx);
    int field_num = _sv0t68;
    Value _sv0t69 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, obj_idx7, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value vobj;
    vobj = _sv0t69;
    int _sv0t70 = (0 - 2000000);
    int fh = (_sv0t70 - field_num);
    Value _sv0t71;
    int _sv0t72 = sv0_box_alloc(3);
    sv0_box_store(_sv0t72, 0, vobj.tag);
    sv0_box_store(_sv0t72, 1, vobj.p0);
    sv0_box_store(_sv0t72, 2, vobj.p1);
    _sv0t71.tag = 6;
    _sv0t71.p0 = _sv0t72;
    _sv0t71.p1 = fh;
    return _sv0t71;
  ...
  if ((tag == 5)) {
    int _sv0t73 = sv0_vec_get(ed1, idx);
    int rcv_mc = _sv0t73;
    int _sv0t74 = sv0_vec_get(ed2, idx);
    int method_tok = _sv0t74;
    int _sv0t75 = sv0_vec_get(ed3, idx);
    int args_count_mc = _sv0t75;
    int _sv0t76 = sv0_vec_get(ed4, idx);
    int args_sidecar_mc = _sv0t76;
    Value _sv0t77 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, rcv_mc, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value v_rcv;
    v_rcv = _sv0t77;
    int _sv0t78 = sv0_vec_new();
    int arg_vals_mc = _sv0t78;
    int _sv0t79 = sv0_box_alloc(3);
    sv0_box_store(_sv0t79, 0, v_rcv.tag);
    sv0_box_store(_sv0t79, 1, v_rcv.p0);
    sv0_box_store(_sv0t79, 2, v_rcv.p1);
    sv0_vec_push(arg_vals_mc, _sv0t79);
    int aj_mc = 0;
    while (1) {
      ...
      int _sv0t80 = block_stmt_index(pp, 0, args_sidecar_mc, aj_mc);
      int ax_mc = _sv0t80;
      Value _sv0t81 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, ax_mc, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Value av_mc;
      av_mc = _sv0t81;
      int _sv0t82 = sv0_box_alloc(3);
      sv0_box_store(_sv0t82, 0, av_mc.tag);
      sv0_box_store(_sv0t82, 1, av_mc.p0);
      sv0_box_store(_sv0t82, 2, av_mc.p1);
      sv0_vec_push(arg_vals_mc, _sv0t82);
      aj_mc = (aj_mc + 1);
    }
    int _sv0t83 = ctr_fresh(ctr);
    int t_mc = _sv0t83;
    int bid_mc = 0;
    if ((method_tok >= 0)) {
      int _sv0t84 = sv0_vec_len(builtin_map);
      if ((method_tok < _sv0t84)) {
        int _sv0t85 = sv0_vec_get(builtin_map, method_tok);
        bid_mc = _sv0t85;
      } else {
      }
    } else {
    ...
    if ((bid_mc == 0)) {
      int rt_mc = 0;
      int _sv0t86 = lower_mcall_ret_user_ty_tok(et, ed2, idx, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
      int uty_mc = _sv0t86;
      if ((uty_mc >= 0)) {
        rt_mc = uty_mc;
      } else {
      ...
      sv0_box_store(_sv0t87, 0, call_mc.tag);
      sv0_box_store(_sv0t87, 1, call_mc.p0);
      sv0_box_store(_sv0t87, 2, call_mc.p1);
      sv0_box_store(_sv0t87, 3, call_mc.p2);
      sv0_box_store(_sv0t87, 4, call_mc.p3);
    ...
    if ((bid_mc == 14)) {
      int _sv0t89 = sv0_vec_len(arg_vals_mc);
      if ((_sv0t89 >= 2)) {
        int _sv0t90 = sv0_vec_get(arg_vals_mc, 1);
        Value _sv0t91;
        int _sv0t92 = sv0_box_load(_sv0t90, 0);
        _sv0t91.tag = _sv0t92;
        int _sv0t93 = sv0_box_load(_sv0t90, 1);
        _sv0t91.p0 = _sv0t93;
        int _sv0t94 = sv0_box_load(_sv0t90, 2);
        _sv0t91.p1 = _sv0t94;
        Value na_mv;
        na_mv.tag = (_sv0t91).tag;
        na_mv.p0 = (_sv0t91).p0;
        na_mv.p1 = (_sv0t91).p1;
        return na_mv;
      } else {
    ...
    if ((is_void_mc == 1)) {
      ...
      sv0_box_store(_sv0t96, 0, vc_mc.tag);
      sv0_box_store(_sv0t96, 1, vc_mc.p0);
      sv0_box_store(_sv0t96, 2, vc_mc.p1);
      sv0_box_store(_sv0t96, 3, vc_mc.p2);
      sv0_box_store(_sv0t96, 4, vc_mc.p3);
    ...
    sv0_box_store(_sv0t98, 0, bc_mc.tag);
    sv0_box_store(_sv0t98, 1, bc_mc.p0);
    sv0_box_store(_sv0t98, 2, bc_mc.p1);
    sv0_box_store(_sv0t98, 3, bc_mc.p2);
    sv0_box_store(_sv0t98, 4, bc_mc.p3);
  ...
  if ((tag == 8)) {
    ...
    int ix = _sv0t101;
    Value _sv0t102 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, ob, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value v_o;
    v_o = _sv0t102;
    int _sv0t103 = sv0_vec_get(et, ix);
    if ((_sv0t103 == 21)) {
      int _sv0t104 = sv0_vec_get(ed1, ix);
      int lo_c = _sv0t104;
      int _sv0t105 = sv0_vec_get(ed2, ix);
      int hi_c = _sv0t105;
      int _sv0t106 = ctr_fresh(ctr);
      int ob_t = _sv0t106;
      Instr _sv0t107;
      Expr _sv0t108 = value_to_expr(v_o);
      int _sv0t109 = sv0_box_alloc(4);
      sv0_box_store(_sv0t109, 0, _sv0t108.tag);
      sv0_box_store(_sv0t109, 1, _sv0t108.p0);
      sv0_box_store(_sv0t109, 2, _sv0t108.p1);
      sv0_box_store(_sv0t109, 3, _sv0t108.p2);
      _sv0t107.tag = 3;
      _sv0t107.p0 = ob_t;
      _sv0t107.p1 = _sv0t109;
      int _sv0t110 = sv0_box_alloc(5);
      sv0_box_store(_sv0t110, 0, _sv0t107.tag);
      sv0_box_store(_sv0t110, 1, _sv0t107.p0);
      sv0_box_store(_sv0t110, 2, _sv0t107.p1);
      sv0_box_store(_sv0t110, 3, _sv0t107.p2);
      sv0_box_store(_sv0t110, 4, _sv0t107.p3);
      sv0_vec_push(out_instrs, _sv0t110);
      if ((lo_c < 0)) {
        if ((hi_c < 0)) {
          ...
          sv0_box_store(_sv0t114, 0, _sv0t113.tag);
          sv0_box_store(_sv0t114, 1, _sv0t113.p0);
          sv0_box_store(_sv0t114, 2, _sv0t113.p1);
          sv0_vec_push(a_fs, _sv0t114);
          Instr _sv0t115;
          int _sv0t116 = (0 - 31);
          _sv0t115.tag = 11;
          _sv0t115.p0 = t_fs;
          _sv0t115.p1 = _sv0t116;
          _sv0t115.p2 = a_fs;
          _sv0t115.p3 = 0;
          int _sv0t117 = sv0_box_alloc(5);
          sv0_box_store(_sv0t117, 0, _sv0t115.tag);
          sv0_box_store(_sv0t117, 1, _sv0t115.p0);
          sv0_box_store(_sv0t117, 2, _sv0t115.p1);
          sv0_box_store(_sv0t117, 3, _sv0t115.p2);
          sv0_box_store(_sv0t117, 4, _sv0t115.p3);
          sv0_vec_push(out_instrs, _sv0t117);
          Value _sv0t118;
          _sv0t118.tag = 3;
          _sv0t118.p0 = t_fs;
      ...
      if ((lo_c < 0)) {
        ...
        _sv0t122.tag = 0;
        _sv0t122.p0 = 0;
        int _sv0t123 = sv0_box_alloc(3);
        sv0_box_store(_sv0t123, 0, _sv0t122.tag);
        sv0_box_store(_sv0t123, 1, _sv0t122.p0);
        sv0_box_store(_sv0t123, 2, _sv0t122.p1);
        _sv0t121.tag = 0;
        _sv0t121.p0 = _sv0t123;
        int _sv0t124 = sv0_box_alloc(4);
        sv0_box_store(_sv0t124, 0, _sv0t121.tag);
        sv0_box_store(_sv0t124, 1, _sv0t121.p0);
        sv0_box_store(_sv0t124, 2, _sv0t121.p1);
        sv0_box_store(_sv0t124, 3, _sv0t121.p2);
        _sv0t120.tag = 3;
        _sv0t120.p0 = lo_t;
        _sv0t120.p1 = _sv0t124;
        int _sv0t125 = sv0_box_alloc(5);
        sv0_box_store(_sv0t125, 0, _sv0t120.tag);
        sv0_box_store(_sv0t125, 1, _sv0t120.p0);
        sv0_box_store(_sv0t125, 2, _sv0t120.p1);
        sv0_box_store(_sv0t125, 3, _sv0t120.p2);
        sv0_box_store(_sv0t125, 4, _sv0t120.p3);
        sv0_vec_push(out_instrs, _sv0t125);
      } else {
        Value _sv0t126 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, lo_c, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
        Value v_lo;
        v_lo = _sv0t126;
        Instr _sv0t127;
        Expr _sv0t128 = value_to_expr(v_lo);
        int _sv0t129 = sv0_box_alloc(4);
        sv0_box_store(_sv0t129, 0, _sv0t128.tag);
        sv0_box_store(_sv0t129, 1, _sv0t128.p0);
        sv0_box_store(_sv0t129, 2, _sv0t128.p1);
        sv0_box_store(_sv0t129, 3, _sv0t128.p2);
        _sv0t127.tag = 3;
        _sv0t127.p0 = lo_t;
        _sv0t127.p1 = _sv0t129;
        int _sv0t130 = sv0_box_alloc(5);
        sv0_box_store(_sv0t130, 0, _sv0t127.tag);
        sv0_box_store(_sv0t130, 1, _sv0t127.p0);
        sv0_box_store(_sv0t130, 2, _sv0t127.p1);
        sv0_box_store(_sv0t130, 3, _sv0t127.p2);
        sv0_box_store(_sv0t130, 4, _sv0t127.p3);
        sv0_vec_push(out_instrs, _sv0t130);
      }
      int _sv0t131 = ctr_fresh(ctr);
      int hi_t = _sv0t131;
      if ((hi_c < 0)) {
        int _sv0t132 = sv0_vec_new();
        int a_ln = _sv0t132;
        Value _sv0t133;
        _sv0t133.tag = 3;
        _sv0t133.p0 = ob_t;
        ...
        sv0_vec_push(a_ln, _sv0t134);
        Instr _sv0t135;
        int _sv0t136 = (0 - 34);
        _sv0t135.tag = 11;
        _sv0t135.p0 = hi_t;
        _sv0t135.p1 = _sv0t136;
        _sv0t135.p2 = a_ln;
        _sv0t135.p3 = 0;
        int _sv0t137 = sv0_box_alloc(5);
        sv0_box_store(_sv0t137, 0, _sv0t135.tag);
        sv0_box_store(_sv0t137, 1, _sv0t135.p0);
        sv0_box_store(_sv0t137, 2, _sv0t135.p1);
        sv0_box_store(_sv0t137, 3, _sv0t135.p2);
        sv0_box_store(_sv0t137, 4, _sv0t135.p3);
        sv0_vec_push(out_instrs, _sv0t137);
      } else {
        Value _sv0t138 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, hi_c, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
        Value v_hi;
        v_hi = _sv0t138;
        Instr _sv0t139;
        Expr _sv0t140 = value_to_expr(v_hi);
        int _sv0t141 = sv0_box_alloc(4);
        sv0_box_store(_sv0t141, 0, _sv0t140.tag);
        sv0_box_store(_sv0t141, 1, _sv0t140.p0);
        sv0_box_store(_sv0t141, 2, _sv0t140.p1);
        sv0_box_store(_sv0t141, 3, _sv0t140.p2);
        _sv0t139.tag = 3;
        _sv0t139.p0 = hi_t;
        _sv0t139.p1 = _sv0t141;
        int _sv0t142 = sv0_box_alloc(5);
        sv0_box_store(_sv0t142, 0, _sv0t139.tag);
        sv0_box_store(_sv0t142, 1, _sv0t139.p0);
        sv0_box_store(_sv0t142, 2, _sv0t139.p1);
        sv0_box_store(_sv0t142, 3, _sv0t139.p2);
        sv0_box_store(_sv0t142, 4, _sv0t139.p3);
        sv0_vec_push(out_instrs, _sv0t142);
      }
      int _sv0t143 = ctr_fresh(ctr);
      int t_sl = _sv0t143;
      ...
      sv0_vec_push(a_sl, _sv0t146);
      Value _sv0t147;
      _sv0t147.tag = 3;
      _sv0t147.p0 = lo_t;
      int _sv0t148 = sv0_box_alloc(3);
      sv0_box_store(_sv0t148, 0, _sv0t147.tag);
      sv0_box_store(_sv0t148, 1, _sv0t147.p0);
      sv0_box_store(_sv0t148, 2, _sv0t147.p1);
      sv0_vec_push(a_sl, _sv0t148);
      Value _sv0t149;
      _sv0t149.tag = 3;
      _sv0t149.p0 = hi_t;
      int _sv0t150 = sv0_box_alloc(3);
      sv0_box_store(_sv0t150, 0, _sv0t149.tag);
      sv0_box_store(_sv0t150, 1, _sv0t149.p0);
      sv0_box_store(_sv0t150, 2, _sv0t149.p1);
      sv0_vec_push(a_sl, _sv0t150);
      Instr _sv0t151;
      int _sv0t152 = (0 - 30);
      _sv0t151.tag = 11;
      _sv0t151.p0 = t_sl;
      _sv0t151.p1 = _sv0t152;
      _sv0t151.p2 = a_sl;
      _sv0t151.p3 = 0;
      int _sv0t153 = sv0_box_alloc(5);
      sv0_box_store(_sv0t153, 0, _sv0t151.tag);
      sv0_box_store(_sv0t153, 1, _sv0t151.p0);
      sv0_box_store(_sv0t153, 2, _sv0t151.p1);
      sv0_box_store(_sv0t153, 3, _sv0t151.p2);
      sv0_box_store(_sv0t153, 4, _sv0t151.p3);
    ...
    Value _sv0t155 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, ix, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value v_i;
    v_i = _sv0t155;
    int _sv0t156 = ctr_fresh(ctr);
    int t_ix = _sv0t156;
    int _sv0t157 = sv0_vec_new();
    int arg_ix = _sv0t157;
    int _sv0t158 = sv0_box_alloc(3);
    sv0_box_store(_sv0t158, 0, v_o.tag);
    sv0_box_store(_sv0t158, 1, v_o.p0);
    sv0_box_store(_sv0t158, 2, v_o.p1);
    sv0_vec_push(arg_ix, _sv0t158);
    int _sv0t159 = sv0_box_alloc(3);
    sv0_box_store(_sv0t159, 0, v_i.tag);
    sv0_box_store(_sv0t159, 1, v_i.p0);
    sv0_box_store(_sv0t159, 2, v_i.p1);
    sv0_vec_push(arg_ix, _sv0t159);
    int _sv0t160 = lower_index_base_tok(et, ed1, ed2, pp, ob);
    int _sv0t161 = lower_index_bid(builtin_map, _sv0t160, 32, 41);
    int idx_get_fn_h = (0 - _sv0t161);
    Instr call_ix;
    call_ix.tag = 11;
    call_ix.p0 = t_ix;
    call_ix.p1 = idx_get_fn_h;
    call_ix.p2 = arg_ix;
    call_ix.p3 = 0;
    int _sv0t162 = sv0_box_alloc(5);
    sv0_box_store(_sv0t162, 0, call_ix.tag);
    sv0_box_store(_sv0t162, 1, call_ix.p0);
    sv0_box_store(_sv0t162, 2, call_ix.p1);
    sv0_box_store(_sv0t162, 3, call_ix.p2);
    sv0_box_store(_sv0t162, 4, call_ix.p3);
  ...
  if ((tag == 26)) {
    int _sv0t164 = sv0_vec_get(ed2, idx);
    int elem_count_ar = _sv0t164;
    int _sv0t165 = ctr_fresh(ctr);
    int t_ar = _sv0t165;
    int _sv0t166 = sv0_vec_new();
    int args_new = _sv0t166;
    int bid_new = 7;
    ...
    sv0_box_store(_sv0t167, 0, ins_new.tag);
    sv0_box_store(_sv0t167, 1, ins_new.p0);
    sv0_box_store(_sv0t167, 2, ins_new.p1);
    sv0_box_store(_sv0t167, 3, ins_new.p2);
    sv0_box_store(_sv0t167, 4, ins_new.p3);
    sv0_vec_push(out_instrs, _sv0t167);
    int push_bid_ar = 8;
    int _sv0t168 = lower_array_lit_is_f64(et, ed1, ed2, ed3, pp, tok_tags, builtin_map, idx);
    if (_sv0t168) {
      push_bid_ar = 38;
    } else {
    }
    ...
    while (1) {
      ...
      int _sv0t169 = (idx - elem_count_ar);
      int ek_ar = (_sv0t169 + ak_ar);
      Value _sv0t170 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, ek_ar, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Value v_el;
      v_el = _sv0t170;
      int _sv0t171 = sv0_vec_new();
      int args_push = _sv0t171;
      Value _sv0t172;
      _sv0t172.tag = 3;
      _sv0t172.p0 = t_ar;
      int _sv0t173 = sv0_box_alloc(3);
      sv0_box_store(_sv0t173, 0, _sv0t172.tag);
      sv0_box_store(_sv0t173, 1, _sv0t172.p0);
      sv0_box_store(_sv0t173, 2, _sv0t172.p1);
      sv0_vec_push(args_push, _sv0t173);
      int _sv0t174 = sv0_box_alloc(3);
      sv0_box_store(_sv0t174, 0, v_el.tag);
      sv0_box_store(_sv0t174, 1, v_el.p0);
      sv0_box_store(_sv0t174, 2, v_el.p1);
      sv0_vec_push(args_push, _sv0t174);
      int bid_push = push_bid_ar;
      ...
      sv0_box_store(_sv0t175, 0, ins_push.tag);
      sv0_box_store(_sv0t175, 1, ins_push.p0);
      sv0_box_store(_sv0t175, 2, ins_push.p1);
      sv0_box_store(_sv0t175, 3, ins_push.p2);
      sv0_box_store(_sv0t175, 4, ins_push.p3);
  ...
  if ((tag == 21)) {
    ...
    int hi_ix = _sv0t178;
    if ((lo_ix >= 0)) {
      int _sv0t179 = lower_for_effect(et, ed1, ed2, ed3, ed4, pp, tok_tags, lo_ix, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    } else {
    }
    if ((hi_ix >= 0)) {
      int _sv0t180 = lower_for_effect(et, ed1, ed2, ed3, ed4, pp, tok_tags, hi_ix, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    } else {
    }
    Value _sv0t181;
    _sv0t181.tag = 4;
  ...
  if ((tag == 9)) {
    int _sv0t182 = (0 - 1);
    Value _sv0t183 = lower_tag_block(et, ed1, ed2, ed3, ed4, pp, tok_tags, idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases, 0, _sv0t182);
    return _sv0t183;
  } else {
  }
  if ((tag == 34)) {
    int _sv0t184 = sv0_vec_get(ed1, idx);
    int _sv0t185 = (0 - 1);
    Value _sv0t186 = lower_tag_block(et, ed1, ed2, ed3, ed4, pp, tok_tags, _sv0t184, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases, 0, _sv0t185);
    return _sv0t186;
  } else {
  }
  if ((tag == 10)) {
    Value _sv0t187 = lower_tag_if(et, ed1, ed2, ed3, ed4, pp, tok_tags, idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    return _sv0t187;
  } else {
  }
  if ((tag == 11)) {
    Value _sv0t188 = lower_tag_match(et, ed1, ed2, ed3, ed4, pp, tok_tags, idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    return _sv0t188;
  } else {
  }
  if ((tag == 12)) {
    int _sv0t189 = sv0_vec_get(ed1, idx);
    int cond_idx2 = _sv0t189;
    int _sv0t190 = sv0_vec_get(ed2, idx);
    int body_idx2 = _sv0t190;
    int _sv0t191 = sv0_vec_new();
    int cond_is = _sv0t191;
    Expr _sv0t192 = lower_cond_expr(et, ed1, ed2, ed3, ed4, pp, tok_tags, cond_idx2, ctr, cond_is, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Expr ce2;
    ce2 = _sv0t192;
    int _sv0t193 = sv0_vec_new();
    int body_is = _sv0t193;
    int _sv0t194 = lower_for_effect(et, ed1, ed2, ed3, ed4, pp, tok_tags, body_idx2, ctr, body_is, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    int discard3 = _sv0t194;
    int _sv0t195 = sv0_vec_len(cond_is);
    if ((_sv0t195 == 0)) {
      Instr wl;
      int _sv0t196 = sv0_box_alloc(4);
      sv0_box_store(_sv0t196, 0, ce2.tag);
      sv0_box_store(_sv0t196, 1, ce2.p0);
      sv0_box_store(_sv0t196, 2, ce2.p1);
      sv0_box_store(_sv0t196, 3, ce2.p2);
      wl.tag = 7;
      wl.p0 = _sv0t196;
      wl.p1 = body_is;
      int _sv0t197 = sv0_box_alloc(5);
      sv0_box_store(_sv0t197, 0, wl.tag);
      sv0_box_store(_sv0t197, 1, wl.p0);
      sv0_box_store(_sv0t197, 2, wl.p1);
      sv0_box_store(_sv0t197, 3, wl.p2);
      sv0_box_store(_sv0t197, 4, wl.p3);
      sv0_vec_push(out_instrs, _sv0t197);
    } else {
      int _sv0t198 = ctr_fresh(ctr);
      int ct_w = _sv0t198;
      int _sv0t199 = sv0_vec_new();
      int loop_body = _sv0t199;
      int ci = 0;
      while (1) {
        int _sv0t200 = sv0_vec_len(cond_is);
        int _sv0t202 = (ci < _sv0t200);
        if ((!_sv0t202)) {
          break;
        } else {
        }
        int _sv0t201 = sv0_vec_get(cond_is, ci);
        sv0_vec_push(loop_body, _sv0t201);
        ci = (ci + 1);
      }
      Instr asg_w;
      int _sv0t203 = sv0_box_alloc(4);
      sv0_box_store(_sv0t203, 0, ce2.tag);
      sv0_box_store(_sv0t203, 1, ce2.p0);
      sv0_box_store(_sv0t203, 2, ce2.p1);
      sv0_box_store(_sv0t203, 3, ce2.p2);
      asg_w.tag = 3;
      asg_w.p0 = ct_w;
      asg_w.p1 = _sv0t203;
      int _sv0t204 = sv0_box_alloc(5);
      sv0_box_store(_sv0t204, 0, asg_w.tag);
      sv0_box_store(_sv0t204, 1, asg_w.p0);
      sv0_box_store(_sv0t204, 2, asg_w.p1);
      sv0_box_store(_sv0t204, 3, asg_w.p2);
      sv0_box_store(_sv0t204, 4, asg_w.p3);
      sv0_vec_push(loop_body, _sv0t204);
      Expr neg_w;
      Value _sv0t205;
      _sv0t205.tag = 3;
      _sv0t205.p0 = ct_w;
      int _sv0t206 = sv0_box_alloc(3);
      sv0_box_store(_sv0t206, 0, _sv0t205.tag);
      sv0_box_store(_sv0t206, 1, _sv0t205.p0);
      sv0_box_store(_sv0t206, 2, _sv0t205.p1);
      neg_w.tag = 3;
      neg_w.p0 = 1;
      neg_w.p1 = _sv0t206;
      int _sv0t207 = sv0_vec_new();
      int brk_blk = _sv0t207;
      Instr _sv0t208;
      _sv0t208.tag = 9;
      int _sv0t209 = sv0_box_alloc(5);
      sv0_box_store(_sv0t209, 0, _sv0t208.tag);
      sv0_box_store(_sv0t209, 1, _sv0t208.p0);
      sv0_box_store(_sv0t209, 2, _sv0t208.p1);
      sv0_box_store(_sv0t209, 3, _sv0t208.p2);
      sv0_box_store(_sv0t209, 4, _sv0t208.p3);
      sv0_vec_push(brk_blk, _sv0t209);
      int _sv0t210 = sv0_vec_new();
      int empty_else = _sv0t210;
      Instr if_w;
      int _sv0t211 = sv0_box_alloc(4);
      sv0_box_store(_sv0t211, 0, neg_w.tag);
      sv0_box_store(_sv0t211, 1, neg_w.p0);
      sv0_box_store(_sv0t211, 2, neg_w.p1);
      sv0_box_store(_sv0t211, 3, neg_w.p2);
      if_w.tag = 6;
      if_w.p0 = _sv0t211;
      if_w.p1 = brk_blk;
      if_w.p2 = empty_else;
      int _sv0t212 = sv0_box_alloc(5);
      sv0_box_store(_sv0t212, 0, if_w.tag);
      sv0_box_store(_sv0t212, 1, if_w.p0);
      sv0_box_store(_sv0t212, 2, if_w.p1);
      sv0_box_store(_sv0t212, 3, if_w.p2);
      sv0_box_store(_sv0t212, 4, if_w.p3);
      sv0_vec_push(loop_body, _sv0t212);
      int bi = 0;
      while (1) {
        int _sv0t213 = sv0_vec_len(body_is);
        int _sv0t215 = (bi < _sv0t213);
        if ((!_sv0t215)) {
          break;
        } else {
        }
        int _sv0t214 = sv0_vec_get(body_is, bi);
        sv0_vec_push(loop_body, _sv0t214);
        bi = (bi + 1);
      }
      Instr wl;
      Expr _sv0t216;
      Value _sv0t217;
      _sv0t217.tag = 1;
      int _sv0t218 = sv0_box_alloc(3);
      sv0_box_store(_sv0t218, 0, _sv0t217.tag);
      sv0_box_store(_sv0t218, 1, _sv0t217.p0);
      sv0_box_store(_sv0t218, 2, _sv0t217.p1);
      _sv0t216.tag = 0;
      _sv0t216.p0 = _sv0t218;
      int _sv0t219 = sv0_box_alloc(4);
      sv0_box_store(_sv0t219, 0, _sv0t216.tag);
      sv0_box_store(_sv0t219, 1, _sv0t216.p0);
      sv0_box_store(_sv0t219, 2, _sv0t216.p1);
      sv0_box_store(_sv0t219, 3, _sv0t216.p2);
      wl.tag = 7;
      wl.p0 = _sv0t219;
      wl.p1 = loop_body;
      int _sv0t220 = sv0_box_alloc(5);
      sv0_box_store(_sv0t220, 0, wl.tag);
      sv0_box_store(_sv0t220, 1, wl.p0);
      sv0_box_store(_sv0t220, 2, wl.p1);
      sv0_box_store(_sv0t220, 3, wl.p2);
      sv0_box_store(_sv0t220, 4, wl.p3);
  ...
  if ((tag == 13)) {
    Value _sv0t222 = lower_tag_for(et, ed1, ed2, ed3, ed4, pp, tok_tags, idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    return _sv0t222;
  } else {
  }
  if ((tag == 14)) {
    int _sv0t223 = sv0_vec_get(ed1, idx);
    int loop_body = _sv0t223;
    Value true_v;
    true_v.tag = 1;
    Expr true_e;
    int _sv0t224 = sv0_box_alloc(3);
    sv0_box_store(_sv0t224, 0, true_v.tag);
    sv0_box_store(_sv0t224, 1, true_v.p0);
    sv0_box_store(_sv0t224, 2, true_v.p1);
    true_e.tag = 0;
    true_e.p0 = _sv0t224;
    int _sv0t225 = sv0_vec_new();
    int loop_is = _sv0t225;
    int _sv0t226 = lower_for_effect(et, ed1, ed2, ed3, ed4, pp, tok_tags, loop_body, ctr, loop_is, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    int discard4 = _sv0t226;
    Instr ll;
    int _sv0t227 = sv0_box_alloc(4);
    sv0_box_store(_sv0t227, 0, true_e.tag);
    sv0_box_store(_sv0t227, 1, true_e.p0);
    sv0_box_store(_sv0t227, 2, true_e.p1);
    sv0_box_store(_sv0t227, 3, true_e.p2);
    ll.tag = 7;
    ll.p0 = _sv0t227;
    ll.p1 = loop_is;
    int _sv0t228 = sv0_box_alloc(5);
    sv0_box_store(_sv0t228, 0, ll.tag);
    sv0_box_store(_sv0t228, 1, ll.p0);
    sv0_box_store(_sv0t228, 2, ll.p1);
    sv0_box_store(_sv0t228, 3, ll.p2);
    sv0_box_store(_sv0t228, 4, ll.p3);
    sv0_vec_push(out_instrs, _sv0t228);
    Value _sv0t229;
    _sv0t229.tag = 4;
    return _sv0t229;
  } else {
  ...
  if ((tag == 15)) {
    ...
    int val_idx = _sv0t230;
    if ((val_idx >= 0)) {
      Value _sv0t231 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, val_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Value rv;
      rv = _sv0t231;
      Instr ret_instr;
      int _sv0t232 = sv0_box_alloc(3);
      sv0_box_store(_sv0t232, 0, rv.tag);
      sv0_box_store(_sv0t232, 1, rv.p0);
      sv0_box_store(_sv0t232, 2, rv.p1);
      ret_instr.tag = 15;
      ret_instr.p0 = _sv0t232;
      int _sv0t233 = sv0_box_alloc(5);
      sv0_box_store(_sv0t233, 0, ret_instr.tag);
      sv0_box_store(_sv0t233, 1, ret_instr.p0);
      sv0_box_store(_sv0t233, 2, ret_instr.p1);
      sv0_box_store(_sv0t233, 3, ret_instr.p2);
      sv0_box_store(_sv0t233, 4, ret_instr.p3);
      sv0_vec_push(out_instrs, _sv0t233);
      Value _sv0t234;
      _sv0t234.tag = 4;
      return _sv0t234;
    } else {
    ...
    Instr ret_void;
    ret_void.tag = 14;
    int _sv0t235 = sv0_box_alloc(5);
    sv0_box_store(_sv0t235, 0, ret_void.tag);
    sv0_box_store(_sv0t235, 1, ret_void.p0);
    sv0_box_store(_sv0t235, 2, ret_void.p1);
    sv0_box_store(_sv0t235, 3, ret_void.p2);
    sv0_box_store(_sv0t235, 4, ret_void.p3);
    sv0_vec_push(out_instrs, _sv0t235);
    Value _sv0t236;
    _sv0t236.tag = 4;
    return _sv0t236;
  } else {
  ...
  if ((tag == 16)) {
    ...
    sv0_box_store(_sv0t237, 0, brk.tag);
    sv0_box_store(_sv0t237, 1, brk.p0);
    sv0_box_store(_sv0t237, 2, brk.p1);
    sv0_box_store(_sv0t237, 3, brk.p2);
    sv0_box_store(_sv0t237, 4, brk.p3);
  ...
  if ((tag == 17)) {
    Instr cnt;
    cnt.tag = 10;
    int _sv0t239 = sv0_box_alloc(5);
    sv0_box_store(_sv0t239, 0, cnt.tag);
    sv0_box_store(_sv0t239, 1, cnt.p0);
    sv0_box_store(_sv0t239, 2, cnt.p1);
    sv0_box_store(_sv0t239, 3, cnt.p2);
    sv0_box_store(_sv0t239, 4, cnt.p3);
    sv0_vec_push(out_instrs, _sv0t239);
    Value _sv0t240;
    _sv0t240.tag = 4;
    return _sv0t240;
  } else {
  ...
  if ((tag == 18)) {
    int _sv0t241 = sv0_vec_get(ed1, idx);
    int lhs_idx = _sv0t241;
    int _sv0t242 = sv0_vec_get(ed2, idx);
    int rhs_idx = _sv0t242;
    int _sv0t243 = sv0_vec_get(et, lhs_idx);
    if ((_sv0t243 == 8)) {
      int _sv0t244 = sv0_vec_get(ed1, lhs_idx);
      int base_c = _sv0t244;
      int _sv0t245 = sv0_vec_get(ed2, lhs_idx);
      int ixe_c = _sv0t245;
      Value _sv0t246 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, base_c, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Value v_base;
      v_base = _sv0t246;
      Value _sv0t247 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, ixe_c, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Value v_ixe;
      v_ixe = _sv0t247;
      Value _sv0t248 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, rhs_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Value v_rhs_s;
      v_rhs_s = _sv0t248;
      int _sv0t249 = sv0_vec_new();
      int a_iset = _sv0t249;
      int _sv0t250 = sv0_box_alloc(3);
      sv0_box_store(_sv0t250, 0, v_base.tag);
      sv0_box_store(_sv0t250, 1, v_base.p0);
      sv0_box_store(_sv0t250, 2, v_base.p1);
      sv0_vec_push(a_iset, _sv0t250);
      int _sv0t251 = sv0_box_alloc(3);
      sv0_box_store(_sv0t251, 0, v_ixe.tag);
      sv0_box_store(_sv0t251, 1, v_ixe.p0);
      sv0_box_store(_sv0t251, 2, v_ixe.p1);
      sv0_vec_push(a_iset, _sv0t251);
      int _sv0t252 = sv0_box_alloc(3);
      sv0_box_store(_sv0t252, 0, v_rhs_s.tag);
      sv0_box_store(_sv0t252, 1, v_rhs_s.p0);
      sv0_box_store(_sv0t252, 2, v_rhs_s.p1);
      sv0_vec_push(a_iset, _sv0t252);
      Instr _sv0t253;
      int _sv0t254 = lower_index_base_tok(et, ed1, ed2, pp, base_c);
      int _sv0t255 = lower_index_bid(builtin_map, _sv0t254, 33, 42);
      int _sv0t256 = (0 - _sv0t255);
      _sv0t253.tag = 11;
      _sv0t253.p0 = 0;
      _sv0t253.p1 = _sv0t256;
      _sv0t253.p2 = a_iset;
      _sv0t253.p3 = 0;
      int _sv0t257 = sv0_box_alloc(5);
      sv0_box_store(_sv0t257, 0, _sv0t253.tag);
      sv0_box_store(_sv0t257, 1, _sv0t253.p0);
      sv0_box_store(_sv0t257, 2, _sv0t253.p1);
      sv0_box_store(_sv0t257, 3, _sv0t253.p2);
      sv0_box_store(_sv0t257, 4, _sv0t253.p3);
      sv0_vec_push(out_instrs, _sv0t257);
      Value _sv0t258;
      _sv0t258.tag = 4;
      return _sv0t258;
    } else {
    }
    Expr _sv0t259 = lower_expr_with_instrs(et, ed1, ed2, ed3, ed4, pp, tok_tags, rhs_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Expr rhs_e;
    rhs_e = _sv0t259;
    int _sv0t260 = sv0_vec_new();
    int aout = _sv0t260;
    int _sv0t261 = classify_assign_lhs_arena(et, ed1, ed2, pp, lhs_idx, aout);
    int cls = _sv0t261;
    if ((cls == 0)) {
      int _sv0t262 = sv0_vec_get(aout, 0);
      int var_tok = _sv0t262;
      Instr store;
      int _sv0t263 = sv0_box_alloc(4);
      sv0_box_store(_sv0t263, 0, rhs_e.tag);
      sv0_box_store(_sv0t263, 1, rhs_e.p0);
      sv0_box_store(_sv0t263, 2, rhs_e.p1);
      sv0_box_store(_sv0t263, 3, rhs_e.p2);
      store.tag = 4;
      store.p0 = var_tok;
      store.p1 = _sv0t263;
      int _sv0t264 = sv0_box_alloc(5);
      sv0_box_store(_sv0t264, 0, store.tag);
      sv0_box_store(_sv0t264, 1, store.p0);
      sv0_box_store(_sv0t264, 2, store.p1);
      sv0_box_store(_sv0t264, 3, store.p2);
      sv0_box_store(_sv0t264, 4, store.p3);
      sv0_vec_push(out_instrs, _sv0t264);
    } else {
    }
    if ((cls == 1)) {
      int _sv0t265 = sv0_vec_get(aout, 0);
      int obj_tok = _sv0t265;
      int _sv0t266 = sv0_vec_get(aout, 1);
      int fld_tok = _sv0t266;
      Instr sf;
      int _sv0t267 = sv0_box_alloc(4);
      sv0_box_store(_sv0t267, 0, rhs_e.tag);
      sv0_box_store(_sv0t267, 1, rhs_e.p0);
      sv0_box_store(_sv0t267, 2, rhs_e.p1);
      sv0_box_store(_sv0t267, 3, rhs_e.p2);
      sf.tag = 5;
      sf.p0 = obj_tok;
      sf.p1 = fld_tok;
      sf.p2 = _sv0t267;
      int _sv0t268 = sv0_box_alloc(5);
      sv0_box_store(_sv0t268, 0, sf.tag);
      sv0_box_store(_sv0t268, 1, sf.p0);
      sv0_box_store(_sv0t268, 2, sf.p1);
      sv0_box_store(_sv0t268, 3, sf.p2);
      sv0_box_store(_sv0t268, 4, sf.p3);
      sv0_vec_push(out_instrs, _sv0t268);
    } else {
    }
    Value _sv0t269;
    _sv0t269.tag = 4;
  ...
  if ((tag == 19)) {
    int _sv0t270 = sv0_vec_get(ed1, idx);
    int op_tag = _sv0t270;
    int _sv0t271 = sv0_vec_get(ed2, idx);
    int lhs_idx2 = _sv0t271;
    int _sv0t272 = sv0_vec_get(ed3, idx);
    int rhs_idx2 = _sv0t272;
    int _sv0t273 = sv0_vec_get(et, lhs_idx2);
    if ((_sv0t273 == 8)) {
      int _sv0t274 = sv0_vec_get(ed1, lhs_idx2);
      int base_ca = _sv0t274;
      int _sv0t275 = sv0_vec_get(ed2, lhs_idx2);
      int ixe_ca = _sv0t275;
      Value _sv0t276 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, base_ca, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Value v_base_a;
      v_base_a = _sv0t276;
      Value _sv0t277 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, ixe_ca, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Value v_ixe_a;
      v_ixe_a = _sv0t277;
      Value _sv0t278 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, rhs_idx2, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Value v_rhs_a;
      v_rhs_a = _sv0t278;
      int _sv0t279 = lower_index_base_tok(et, ed1, ed2, pp, base_ca);
      int btok_a = _sv0t279;
      int _sv0t280 = lower_index_bid(builtin_map, btok_a, 32, 41);
      int get_bid_a = _sv0t280;
      int _sv0t281 = lower_index_bid(builtin_map, btok_a, 33, 42);
      int set_bid_a = _sv0t281;
      int _sv0t282 = ctr_fresh(ctr);
      int t_cur_a = _sv0t282;
      int _sv0t283 = sv0_vec_new();
      int a_get_a = _sv0t283;
      int _sv0t284 = sv0_box_alloc(3);
      sv0_box_store(_sv0t284, 0, v_base_a.tag);
      sv0_box_store(_sv0t284, 1, v_base_a.p0);
      sv0_box_store(_sv0t284, 2, v_base_a.p1);
      sv0_vec_push(a_get_a, _sv0t284);
      int _sv0t285 = sv0_box_alloc(3);
      sv0_box_store(_sv0t285, 0, v_ixe_a.tag);
      sv0_box_store(_sv0t285, 1, v_ixe_a.p0);
      sv0_box_store(_sv0t285, 2, v_ixe_a.p1);
      sv0_vec_push(a_get_a, _sv0t285);
      Instr _sv0t286;
      int _sv0t287 = (0 - get_bid_a);
      _sv0t286.tag = 11;
      _sv0t286.p0 = t_cur_a;
      _sv0t286.p1 = _sv0t287;
      _sv0t286.p2 = a_get_a;
      _sv0t286.p3 = 0;
      int _sv0t288 = sv0_box_alloc(5);
      sv0_box_store(_sv0t288, 0, _sv0t286.tag);
      sv0_box_store(_sv0t288, 1, _sv0t286.p0);
      sv0_box_store(_sv0t288, 2, _sv0t286.p1);
      sv0_box_store(_sv0t288, 3, _sv0t286.p2);
      sv0_box_store(_sv0t288, 4, _sv0t286.p3);
      sv0_vec_push(out_instrs, _sv0t288);
      int _sv0t289 = ctr_fresh(ctr);
      int t_new_a = _sv0t289;
      Expr bin_a;
      Value _sv0t290;
      _sv0t290.tag = 3;
      _sv0t290.p0 = t_cur_a;
      int _sv0t291 = sv0_box_alloc(3);
      sv0_box_store(_sv0t291, 0, _sv0t290.tag);
      sv0_box_store(_sv0t291, 1, _sv0t290.p0);
      sv0_box_store(_sv0t291, 2, _sv0t290.p1);
      int _sv0t292 = sv0_box_alloc(3);
      sv0_box_store(_sv0t292, 0, v_rhs_a.tag);
      sv0_box_store(_sv0t292, 1, v_rhs_a.p0);
      sv0_box_store(_sv0t292, 2, v_rhs_a.p1);
      bin_a.tag = 2;
      bin_a.p0 = op_tag;
      bin_a.p1 = _sv0t291;
      bin_a.p2 = _sv0t292;
      Instr _sv0t293;
      int _sv0t294 = sv0_box_alloc(4);
      sv0_box_store(_sv0t294, 0, bin_a.tag);
      sv0_box_store(_sv0t294, 1, bin_a.p0);
      sv0_box_store(_sv0t294, 2, bin_a.p1);
      sv0_box_store(_sv0t294, 3, bin_a.p2);
      _sv0t293.tag = 3;
      _sv0t293.p0 = t_new_a;
      ...
      int _sv0t296 = sv0_vec_new();
      int a_set_a = _sv0t296;
      int _sv0t297 = sv0_box_alloc(3);
      sv0_box_store(_sv0t297, 0, v_base_a.tag);
      sv0_box_store(_sv0t297, 1, v_base_a.p0);
      sv0_box_store(_sv0t297, 2, v_base_a.p1);
      sv0_vec_push(a_set_a, _sv0t297);
      int _sv0t298 = sv0_box_alloc(3);
      sv0_box_store(_sv0t298, 0, v_ixe_a.tag);
      sv0_box_store(_sv0t298, 1, v_ixe_a.p0);
      sv0_box_store(_sv0t298, 2, v_ixe_a.p1);
      sv0_vec_push(a_set_a, _sv0t298);
      Value _sv0t299;
      _sv0t299.tag = 3;
      _sv0t299.p0 = t_new_a;
      int _sv0t300 = sv0_box_alloc(3);
      sv0_box_store(_sv0t300, 0, _sv0t299.tag);
      sv0_box_store(_sv0t300, 1, _sv0t299.p0);
      sv0_box_store(_sv0t300, 2, _sv0t299.p1);
      sv0_vec_push(a_set_a, _sv0t300);
      Instr _sv0t301;
      int _sv0t302 = (0 - set_bid_a);
      _sv0t301.tag = 11;
      _sv0t301.p0 = 0;
      _sv0t301.p1 = _sv0t302;
      _sv0t301.p2 = a_set_a;
      _sv0t301.p3 = 0;
      int _sv0t303 = sv0_box_alloc(5);
      sv0_box_store(_sv0t303, 0, _sv0t301.tag);
      sv0_box_store(_sv0t303, 1, _sv0t301.p0);
      sv0_box_store(_sv0t303, 2, _sv0t301.p1);
      sv0_box_store(_sv0t303, 3, _sv0t301.p2);
      sv0_box_store(_sv0t303, 4, _sv0t301.p3);
      sv0_vec_push(out_instrs, _sv0t303);
      Value _sv0t304;
      _sv0t304.tag = 4;
      return _sv0t304;
    } else {
    }
    int _sv0t305 = sv0_vec_new();
    int aout2 = _sv0t305;
    int _sv0t306 = classify_assign_lhs_arena(et, ed1, ed2, pp, lhs_idx2, aout2);
    int cls2 = _sv0t306;
    if ((cls2 == 0)) {
      int _sv0t307 = sv0_vec_get(aout2, 0);
      int var_tok2 = _sv0t307;
      Value cur;
      cur.tag = 3;
      cur.p0 = var_tok2;
      Value _sv0t308 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, rhs_idx2, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Value rv2;
      rv2 = _sv0t308;
      Expr be;
      int _sv0t309 = sv0_box_alloc(3);
      sv0_box_store(_sv0t309, 0, cur.tag);
      sv0_box_store(_sv0t309, 1, cur.p0);
      sv0_box_store(_sv0t309, 2, cur.p1);
      int _sv0t310 = sv0_box_alloc(3);
      sv0_box_store(_sv0t310, 0, rv2.tag);
      sv0_box_store(_sv0t310, 1, rv2.p0);
      sv0_box_store(_sv0t310, 2, rv2.p1);
      be.tag = 2;
      be.p0 = op_tag;
      be.p1 = _sv0t309;
      be.p2 = _sv0t310;
      Instr st2;
      int _sv0t311 = sv0_box_alloc(4);
      sv0_box_store(_sv0t311, 0, be.tag);
      sv0_box_store(_sv0t311, 1, be.p0);
      sv0_box_store(_sv0t311, 2, be.p1);
      sv0_box_store(_sv0t311, 3, be.p2);
      st2.tag = 4;
      st2.p0 = var_tok2;
      st2.p1 = _sv0t311;
      int _sv0t312 = sv0_box_alloc(5);
      sv0_box_store(_sv0t312, 0, st2.tag);
      sv0_box_store(_sv0t312, 1, st2.p0);
      sv0_box_store(_sv0t312, 2, st2.p1);
      sv0_box_store(_sv0t312, 3, st2.p2);
      sv0_box_store(_sv0t312, 4, st2.p3);
      sv0_vec_push(out_instrs, _sv0t312);
    } else {
    }
    if ((cls2 == 1)) {
      int _sv0t313 = sv0_vec_get(aout2, 0);
      int obj_tok2 = _sv0t313;
      int _sv0t314 = sv0_vec_get(aout2, 1);
      int fld_tok2 = _sv0t314;
      Value cur2;
      Value _sv0t315;
      _sv0t315.tag = 3;
      _sv0t315.p0 = obj_tok2;
      int _sv0t316 = sv0_box_alloc(3);
      sv0_box_store(_sv0t316, 0, _sv0t315.tag);
      sv0_box_store(_sv0t316, 1, _sv0t315.p0);
      sv0_box_store(_sv0t316, 2, _sv0t315.p1);
      cur2.tag = 6;
      cur2.p0 = _sv0t316;
      cur2.p1 = fld_tok2;
      Value _sv0t317 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, rhs_idx2, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Value rv2b;
      rv2b = _sv0t317;
      Expr be2;
      int _sv0t318 = sv0_box_alloc(3);
      sv0_box_store(_sv0t318, 0, cur2.tag);
      sv0_box_store(_sv0t318, 1, cur2.p0);
      sv0_box_store(_sv0t318, 2, cur2.p1);
      int _sv0t319 = sv0_box_alloc(3);
      sv0_box_store(_sv0t319, 0, rv2b.tag);
      sv0_box_store(_sv0t319, 1, rv2b.p0);
      sv0_box_store(_sv0t319, 2, rv2b.p1);
      be2.tag = 2;
      be2.p0 = op_tag;
      be2.p1 = _sv0t318;
      be2.p2 = _sv0t319;
      Instr sf2;
      int _sv0t320 = sv0_box_alloc(4);
      sv0_box_store(_sv0t320, 0, be2.tag);
      sv0_box_store(_sv0t320, 1, be2.p0);
      sv0_box_store(_sv0t320, 2, be2.p1);
      sv0_box_store(_sv0t320, 3, be2.p2);
      sf2.tag = 5;
      sf2.p0 = obj_tok2;
      sf2.p1 = fld_tok2;
      sf2.p2 = _sv0t320;
      int _sv0t321 = sv0_box_alloc(5);
      sv0_box_store(_sv0t321, 0, sf2.tag);
      sv0_box_store(_sv0t321, 1, sf2.p0);
      sv0_box_store(_sv0t321, 2, sf2.p1);
      sv0_box_store(_sv0t321, 3, sf2.p2);
      sv0_box_store(_sv0t321, 4, sf2.p3);
  ...
  if ((tag == 20)) {
    int _sv0t323 = sv0_vec_get(ed1, idx);
    int cast_src = _sv0t323;
    Value _sv0t324 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, cast_src, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value cv;
    cv = _sv0t324;
    int _sv0t325 = ctr_fresh(ctr);
    int ct2 = _sv0t325;
    int _sv0t326 = sv0_vec_get(ed4, idx);
    int ptr_cast_kind = _sv0t326;
    if ((ptr_cast_kind != 0)) {
      int _sv0t327 = sv0_vec_get(ed3, idx);
      int inner_tok = _sv0t327;
      Instr _sv0t328;
      _sv0t328.tag = 16;
      _sv0t328.p0 = inner_tok;
      _sv0t328.p1 = ptr_cast_kind;
      _sv0t328.p2 = ct2;
      int _sv0t329 = sv0_box_alloc(5);
      sv0_box_store(_sv0t329, 0, _sv0t328.tag);
      sv0_box_store(_sv0t329, 1, _sv0t328.p0);
      sv0_box_store(_sv0t329, 2, _sv0t328.p1);
      sv0_box_store(_sv0t329, 3, _sv0t328.p2);
      sv0_box_store(_sv0t329, 4, _sv0t328.p3);
      sv0_vec_push(out_instrs, _sv0t329);
      Expr store_e;
      int _sv0t330 = sv0_box_alloc(3);
      sv0_box_store(_sv0t330, 0, cv.tag);
      sv0_box_store(_sv0t330, 1, cv.p0);
      sv0_box_store(_sv0t330, 2, cv.p1);
      store_e.tag = 3;
      store_e.p0 = 99;
      store_e.p1 = _sv0t330;
      Instr _sv0t331;
      int _sv0t332 = sv0_box_alloc(4);
      sv0_box_store(_sv0t332, 0, store_e.tag);
      sv0_box_store(_sv0t332, 1, store_e.p0);
      sv0_box_store(_sv0t332, 2, store_e.p1);
      sv0_box_store(_sv0t332, 3, store_e.p2);
      _sv0t331.tag = 4;
      _sv0t331.p0 = ct2;
      _sv0t331.p1 = _sv0t332;
      int _sv0t333 = sv0_box_alloc(5);
      sv0_box_store(_sv0t333, 0, _sv0t331.tag);
      sv0_box_store(_sv0t333, 1, _sv0t331.p0);
      sv0_box_store(_sv0t333, 2, _sv0t331.p1);
      sv0_box_store(_sv0t333, 3, _sv0t331.p2);
      sv0_box_store(_sv0t333, 4, _sv0t331.p3);
    ...
    Expr cast_e;
    int _sv0t335 = sv0_box_alloc(3);
    sv0_box_store(_sv0t335, 0, cv.tag);
    sv0_box_store(_sv0t335, 1, cv.p0);
    sv0_box_store(_sv0t335, 2, cv.p1);
    cast_e.tag = 3;
    cast_e.p0 = 99;
    cast_e.p1 = _sv0t335;
    Instr cast_i;
    int _sv0t336 = sv0_box_alloc(4);
    sv0_box_store(_sv0t336, 0, cast_e.tag);
    sv0_box_store(_sv0t336, 1, cast_e.p0);
    sv0_box_store(_sv0t336, 2, cast_e.p1);
    sv0_box_store(_sv0t336, 3, cast_e.p2);
    cast_i.tag = 3;
    cast_i.p0 = ct2;
    cast_i.p1 = _sv0t336;
    int _sv0t337 = sv0_box_alloc(5);
    sv0_box_store(_sv0t337, 0, cast_i.tag);
    sv0_box_store(_sv0t337, 1, cast_i.p0);
    sv0_box_store(_sv0t337, 2, cast_i.p1);
    sv0_box_store(_sv0t337, 3, cast_i.p2);
    sv0_box_store(_sv0t337, 4, cast_i.p3);
    sv0_vec_push(out_instrs, _sv0t337);
    Value _sv0t338;
    _sv0t338.tag = 3;
    _sv0t338.p0 = ct2;
    return _sv0t338;
  ...
  if ((tag == 22)) {
    Value _sv0t339 = lower_tag_try(et, ed1, ed2, ed3, ed4, pp, tok_tags, idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    return _sv0t339;
  } else {
  }
  if ((tag == 24)) {
    Value _sv0t340 = lower_tag_struct(et, ed1, ed2, ed3, ed4, pp, tok_tags, idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    return _sv0t340;
  } else {
  }
  if ((tag == 25)) {
    int _sv0t341 = sv0_vec_get(ed2, idx);
    int elem_count = _sv0t341;
    if ((elem_count == 1)) {
      int _sv0t342 = sv0_vec_get(ed1, idx);
      int first_idx = _sv0t342;
      Value _sv0t343 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, first_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      return _sv0t343;
    } else {
    }
  } else {
  ...
  if ((tag == 23)) {
    int _sv0t344 = sv0_vec_get(ed1, idx);
    int assert_expr = _sv0t344;
    int _sv0t345 = lower_for_effect(et, ed1, ed2, ed3, ed4, pp, tok_tags, assert_expr, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value _sv0t346;
    _sv0t346.tag = 4;
    return _sv0t346;
  } else {
...
static Expr lower_expr_with_instrs(int et, int ed1, int ed2, int ed3, int ed4, int pp, int tok_tags, int idx, int ctr, int out_instrs, int enum_names, int enum_tag_offsets, int enum_tag_counts, int enum_tags_flat, int fn_ctx, int builtin_map, int lit_sf_names, const char* source, int starts, int ends, int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int enum_vnames_flat, int enum_max_payloads, int item_fn_row, int match_env_inherit, int import_aliases) {
  ...
  if ((etag == 10)) {
    ...
    int else_idx_wi = _sv0t1;
    int _sv0t2 = (else_idx_wi >= 0);
    int _sv0t3 = lower_vm_target();
    int _sv0t4 = (_sv0t3 == 1);
    if ((_sv0t2 && _sv0t4)) {
      int _sv0t5 = sv0_vec_get(ed1, idx);
      int cond_idx_wi = _sv0t5;
      int _sv0t6 = sv0_vec_get(ed2, idx);
      int then_idx_wi = _sv0t6;
      int _sv0t7 = ctr_fresh(ctr);
      int t_wi = _sv0t7;
      int _sv0t8 = ctr_fresh(ctr);
      int u_wi = _sv0t8;
      Expr _sv0t9 = lower_cond_expr(et, ed1, ed2, ed3, ed4, pp, tok_tags, cond_idx_wi, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Expr cond_e_wi;
      cond_e_wi = _sv0t9;
      Instr _sv0t10;
      _sv0t10.tag = 1;
      ...
      int then_wi = _sv0t14;
      int _sv0t15 = lower_into_var_instrs(et, ed1, ed2, ed3, ed4, pp, tok_tags, then_idx_wi, u_wi, ctr, then_wi, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      ...
      sv0_box_store(_sv0t24, 4, _sv0t21.p3);
      sv0_vec_push(out_instrs, _sv0t24);
      Value _sv0t25;
      _sv0t25.tag = 3;
      _sv0t25.p0 = t_wi;
      Expr _sv0t26 = value_to_expr(_sv0t25);
      return _sv0t26;
    } else {
    }
  } else {
  ...
  if ((etag == 3)) {
    ...
    int rhs_idx = _sv0t29;
    Value _sv0t30 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, lhs_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value vl;
    vl = _sv0t30;
    Value _sv0t31 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, rhs_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Value vr;
    vr = _sv0t31;
    Expr _sv0t32;
    ...
    _sv0t32.tag = 2;
    _sv0t32.p0 = binop_tag;
    _sv0t32.p1 = _sv0t33;
    _sv0t32.p2 = _sv0t34;
    return _sv0t32;
  } else {
  }
  Value _sv0t35 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
  Value v;
  v = _sv0t35;
  Expr _sv0t36 = value_to_expr(v);
  return _sv0t36;
}
...
static int lower_for_effect(int et, int ed1, int ed2, int ed3, int ed4, int pp, int tok_tags, int idx, int ctr, int out_instrs, int enum_names, int enum_tag_offsets, int enum_tag_counts, int enum_tags_flat, int fn_ctx, int builtin_map, int lit_sf_names, const char* source, int starts, int ends, int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int enum_vnames_flat, int enum_max_payloads, int item_fn_row, int match_env_inherit, int import_aliases) {
  ...
  if ((etag == 34)) {
    ...
    int _sv0t4 = (0 - 1);
    Value _sv0t5 = lower_tag_block(et, ed1, ed2, ed3, ed4, pp, tok_tags, _sv0t3, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases, 1, _sv0t4);
    return 0;
  } else {
  }
  if ((etag == 10)) {
    int _sv0t6 = sv0_vec_get(ed1, idx);
    int cond_idx = _sv0t6;
    int _sv0t7 = sv0_vec_get(ed2, idx);
    int then_idx = _sv0t7;
    int _sv0t8 = sv0_vec_get(ed3, idx);
    int else_idx = _sv0t8;
    Expr _sv0t9 = lower_cond_expr(et, ed1, ed2, ed3, ed4, pp, tok_tags, cond_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    Expr cond_e;
    cond_e = _sv0t9;
    int _sv0t10 = sv0_vec_new();
    int then_is = _sv0t10;
    int _sv0t11 = lower_for_effect(et, ed1, ed2, ed3, ed4, pp, tok_tags, then_idx, ctr, then_is, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    int rt = _sv0t11;
    int _sv0t12 = sv0_vec_new();
    int else_is = _sv0t12;
    if ((else_idx >= 0)) {
      int _sv0t13 = lower_for_effect(et, ed1, ed2, ed3, ed4, pp, tok_tags, else_idx, ctr, else_is, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      int re = _sv0t13;
    } else {
    }
    Instr _sv0t14;
  ...
  Value _sv0t17 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
  Value discard5;
  discard5 = _sv0t17;
  return 0;
}
...
static int lower_into_var_instrs(int et, int ed1, int ed2, int ed3, int ed4, int pp, int tok_tags, int body_idx, int dest, int ctr, int out_instrs, int enum_names, int enum_tag_offsets, int enum_tag_counts, int enum_tags_flat, int fn_ctx, int builtin_map, int lit_sf_names, const char* source, int starts, int ends, int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int enum_vnames_flat, int enum_max_payloads, int item_fn_row, int match_env_inherit, int import_aliases) {
  ...
  if ((btag == 34)) {
    int _sv0t2 = sv0_vec_get(ed1, body_idx);
    Value _sv0t3 = lower_tag_block(et, ed1, ed2, ed3, ed4, pp, tok_tags, _sv0t2, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases, 0, dest);
    return 0;
  } else {
  }
  if ((btag == 10)) {
    int _sv0t4 = sv0_vec_get(ed3, body_idx);
    int else_idx_iv = _sv0t4;
    if ((else_idx_iv < 0)) {
      int _sv0t5 = sv0_vec_get(ed1, body_idx);
      int cond_idx_iv = _sv0t5;
      int _sv0t6 = sv0_vec_get(ed2, body_idx);
      int then_idx_iv = _sv0t6;
      Expr _sv0t7 = lower_cond_expr(et, ed1, ed2, ed3, ed4, pp, tok_tags, cond_idx_iv, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Expr cond_e_iv;
      cond_e_iv = _sv0t7;
      int _sv0t8 = sv0_vec_new();
      int then_is_iv = _sv0t8;
      int _sv0t9 = lower_into_var_instrs(et, ed1, ed2, ed3, ed4, pp, tok_tags, then_idx_iv, dest, ctr, then_is_iv, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      int _sv0t10 = sv0_vec_new();
      int else_is_iv = _sv0t10;
      Instr _sv0t11;
      Expr _sv0t12;
      Value _sv0t13;
      _sv0t13.tag = 4;
      ...
      _sv0t12.p0 = _sv0t14;
      int _sv0t15 = sv0_box_alloc(4);
      sv0_box_store(_sv0t15, 0, _sv0t12.tag);
      sv0_box_store(_sv0t15, 1, _sv0t12.p0);
      sv0_box_store(_sv0t15, 2, _sv0t12.p1);
      sv0_box_store(_sv0t15, 3, _sv0t12.p2);
      _sv0t11.tag = 4;
      _sv0t11.p0 = dest;
      _sv0t11.p1 = _sv0t15;
      int _sv0t16 = sv0_box_alloc(5);
      sv0_box_store(_sv0t16, 0, _sv0t11.tag);
      sv0_box_store(_sv0t16, 1, _sv0t11.p0);
      sv0_box_store(_sv0t16, 2, _sv0t11.p1);
      ...
      int _sv0t20 = sv0_vec_get(ed1, body_idx);
      int cond_idx_we = _sv0t20;
      int _sv0t21 = sv0_vec_get(ed2, body_idx);
      int then_idx_we = _sv0t21;
      int _sv0t22 = ctr_fresh(ctr);
      int u_we = _sv0t22;
      Expr _sv0t23 = lower_cond_expr(et, ed1, ed2, ed3, ed4, pp, tok_tags, cond_idx_we, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      Expr cond_e_we;
      cond_e_we = _sv0t23;
      Instr _sv0t24;
      _sv0t24.tag = 1;
      ...
      int _sv0t26 = sv0_vec_new();
      int then_we = _sv0t26;
      int _sv0t27 = lower_into_var_instrs(et, ed1, ed2, ed3, ed4, pp, tok_tags, then_idx_we, u_we, ctr, then_we, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      ...
      sv0_box_store(_sv0t35, 3, _sv0t34.p2);
      _sv0t33.tag = 4;
      _sv0t33.p0 = dest;
      _sv0t33.p1 = _sv0t35;
      int _sv0t36 = sv0_box_alloc(5);
      ...
      sv0_box_store(_sv0t36, 4, _sv0t33.p3);
      sv0_vec_push(out_instrs, _sv0t36);
      return 0;
    }
  } else {
  }
  Expr _sv0t37 = lower_expr_with_instrs(et, ed1, ed2, ed3, ed4, pp, tok_tags, body_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
  Expr ee;
  ee = _sv0t37;
  Instr si;
  int _sv0t38 = sv0_box_alloc(4);
  sv0_box_store(_sv0t38, 0, ee.tag);
  sv0_box_store(_sv0t38, 1, ee.p0);
  sv0_box_store(_sv0t38, 2, ee.p1);
  sv0_box_store(_sv0t38, 3, ee.p2);
  si.tag = 4;
  si.p0 = dest;
  si.p1 = _sv0t38;
  int _sv0t39 = sv0_box_alloc(5);
  sv0_box_store(_sv0t39, 0, si.tag);
  sv0_box_store(_sv0t39, 1, si.p0);
  sv0_box_store(_sv0t39, 2, si.p1);
  sv0_box_store(_sv0t39, 3, si.p2);
  sv0_box_store(_sv0t39, 4, si.p3);
...
static int lower_match_arms(int et, int ed1, int ed2, int ed3, int ed4, int pp, int tok_tags, int arms_first, int arm_count, int arms_sidecar, int out_var, int scrut_var, int scrut_cty, int ctr, int out_instrs, int enum_names, int enum_tag_offsets, int enum_tag_counts, int enum_tags_flat, int fn_ctx, int builtin_map, int lit_sf_names, int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int enum_vnames_flat, int enum_max_payloads, int item_fn_row, int match_env, const char* source, int starts, int ends, int import_aliases) {
  ...
  while ((ai >= 0)) {
      ...
      if ((pxt4 == 30)) {
        ...
        if ((k4 == 4)) {
          ...
          if ((pp_c4 >= 2)) {
            ...
            cexp = _sv0t141;
            int meta_idxp = (paux4 - 1);
            int _sv0t144 = sv0_vec_get(et, meta_idxp);
            if ((_sv0t144 == 32)) {
              int _sv0t145 = sv0_vec_get(ed1, meta_idxp);
              int sfcp = _sv0t145;
              int _sv0t146 = sv0_vec_get(ed2, meta_idxp);
              int first_embp = _sv0t146;
              if ((sfcp > 0)) {
                int fjp = 0;
                while ((fjp < sfcp)) {
                  int eidxp = (first_embp + fjp);
                  int _sv0t147 = sv0_vec_get(et, eidxp);
                  if ((_sv0t147 == 31)) {
                    int _sv0t148 = sv0_vec_get(ed4, eidxp);
                    if ((_sv0t148 == 1)) {
                      int _sv0t149 = sv0_vec_get(ed1, eidxp);
                      int bind_tokp = _sv0t149;
                      int _sv0t150 = sv0_vec_get(ed3, eidxp);
                      int fld_metap = _sv0t150;
                      int fnam_tokp = bind_tokp;
                      if ((fld_metap != 0)) {
                        fnam_tokp = fld_metap;
                      } else {
                      }
                      if ((bind_tokp >= 0)) {
                        ...
                        Value _sv0t151;
                        _sv0t151.tag = 3;
                        _sv0t151.p0 = scrut_var;
                        int _sv0t152 = sv0_box_alloc(3);
                        sv0_box_store(_sv0t152, 0, _sv0t151.tag);
                        sv0_box_store(_sv0t152, 1, _sv0t151.p0);
                        sv0_box_store(_sv0t152, 2, _sv0t151.p1);
                        rhsvp.tag = 6;
                        rhsvp.p0 = _sv0t152;
                        rhsvp.p1 = fnam_tokp;
                        Instr asgxp;
                        Expr _sv0t153;
                        int _sv0t154 = sv0_box_alloc(3);
                        sv0_box_store(_sv0t154, 0, rhsvp.tag);
                        sv0_box_store(_sv0t154, 1, rhsvp.p0);
                        sv0_box_store(_sv0t154, 2, rhsvp.p1);
                        _sv0t153.tag = 0;
                        _sv0t153.p0 = _sv0t154;
                        int _sv0t155 = sv0_box_alloc(4);
                        sv0_box_store(_sv0t155, 0, _sv0t153.tag);
                        sv0_box_store(_sv0t155, 1, _sv0t153.p0);
                        sv0_box_store(_sv0t155, 2, _sv0t153.p1);
                        sv0_box_store(_sv0t155, 3, _sv0t153.p2);
                        asgxp.tag = 3;
                        asgxp.p0 = bind_tokp;
                        asgxp.p1 = _sv0t155;
                        int _sv0t156 = sv0_box_alloc(5);
                        sv0_box_store(_sv0t156, 0, asgxp.tag);
                        sv0_box_store(_sv0t156, 1, asgxp.p0);
                        sv0_box_store(_sv0t156, 2, asgxp.p1);
                        sv0_box_store(_sv0t156, 3, asgxp.p2);
                        sv0_box_store(_sv0t156, 4, asgxp.p3);
                        sv0_vec_push(bind_instrs, _sv0t156);
                      } else {
                      }
                    } else {
                    }
          ...
          _sv0t158.tag = 2;
          int _sv0t159 = sv0_box_alloc(3);
          sv0_box_store(_sv0t159, 0, _sv0t158.tag);
          sv0_box_store(_sv0t159, 1, _sv0t158.p0);
          sv0_box_store(_sv0t159, 2, _sv0t158.p1);
          _sv0t157.tag = 0;
          _sv0t157.p0 = _sv0t159;
          cexp = _sv0t157;
        }
      } else {
        Expr _sv0t160;
        ...
        _sv0t160.tag = 0;
        _sv0t160.p0 = _sv0t162;
        cexp = _sv0t160;
      }
    } else {
    }
    int _sv0t163 = lower_match_env_clone(match_env);
    int arm_menv = _sv0t163;
    if ((pat_tag == 1)) {
      int _sv0t164 = lower_match_env_append_scrut_pair(arm_menv, pat_d1, scrut_cty);
      int dap = _sv0t164;
    } else {
    }
    if ((pat_tag == 5)) {
      int _sv0t165 = sv0_vec_get(ed2, arm_idx);
      int paux5s = _sv0t165;
      int _sv0t166 = sv0_vec_get(et, paux5s);
      int pxt5s = _sv0t166;
      if ((pxt5s == 30)) {
        int _sv0t167 = sv0_vec_get(ed4, paux5s);
        int k5s = _sv0t167;
        if ((k5s == 5)) {
          int _sv0t168 = sv0_vec_get(ed2, paux5s);
          int pp_c5s = _sv0t168;
          if ((pp_c5s >= 2)) {
            int _sv0t169 = sv0_vec_get(ed1, paux5s);
            int pp_s5s = _sv0t169;
            int _sv0t170 = sv0_vec_get(pp, pp_s5s);
            int en_hs = _sv0t170;
            int _sv0t171 = (pp_s5s + 1);
            int _sv0t172 = sv0_vec_get(pp, _sv0t171);
            int vn_hs = _sv0t172;
            int _sv0t173 = enum_tag_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, en_hs, vn_hs, source, starts, ends);
            int kvs = _sv0t173;
            if ((kvs >= 0)) {
              int _sv0t174 = sv0_vec_get(ed3, paux5s);
              int argc5s = _sv0t174;
              if ((argc5s > 0)) {
                int meta_idx5s = (paux5s - 1);
                int _sv0t175 = sv0_vec_get(et, meta_idx5s);
                if ((_sv0t175 == 33)) {
                  int _sv0t176 = sv0_vec_get(ed1, meta_idx5s);
                  int ac5s = _sv0t176;
                  int _sv0t177 = sv0_vec_get(ed2, meta_idx5s);
                  int first_e5s = _sv0t177;
                  int ejs = 0;
                  while ((ejs < ac5s)) {
                    int eidx5s = (first_e5s + ejs);
                    int _sv0t178 = sv0_vec_get(et, eidx5s);
                    int etg5s = _sv0t178;
                    if ((etg5s == 31)) {
                      int _sv0t179 = sv0_vec_get(ed4, eidx5s);
                      int ipt5s = _sv0t179;
                      if ((ipt5s == 1)) {
                        int _sv0t180 = sv0_vec_get(ed1, eidx5s);
                        int bind_tok5s = _sv0t180;
                        int _sv0t181 = sv0_vec_get(ed2, eidx5s);
                        int pi5s = _sv0t181;
                        if ((bind_tok5s >= 0)) {
                          if ((pi5s >= 0)) {
                            int _sv0t182 = lower_match_env_append_scrut_pair(arm_menv, bind_tok5s, scrut_cty);
                            int das = _sv0t182;
                          } else {
                          }
                        } else {
    ...
    if ((pat_tag == 4)) {
      int _sv0t183 = sv0_vec_get(ed2, arm_idx);
      int paux4s = _sv0t183;
      int _sv0t184 = sv0_vec_get(et, paux4s);
      int pxt4s = _sv0t184;
      if ((pxt4s == 30)) {
        int _sv0t185 = sv0_vec_get(ed4, paux4s);
        int k4s = _sv0t185;
        if ((k4s == 4)) {
          int _sv0t186 = sv0_vec_get(ed2, paux4s);
          int pp_c4s = _sv0t186;
          if ((pp_c4s >= 2)) {
            int _sv0t187 = sv0_vec_get(ed1, paux4s);
            int pp_s4s = _sv0t187;
            int _sv0t188 = sv0_vec_get(pp, pp_s4s);
            int en_h4s = _sv0t188;
            int _sv0t189 = (pp_s4s + 1);
            int _sv0t190 = sv0_vec_get(pp, _sv0t189);
            int vn_h4s = _sv0t190;
            int _sv0t191 = enum_tag_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, en_h4s, vn_h4s, source, starts, ends);
            int kv4s = _sv0t191;
            if ((kv4s >= 0)) {
              int meta_idxs = (paux4s - 1);
              int _sv0t192 = sv0_vec_get(et, meta_idxs);
              if ((_sv0t192 == 32)) {
                int _sv0t193 = sv0_vec_get(ed1, meta_idxs);
                int sfc4s = _sv0t193;
                int _sv0t194 = sv0_vec_get(ed2, meta_idxs);
                int first_embs = _sv0t194;
                if ((sfc4s > 0)) {
                  int fjs = 0;
                  while ((fjs < sfc4s)) {
                    int eidxs = (first_embs + fjs);
                    int _sv0t195 = sv0_vec_get(et, eidxs);
                    int etgs = _sv0t195;
                    if ((etgs == 31)) {
                      int _sv0t196 = sv0_vec_get(ed4, eidxs);
                      int ipts = _sv0t196;
                      if ((ipts == 1)) {
                        int _sv0t197 = sv0_vec_get(ed1, eidxs);
                        int bind_toks = _sv0t197;
                        int _sv0t198 = sv0_vec_get(ed3, eidxs);
                        int fld_metas = _sv0t198;
                        int fnam_toks = bind_toks;
                        if ((fld_metas != 0)) {
                          fnam_toks = fld_metas;
                        } else {
                        }
                        int _sv0t199 = lower_pat_struct_field_payload_index(tok_tags, source, starts, ends, vn_h4s, sfc4s, fnam_toks);
                        int pis = _sv0t199;
                        if ((bind_toks >= 0)) {
                          if ((pis >= 0)) {
                            int _sv0t200 = lower_match_env_append_scrut_pair(arm_menv, bind_toks, scrut_cty);
                            int dss = _sv0t200;
                          } else {
                          }
                        } else {
...
static Value lower_tag_match(int et, int ed1, int ed2, int ed3, int ed4, int pp, int tok_tags, int idx, int ctr, int out_instrs, int enum_names, int enum_tag_offsets, int enum_tag_counts, int enum_tags_flat, int fn_ctx, int builtin_map, int lit_sf_names, const char* source, int starts, int ends, int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int enum_vnames_flat, int enum_max_payloads, int item_fn_row, int match_env_inherit, int import_aliases) {
  ...
  if ((_sv0t19 == 0)) {
    ...
    zero_init.tag = 4;
    zero_init.p0 = out;
    zero_init.p1 = _sv0t23;
    int _sv0t24 = sv0_box_alloc(5);
    sv0_box_store(_sv0t24, 0, zero_init.tag);
    sv0_box_store(_sv0t24, 1, zero_init.p0);
    sv0_box_store(_sv0t24, 2, zero_init.p1);
    sv0_box_store(_sv0t24, 3, zero_init.p2);
    sv0_box_store(_sv0t24, 4, zero_init.p3);
    sv0_vec_push(out_instrs, _sv0t24);
  } else {
  }
  int _sv0t25 = lower_match_arms(et, ed1, ed2, ed3, ed4, pp, tok_tags, arms_first, arm_count, arms_sidecar, out, scrut_var, sct, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env, source, starts, ends, import_aliases);
  int discard_ma = _sv0t25;
  Value _sv0t26;
  _sv0t26.tag = 3;
  _sv0t26.p0 = out;
  return _sv0t26;
...
static int lower_stmt(int et, int ed1, int ed2, int ed3, int ed4, int pp, int tok_tags, int s_idx, int ctr, int out_instrs, int enum_names, int enum_tag_offsets, int enum_tag_counts, int enum_tags_flat, int fn_ctx, int builtin_map, int lit_sf_names, const char* source, int starts, int ends, int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int enum_vnames_flat, int enum_max_payloads, int item_fn_row, int match_env_inherit, int import_aliases, int str_locals) {
  ...
  if ((s_tag == 27)) {
    ...
    if ((init_idx >= 0)) {
      ...
      if ((_sv0t3 == 24)) {
        ...
        if ((sppc == 2)) {
          if ((fc_ls > 0)) {
            int _sv0t7 = sv0_vec_get(pp, spps_ls);
            int en_tok_ls = _sv0t7;
            int _sv0t8 = (spps_ls + 1);
            int _sv0t9 = sv0_vec_get(pp, _sv0t8);
            int vn_tok_ls = _sv0t9;
            int _sv0t10;
            int _sv0t11;
            if ((en_tok_ls >= 0)) {
              int _sv0t13 = sv0_vec_len(starts);
              int _sv0t12;
              if ((en_tok_ls < _sv0t13)) {
                int _sv0t14;
                if ((vn_tok_ls >= 0)) {
                  int _sv0t15 = sv0_vec_len(starts);
                  _sv0t14 = (vn_tok_ls < _sv0t15);
                } else {
                  _sv0t14 = 0;
                }
                _sv0t12 = _sv0t14;
            ...
            _sv0t10 = _sv0t11;
            int toks_ok_ls2 = _sv0t10;
            int _sv0t16;
            int _sv0t17;
            if (toks_ok_ls2) {
              int _sv0t18 = enum_tag_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, en_tok_ls, vn_tok_ls, source, starts, ends);
              _sv0t17 = _sv0t18;
            } else {
              _sv0t17 = (0 - 2);
            }
            ...
            if ((k_ls2 >= 0)) {
              ...
              _sv0t21.p2 = _sv0t26;
              int _sv0t27 = sv0_box_alloc(5);
              sv0_box_store(_sv0t27, 0, _sv0t21.tag);
              sv0_box_store(_sv0t27, 1, _sv0t21.p0);
              sv0_box_store(_sv0t27, 2, _sv0t21.p1);
              sv0_box_store(_sv0t27, 3, _sv0t21.p2);
              sv0_box_store(_sv0t27, 4, _sv0t21.p3);
              sv0_vec_push(out_instrs, _sv0t27);
              int _sv0t28 = enum_variant_def_tok_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, en_tok_ls, vn_tok_ls, source, starts, ends);
              int def_tok_ls = _sv0t28;
              int _sv0t29 = sv0_vec_get(ed4, init_idx);
              int sf0_ls2 = _sv0t29;
              int fi_ls2 = 0;
              while ((fi_ls2 < fc_ls)) {
                int _sv0t30 = (fi_ls2 * 2);
                int _sv0t31 = (sf0_ls2 + _sv0t30);
                int _sv0t32 = (_sv0t31 + 1);
                int _sv0t33 = sv0_vec_get(lit_sf_names, _sv0t32);
                int ch_ls2 = _sv0t33;
                Value _sv0t34 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, ch_ls2, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
                Value fv_ls2;
                fv_ls2 = _sv0t34;
                int _sv0t35 = (fi_ls2 * 2);
                int _sv0t36 = (sf0_ls2 + _sv0t35);
                int _sv0t37 = sv0_vec_get(lit_sf_names, _sv0t36);
                int fnm_ls2 = _sv0t37;
                int pi_ls2 = (0 - 1);
                if ((def_tok_ls >= 0)) {
                  int _sv0t38 = lower_pat_struct_field_payload_index(tok_tags, source, starts, ends, def_tok_ls, fc_ls, fnm_ls2);
                  pi_ls2 = _sv0t38;
                } else {
                }
                if ((pi_ls2 < 0)) {
                ...
                int _sv0t39 = (0 - 10);
                int pay_ls2 = (_sv0t39 - pi_ls2);
                Instr _sv0t40;
                Expr _sv0t41 = value_to_expr(fv_ls2);
                ...
                sv0_box_store(_sv0t43, 4, _sv0t40.p3);
          ...
          if ((sppc >= 1)) {
            if ((fc_ls > 0)) {
              int _sv0t44 = (spps_ls + sppc);
              int _sv0t45 = (_sv0t44 - 1);
              int _sv0t46 = sv0_vec_get(pp, _sv0t45);
              int sty_tok = _sv0t46;
              Instr decl_n;
              decl_n.tag = 2;
              decl_n.p0 = sty_tok;
              ...
              sv0_box_store(_sv0t47, 0, decl_n.tag);
              sv0_box_store(_sv0t47, 1, decl_n.p0);
              sv0_box_store(_sv0t47, 2, decl_n.p1);
              sv0_box_store(_sv0t47, 3, decl_n.p2);
              sv0_box_store(_sv0t47, 4, decl_n.p3);
              sv0_vec_push(out_instrs, _sv0t47);
              let_declared = 1;
              int _sv0t48 = sv0_vec_get(ed4, init_idx);
              int sf0_ls = _sv0t48;
              int fi_ls = 0;
              while ((fi_ls < fc_ls)) {
                int _sv0t49 = (fi_ls * 2);
                int _sv0t50 = (sf0_ls + _sv0t49);
                int _sv0t51 = (_sv0t50 + 1);
                int _sv0t52 = sv0_vec_get(lit_sf_names, _sv0t51);
                int ch_ls = _sv0t52;
                Value _sv0t53 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, ch_ls, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
                Value fv_ls;
                fv_ls = _sv0t53;
                int _sv0t54 = (fi_ls * 2);
                int _sv0t55 = (sf0_ls + _sv0t54);
                int _sv0t56 = sv0_vec_get(lit_sf_names, _sv0t55);
                int fnm_ls = _sv0t56;
                Instr _sv0t57;
                Expr _sv0t58 = value_to_expr(fv_ls);
                int _sv0t59 = sv0_box_alloc(4);
                ...
                sv0_box_store(_sv0t60, 0, _sv0t57.tag);
                sv0_box_store(_sv0t60, 1, _sv0t57.p0);
                sv0_box_store(_sv0t60, 2, _sv0t57.p1);
                sv0_box_store(_sv0t60, 3, _sv0t57.p2);
                sv0_box_store(_sv0t60, 4, _sv0t57.p3);
                sv0_vec_push(out_instrs, _sv0t60);
                fi_ls = (fi_ls + 1);
              }
              init_handled = 1;
            } else {
      ...
      int _sv0t61 = sv0_vec_get(et, init_idx);
      if ((_sv0t61 == 1)) {
        int _sv0t62 = sv0_vec_get(ed2, init_idx);
        if ((_sv0t62 == 2)) {
          int _sv0t63 = sv0_vec_get(ed1, init_idx);
          int _sv0t64 = sv0_vec_get(pp, _sv0t63);
          int ety_tok = _sv0t64;
          int _sv0t65 = sv0_vec_get(ed1, init_idx);
          int _sv0t66 = (_sv0t65 + 1);
          int _sv0t67 = sv0_vec_get(pp, _sv0t66);
          int vn_tok_n = _sv0t67;
          Instr _sv0t68;
          _sv0t68.tag = 2;
          _sv0t68.p0 = ety_tok;
          _sv0t68.p1 = name_tok;
          ...
          sv0_vec_push(out_instrs, _sv0t69);
          int _sv0t70 = enum_tag_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, ety_tok, vn_tok_n, source, starts, ends);
          int k_n = _sv0t70;
          Expr tag_en;
          Value _sv0t71;
          _sv0t71.tag = 0;
          ...
          Instr _sv0t73;
          int _sv0t74 = (0 - 1);
          int _sv0t75 = sv0_box_alloc(4);
          sv0_box_store(_sv0t75, 0, tag_en.tag);
          sv0_box_store(_sv0t75, 1, tag_en.p0);
          sv0_box_store(_sv0t75, 2, tag_en.p1);
          sv0_box_store(_sv0t75, 3, tag_en.p2);
          _sv0t73.tag = 5;
          _sv0t73.p0 = name_tok;
          _sv0t73.p1 = _sv0t74;
          _sv0t73.p2 = _sv0t75;
          ...
          sv0_box_store(_sv0t76, 4, _sv0t73.p3);
          sv0_vec_push(out_instrs, _sv0t76);
          int _sv0t77 = lower_vm_target();
          if ((_sv0t77 == 0)) {
            int _sv0t78 = enum_max_payload_lookup_str(enum_names, enum_max_payloads, ety_tok, source, starts, ends);
            int maxp_n = _sv0t78;
            int pzn = 0;
            while ((pzn < maxp_n)) {
              Instr pzn_e;
              int _sv0t79 = (0 - 10);
              int _sv0t80 = (_sv0t79 - pzn);
              Expr _sv0t81;
              Value _sv0t82;
              _sv0t82.tag = 0;
              _sv0t82.p0 = 0;
              ...
              _sv0t81.tag = 0;
              _sv0t81.p0 = _sv0t83;
              int _sv0t84 = sv0_box_alloc(4);
              sv0_box_store(_sv0t84, 0, _sv0t81.tag);
              sv0_box_store(_sv0t84, 1, _sv0t81.p0);
              sv0_box_store(_sv0t84, 2, _sv0t81.p1);
              sv0_box_store(_sv0t84, 3, _sv0t81.p2);
              pzn_e.tag = 5;
              pzn_e.p0 = name_tok;
              pzn_e.p1 = _sv0t80;
              pzn_e.p2 = _sv0t84;
              int _sv0t85 = sv0_box_alloc(5);
              sv0_box_store(_sv0t85, 0, pzn_e.tag);
              sv0_box_store(_sv0t85, 1, pzn_e.p0);
              sv0_box_store(_sv0t85, 2, pzn_e.p1);
              sv0_box_store(_sv0t85, 3, pzn_e.p2);
              sv0_box_store(_sv0t85, 4, pzn_e.p3);
      ...
      int _sv0t86 = sv0_vec_get(et, init_idx);
      if ((_sv0t86 == 4)) {
        int _sv0t87 = sv0_vec_get(ed1, init_idx);
        int callee_ci = _sv0t87;
        int _sv0t88 = sv0_vec_get(et, callee_ci);
        if ((_sv0t88 == 1)) {
          int _sv0t89 = sv0_vec_get(ed2, callee_ci);
          if ((_sv0t89 == 2)) {
            int _sv0t90 = sv0_vec_get(ed1, callee_ci);
            int _sv0t91 = sv0_vec_get(pp, _sv0t90);
            int en_tok_c = _sv0t91;
            int _sv0t92 = sv0_vec_get(ed1, callee_ci);
            int _sv0t93 = (_sv0t92 + 1);
            int _sv0t94 = sv0_vec_get(pp, _sv0t93);
            int vn_tok_c = _sv0t94;
            Instr _sv0t95;
            _sv0t95.tag = 2;
            _sv0t95.p0 = en_tok_c;
            _sv0t95.p1 = name_tok;
            ...
            sv0_vec_push(out_instrs, _sv0t96);
            int _sv0t97 = enum_tag_lookup_str(enum_names, enum_tags_flat, enum_tag_offsets, enum_tag_counts, en_tok_c, vn_tok_c, source, starts, ends);
            int k_c = _sv0t97;
            int _sv0t98 = sv0_vec_get(ed2, init_idx);
            int args_first_c = _sv0t98;
            int _sv0t99 = sv0_vec_get(ed3, init_idx);
            int args_count_c = _sv0t99;
            int _sv0t100 = sv0_vec_get(ed4, init_idx);
            int args_sidecar_c = _sv0t100;
            int _sv0t101 = sv0_vec_new();
            int arg_vals_c = _sv0t101;
            int ai_c = 0;
            while ((ai_c < args_count_c)) {
              int _sv0t102 = block_stmt_index(pp, args_first_c, args_sidecar_c, ai_c);
              int a_idx_c = _sv0t102;
              Value _sv0t103 = lower_expr_to_value(et, ed1, ed2, ed3, ed4, pp, tok_tags, a_idx_c, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
              Value av_c;
              av_c = _sv0t103;
              int _sv0t104 = sv0_box_alloc(3);
              sv0_box_store(_sv0t104, 0, av_c.tag);
              sv0_box_store(_sv0t104, 1, av_c.p0);
              sv0_box_store(_sv0t104, 2, av_c.p1);
              sv0_vec_push(arg_vals_c, _sv0t104);
              ai_c = (ai_c + 1);
            }
            Expr tag_ec;
            Value _sv0t105;
            ...
            int _sv0t108 = (0 - 1);
            int _sv0t109 = sv0_box_alloc(4);
            sv0_box_store(_sv0t109, 0, tag_ec.tag);
            sv0_box_store(_sv0t109, 1, tag_ec.p0);
            sv0_box_store(_sv0t109, 2, tag_ec.p1);
            sv0_box_store(_sv0t109, 3, tag_ec.p2);
            _sv0t107.tag = 5;
            _sv0t107.p0 = name_tok;
            _sv0t107.p1 = _sv0t108;
            _sv0t107.p2 = _sv0t109;
            int _sv0t110 = sv0_box_alloc(5);
            sv0_box_store(_sv0t110, 0, _sv0t107.tag);
            sv0_box_store(_sv0t110, 1, _sv0t107.p0);
            sv0_box_store(_sv0t110, 2, _sv0t107.p1);
            ...
            while ((pi_c < args_count_c)) {
              int _sv0t111 = sv0_vec_get(arg_vals_c, pi_c);
              Value _sv0t112;
              int _sv0t113 = sv0_box_load(_sv0t111, 0);
              _sv0t112.tag = _sv0t113;
              int _sv0t114 = sv0_box_load(_sv0t111, 1);
              _sv0t112.p0 = _sv0t114;
              int _sv0t115 = sv0_box_load(_sv0t111, 2);
              _sv0t112.p1 = _sv0t115;
              Value pv_c;
              pv_c.tag = (_sv0t112).tag;
              pv_c.p0 = (_sv0t112).p0;
              pv_c.p1 = (_sv0t112).p1;
              int _sv0t116 = (0 - 10);
              int fs_c = (_sv0t116 - pi_c);
              Instr _sv0t117;
              Expr _sv0t118 = value_to_expr(pv_c);
              int _sv0t119 = sv0_box_alloc(4);
              sv0_box_store(_sv0t119, 0, _sv0t118.tag);
              sv0_box_store(_sv0t119, 1, _sv0t118.p0);
              sv0_box_store(_sv0t119, 2, _sv0t118.p1);
              sv0_box_store(_sv0t119, 3, _sv0t118.p2);
              _sv0t117.tag = 5;
              _sv0t117.p0 = name_tok;
              _sv0t117.p1 = fs_c;
              _sv0t117.p2 = _sv0t119;
              int _sv0t120 = sv0_box_alloc(5);
              sv0_box_store(_sv0t120, 0, _sv0t117.tag);
              sv0_box_store(_sv0t120, 1, _sv0t117.p0);
              sv0_box_store(_sv0t120, 2, _sv0t117.p1);
              sv0_box_store(_sv0t120, 3, _sv0t117.p2);
              sv0_box_store(_sv0t120, 4, _sv0t117.p3);
      ...
      int _sv0t121 = lower_expr_yields_string(et, ed1, ed2, ed3, pp, init_idx, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
      if (_sv0t121) {
        int _sv0t122 = lower_string_cty();
        deferred_ty = _sv0t122;
        has_deferred_decl = 1;
        let_declared = 1;
        str_let = 1;
      } else {
      ...
      if ((let_declared == 0)) {
        int _sv0t123 = lower_call_ret_is_string(et, ed1, ed2, pp, init_idx, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
        if (_sv0t123) {
          Instr decl_su;
          int _sv0t124 = lower_string_cty();
          decl_su.tag = 2;
          ...
          decl_su.p1 = name_tok;
          int _sv0t125 = sv0_box_alloc(5);
          sv0_box_store(_sv0t125, 0, decl_su.tag);
          sv0_box_store(_sv0t125, 1, decl_su.p0);
          sv0_box_store(_sv0t125, 2, decl_su.p1);
          sv0_box_store(_sv0t125, 3, decl_su.p2);
          sv0_box_store(_sv0t125, 4, decl_su.p3);
          sv0_vec_push(out_instrs, _sv0t125);
          let_declared = 1;
          str_let = 1;
        } else {
      ...
      if ((let_declared == 0)) {
        int _sv0t126 = lower_init_ident_is_string_param(et, ed1, ed2, pp, init_idx, item_tags, item_names, item_d3, tok_tags, source, starts, ends, item_fn_row);
        if (_sv0t126) {
          Instr decl_sp;
          int _sv0t127 = lower_string_cty();
          decl_sp.tag = 2;
          decl_sp.p0 = _sv0t127;
          decl_sp.p1 = name_tok;
          int _sv0t128 = sv0_box_alloc(5);
          sv0_box_store(_sv0t128, 0, decl_sp.tag);
          sv0_box_store(_sv0t128, 1, decl_sp.p0);
          sv0_box_store(_sv0t128, 2, decl_sp.p1);
          sv0_box_store(_sv0t128, 3, decl_sp.p2);
          sv0_box_store(_sv0t128, 4, decl_sp.p3);
          sv0_vec_push(out_instrs, _sv0t128);
          let_declared = 1;
          str_let = 1;
        } else {
        }
      ...
      if ((let_declared == 0)) {
        int _sv0t129 = lower_ident_in_str_locals(et, ed1, ed2, pp, init_idx, str_locals, source, starts, ends);
        if (_sv0t129) {
          Instr decl_sl;
          int _sv0t130 = lower_string_cty();
          decl_sl.tag = 2;
          decl_sl.p0 = _sv0t130;
          decl_sl.p1 = name_tok;
          int _sv0t131 = sv0_box_alloc(5);
          sv0_box_store(_sv0t131, 0, decl_sl.tag);
          sv0_box_store(_sv0t131, 1, decl_sl.p0);
          sv0_box_store(_sv0t131, 2, decl_sl.p1);
          sv0_box_store(_sv0t131, 3, decl_sl.p2);
          sv0_box_store(_sv0t131, 4, decl_sl.p3);
      ...
      if ((let_declared == 0)) {
        int _sv0t132 = lower_call_ret_user_ty_tok(et, ed1, ed2, pp, init_idx, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
        int uty_ls = _sv0t132;
        if ((uty_ls >= 0)) {
          deferred_ty = uty_ls;
          has_deferred_decl = 1;
      ...
      if ((let_declared == 0)) {
        int _sv0t133 = lower_mcall_ret_user_ty_tok(et, ed2, init_idx, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
        int mty_ls = _sv0t133;
        if ((mty_ls >= 0)) {
          deferred_ty = mty_ls;
          has_deferred_decl = 1;
      ...
      if ((let_declared == 0)) {
        int _sv0t134 = sv0_vec_get(ed2, s_idx);
        int _sv0t135 = lower_typed_box_deref_let(et, ed1, ed2, ed3, ed4, pp, tok_tags, init_idx, name_tok, _sv0t134, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
        int bd = _sv0t135;
        if ((bd == 1)) {
          let_declared = 1;
          init_handled = 1;
      ...
      if ((let_declared == 0)) {
        int _sv0t136 = sv0_vec_get(ed2, s_idx);
        int ann_tok_lsp = _sv0t136;
        if ((ann_tok_lsp >= 0)) {
          int _sv0t137 = lower_tok_in_bounds(ann_tok_lsp, starts, ends);
          if (_sv0t137) {
            int _sv0t138 = sv0_vec_get(tok_tags, ann_tok_lsp);
            if ((_sv0t138 == 24)) {
              int _sv0t139 = (ann_tok_lsp + 1);
              int _sv0t140 = sv0_vec_get(tok_tags, _sv0t139);
              int kind_tag_ls = _sv0t140;
              if ((kind_tag_ls == 58)) {
                deferred_ptr_kind = 1;
              } else {
              }
      ...
      if ((let_declared == 0)) {
        if ((deferred_ptr_kind == 0)) {
          int _sv0t141 = sv0_vec_get(ed2, s_idx);
          int ann_tok_ls = _sv0t141;
          if ((ann_tok_ls >= 0)) {
            int _sv0t142 = lower_tok_in_bounds(ann_tok_ls, starts, ends);
            if (_sv0t142) {
              const char* _sv0t143 = handle_to_str(ann_tok_ls, source, starts, ends);
              const char* ann_name_ls;
              ann_name_ls = _sv0t143;
              int _sv0t144 = is_wide_primitive_ty_name(ann_name_ls);
              if (_sv0t144) {
    ...
    if ((init_idx >= 0)) {
      if ((init_handled == 0)) {
        Expr _sv0t145 = lower_expr_with_instrs(et, ed1, ed2, ed3, ed4, pp, tok_tags, init_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
        Expr e;
        e = _sv0t145;
        if ((deferred_ptr_kind != 0)) {
          Instr _sv0t146;
          _sv0t146.tag = 16;
          _sv0t146.p0 = deferred_ptr_inner;
          _sv0t146.p1 = deferred_ptr_kind;
          _sv0t146.p2 = name_tok;
          int _sv0t147 = sv0_box_alloc(5);
          ...
          if ((has_deferred_decl == 1)) {
            Instr _sv0t148;
            _sv0t148.tag = 2;
            _sv0t148.p0 = deferred_ty;
            _sv0t148.p1 = name_tok;
            int _sv0t149 = sv0_box_alloc(5);
            sv0_box_store(_sv0t149, 0, _sv0t148.tag);
            sv0_box_store(_sv0t149, 1, _sv0t148.p0);
            sv0_box_store(_sv0t149, 2, _sv0t148.p1);
        ...
        sv0_box_store(_sv0t152, 0, e.tag);
        sv0_box_store(_sv0t152, 1, e.p0);
        sv0_box_store(_sv0t152, 2, e.p1);
        sv0_box_store(_sv0t152, 3, e.p2);
        si.tag = 4;
        si.p0 = name_tok;
        si.p1 = _sv0t152;
        int _sv0t153 = sv0_box_alloc(5);
        sv0_box_store(_sv0t153, 0, si.tag);
        sv0_box_store(_sv0t153, 1, si.p0);
        sv0_box_store(_sv0t153, 2, si.p1);
        sv0_box_store(_sv0t153, 3, si.p2);
        sv0_box_store(_sv0t153, 4, si.p3);
        sv0_vec_push(out_instrs, _sv0t153);
      } else {
      }
    } else {
      if ((deferred_ptr_kind != 0)) {
        Instr _sv0t154;
        _sv0t154.tag = 16;
        _sv0t154.p0 = deferred_ptr_inner;
        _sv0t154.p1 = deferred_ptr_kind;
        _sv0t154.p2 = name_tok;
        int _sv0t155 = sv0_box_alloc(5);
        sv0_box_store(_sv0t155, 0, _sv0t154.tag);
        sv0_box_store(_sv0t155, 1, _sv0t154.p0);
        sv0_box_store(_sv0t155, 2, _sv0t154.p1);
        sv0_box_store(_sv0t155, 3, _sv0t154.p2);
        sv0_box_store(_sv0t155, 4, _sv0t154.p3);
        sv0_vec_push(out_instrs, _sv0t155);
      } else {
        if ((let_declared == 0)) {
          Instr _sv0t156;
          _sv0t156.tag = 1;
          _sv0t156.p0 = name_tok;
          int _sv0t157 = sv0_box_alloc(5);
          sv0_box_store(_sv0t157, 0, _sv0t156.tag);
          sv0_box_store(_sv0t157, 1, _sv0t156.p0);
          sv0_box_store(_sv0t157, 2, _sv0t156.p1);
          sv0_box_store(_sv0t157, 3, _sv0t156.p2);
          sv0_box_store(_sv0t157, 4, _sv0t156.p3);
  ...
  if ((s_tag == 28)) {
    int _sv0t158 = sv0_vec_get(ed1, s_idx);
    int expr_idx = _sv0t158;
    int _sv0t159 = sv0_vec_get(et, expr_idx);
    if ((_sv0t159 == 15)) {
      int _sv0t160 = sv0_vec_get(ed1, expr_idx);
      int ret_val = _sv0t160;
      int _sv0t161 = lower_return(et, ed1, ed2, ed3, ed4, pp, tok_tags, ret_val, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
      return _sv0t161;
    } else {
    }
    int _sv0t162 = lower_for_effect(et, ed1, ed2, ed3, ed4, pp, tok_tags, expr_idx, ctr, out_instrs, enum_names, enum_tag_offsets, enum_tag_counts, enum_tags_flat, fn_ctx, builtin_map, lit_sf_names, source, starts, ends, item_tags, item_names, item_d2, item_d3, item_d4, item_field_counts, enum_vnames_flat, enum_max_payloads, item_fn_row, match_env_inherit, import_aliases);
    int rc = _sv0t162;
    return 0;
  } else {
  }
...
static int lower_vecf_tag_at(int tok_tags, int i) {
  ...
  int _sv0t3 = sv0_vec_get(tok_tags, i);
  return _sv0t3;
}
...
static int lower_vecf_text_is(const char* source, int starts, int ends, int t, const char* want) {
  ...
  int _sv0t2 = sv0_vec_get(starts, t);
  int s0 = _sv0t2;
  int _sv0t3 = sv0_vec_get(ends, t);
  int e0 = _sv0t3;
  int _sv0t4 = (e0 - s0);
  int _sv0t5 = sv0_string_len(want);
  if ((_sv0t4 != _sv0t5)) {
    return 0;
  } else {
  }
  int _sv0t6 = (e0 - s0);
  const char* _sv0t7 = sv0_string_substr(source, s0, _sv0t6);
  int _sv0t8 = sv0_string_eq(_sv0t7, want);
  return _sv0t8;
}
...
static int lower_vecf_same_text(const char* source, int starts, int ends, int a, int b) {
  ...
  int _sv0t1 = sv0_vec_len(starts);
  if ((b >= _sv0t1)) {
    return 0;
  } else {
  }
  int _sv0t2 = sv0_vec_len(ends);
  if ((a >= _sv0t2)) {
    return 0;
  } else {
  }
  int _sv0t3 = sv0_vec_len(ends);
  if ((b >= _sv0t3)) {
    return 0;
  } else {
  }
  int _sv0t4 = sv0_vec_get(ends, a);
  int _sv0t5 = sv0_vec_get(starts, a);
  int la = (_sv0t4 - _sv0t5);
  int _sv0t6 = sv0_vec_get(ends, b);
  int _sv0t7 = sv0_vec_get(starts, b);
  int lb = (_sv0t6 - _sv0t7);
  if ((la != lb)) {
    return 0;
  } else {
  }
  ...
  int _sv0t8 = sv0_vec_get(starts, a);
  const char* _sv0t9 = sv0_string_substr(source, _sv0t8, la);
  int _sv0t10 = sv0_vec_get(starts, b);
  const char* _sv0t11 = sv0_string_substr(source, _sv0t10, lb);
  int _sv0t12 = sv0_string_eq(_sv0t9, _sv0t11);
  return _sv0t12;
}

static int lower_vecf_type_kind(int tok_tags, const char* source, int starts, int ends, int p) {
  int q = p;
  int _sv0t0 = lower_vecf_tag_at(tok_tags, q);
  if ((_sv0t0 == 27)) {
    q = (q + 1);
  } else {
  }
  int _sv0t1 = lower_vecf_tag_at(tok_tags, q);
  if ((_sv0t1 == 77)) {
    q = (q + 1);
  } else {
  }
  int _sv0t2 = lower_vecf_tag_at(tok_tags, q);
  if ((_sv0t2 == 10)) {
    int _sv0t3 = (q + 1);
    int _sv0t4 = lower_vecf_tag_at(tok_tags, _sv0t3);
    if ((_sv0t4 == 5)) {
      int _sv0t5 = (q + 1);
      int _sv0t6 = lower_vecf_text_is(source, starts, ends, _sv0t5, "f64");
      if (_sv0t6) {
        int _sv0t7 = (q + 2);
        int _sv0t8 = lower_vecf_tag_at(tok_tags, _sv0t7);
        int c2 = _sv0t8;
        if ((c2 == 11)) {
          return 1;
        } else {
  ...
  int _sv0t9 = lower_vecf_tag_at(tok_tags, q);
  if ((_sv0t9 != 5)) {
    return 2;
  } else {
  }
  int _sv0t10 = lower_vecf_text_is(source, starts, ends, q, "Vec");
  if ((_sv0t10 != 1)) {
    return 2;
  } else {
  }
  int _sv0t11 = (q + 1);
  int _sv0t12 = lower_vecf_tag_at(tok_tags, _sv0t11);
  if ((_sv0t12 != 38)) {
    return 2;
  } else {
  }
  int _sv0t13 = (q + 2);
  int _sv0t14 = lower_vecf_tag_at(tok_tags, _sv0t13);
  if ((_sv0t14 == 5)) {
    int _sv0t15 = (q + 2);
    int _sv0t16 = lower_vecf_text_is(source, starts, ends, _sv0t15, "f64");
    if (_sv0t16) {
      int _sv0t17 = (q + 3);
      int _sv0t18 = lower_vecf_tag_at(tok_tags, _sv0t17);
      int close = _sv0t18;
      if ((close == 39)) {
        return 1;
      } else {
...
static int lower_vecf_lookup(int names, int kinds, const char* source, int starts, int ends, int name_tok) {
  ...
  while ((i < n)) {
    ...
    int _sv0t2 = lower_vecf_same_text(source, starts, ends, _sv0t1, name_tok);
    if (_sv0t2) {
      int _sv0t3 = sv0_vec_get(kinds, i);
      int k = _sv0t3;
      if ((k == 1)) {
        n1 = (n1 + 1);
      } else {
    ...
    if ((depth == 1)) {
      ...
      if (at_start) {
        if ((t == 5)) {
          at_start = 0;
          int _sv0t4 = (p + 1);
          int _sv0t5 = lower_vecf_tag_at(tok_tags, _sv0t4);
          if ((_sv0t5 == 14)) {
            sv0_vec_push(fnames, p);
            int _sv0t6 = (p + 2);
            int _sv0t7 = lower_vecf_type_kind(tok_tags, source, starts, ends, _sv0t6);
            sv0_vec_push(fkinds, _sv0t7);
          } else {
          }
        } else {
        }
...
static int lower_vecf_params_open(int tok_tags, int i) {
  int p = (i + 2);
  int _sv0t0 = lower_vecf_tag_at(tok_tags, p);
  if ((_sv0t0 == 38)) {
    int d = 0;
    while (1) {
      int _sv0t1 = sv0_vec_len(tok_tags);
  ...
  int _sv0t4 = lower_vecf_tag_at(tok_tags, p);
  if ((_sv0t4 != 6)) {
    int _sv0t5 = (0 - 1);
    return _sv0t5;
  } else {
...
static int lower_vecf_fn_end(int tok_tags, int i) {
  int _sv0t0 = sv0_vec_len(tok_tags);
  int n = _sv0t0;
  int _sv0t1 = lower_vecf_params_open(tok_tags, i);
  int open = _sv0t1;
  if ((open < 0)) {
    int _sv0t2 = (i + 1);
    return _sv0t2;
  ...
  int _sv0t3 = lower_vecf_close_paren(tok_tags, open);
  int close = _sv0t3;
  if ((close < 0)) {
    return n;
  } else {
  ...
  while ((p < n)) {
    ...
    if ((t == 6)) {
      int _sv0t8 = lower_vecf_close_paren(tok_tags, p);
      int c2 = _sv0t8;
      if ((c2 < 0)) {
        return n;
      } else {
...
static int lower_vecf_collect_params(int tok_tags, const char* source, int starts, int ends, int i, int names, int kinds) {
  int _sv0t0 = lower_vecf_params_open(tok_tags, i);
  int open = _sv0t0;
  if ((open < 0)) {
    return 0;
  } else {
  }
  int _sv0t1 = lower_vecf_close_paren(tok_tags, open);
  int close = _sv0t1;
  if ((close < 0)) {
    return 0;
  } else {
    ...
    if ((pd == 1)) {
      ...
      if (at_start) {
        if ((t == 5)) {
          ...
          int _sv0t4 = lower_vecf_tag_at(tok_tags, _sv0t3);
          if ((_sv0t4 == 14)) {
            sv0_vec_push(names, p);
            int _sv0t5 = (p + 2);
            int _sv0t6 = lower_vecf_type_kind(tok_tags, source, starts, ends, _sv0t5);
            sv0_vec_push(kinds, _sv0t6);
          } else {
          }
        } else {
        }
...
static int lower_vecf_collect_lets(int tok_tags, const char* source, int starts, int ends, int lo, int hi, int rnames, int rkinds, int names, int kinds, int map) {
  int i = lo;
  while ((i < hi)) {
    int _sv0t0 = sv0_vec_get(tok_tags, i);
    if ((_sv0t0 == 71)) {
      int p = (i + 1);
      int _sv0t1 = lower_vecf_tag_at(tok_tags, p);
      if ((_sv0t1 == 77)) {
        p = (p + 1);
      } else {
      }
      int _sv0t2 = lower_vecf_tag_at(tok_tags, p);
      if ((_sv0t2 == 5)) {
        int kind = 2;
        int _sv0t3 = (p + 1);
        int _sv0t4 = lower_vecf_tag_at(tok_tags, _sv0t3);
        ...
        if ((nt == 35)) {
          ...
          if ((_sv0t8 == 10)) {
            int _sv0t9 = (p + 3);
            int _sv0t10 = lower_vecf_tag_at(tok_tags, _sv0t9);
            if ((_sv0t10 == 1)) {
              kind = 1;
            } else {
            }
          ...
          if ((_sv0t12 == 5)) {
            ...
            if ((_sv0t14 == 6)) {
              int _sv0t15 = (p + 2);
              int _sv0t16 = lower_vecf_lookup(rnames, rkinds, source, starts, ends, _sv0t15);
              if ((_sv0t16 == 1)) {
                kind = 1;
              } else {
              }
              int _sv0t17 = (p + 2);
              int _sv0t18 = lower_vecf_text_is(source, starts, ends, _sv0t17, "vec_new");
              if (_sv0t18) {
                kind = 4;
              } else {
              }
        ...
        if ((kind == 1)) {
          ...
          if ((nt == 14)) {
            ...
            int bd = 0;
            while ((q < hi)) {
              int _sv0t19 = sv0_vec_get(tok_tags, q);
              int u = _sv0t19;
              if ((u == 10)) {
                bd = (bd + 1);
              } else {
          ...
          if ((eq >= 0)) {
            int _sv0t20 = (eq + 1);
            int _sv0t21 = lower_vecf_tag_at(tok_tags, _sv0t20);
            if ((_sv0t21 == 10)) {
              int _sv0t22 = (eq + 2);
              int _sv0t23 = lower_vecf_tag_at(tok_tags, _sv0t22);
              if ((_sv0t23 != 11)) {
                int _sv0t24 = (eq + 1);
                sv0_vec_set(map, _sv0t24, 4);
              } else {
              }
            } else {
            }
...
static int lower_vecf_fn_param_kind(int pfn_names, int pfn_off, int pfn_cnt, int pkinds, const char* source, int starts, int ends, int name_tok, int argi) {
  int i = 0;
  int _sv0t0 = sv0_vec_len(pfn_names);
  int n = _sv0t0;
  while ((i < n)) {
    int _sv0t1 = sv0_vec_get(pfn_names, i);
    int _sv0t2 = lower_vecf_same_text(source, starts, ends, _sv0t1, name_tok);
    if (_sv0t2) {
      if ((argi < 0)) {
        int _sv0t3 = (0 - 1);
      ...
      int _sv0t4 = sv0_vec_get(pfn_cnt, i);
      if ((argi >= _sv0t4)) {
        int _sv0t5 = (0 - 1);
        return _sv0t5;
      } else {
      }
      int _sv0t6 = sv0_vec_get(pfn_off, i);
      int _sv0t7 = (_sv0t6 + argi);
      int _sv0t8 = sv0_vec_get(pkinds, _sv0t7);
      return _sv0t8;
    } else {
    }
    i = (i + 1);
...
static int lower_vecf_word_type(const char* source, int starts, int ends, int t) {
  int _sv0t0 = lower_vecf_text_is(source, starts, ends, t, "i32");
  if (_sv0t0) {
    return 1;
  } else {
  }
  int _sv0t1 = lower_vecf_text_is(source, starts, ends, t, "i64");
  if (_sv0t1) {
    return 1;
  } else {
  }
  int _sv0t2 = lower_vecf_text_is(source, starts, ends, t, "u64");
  if (_sv0t2) {
    return 1;
  } else {
  }
  int _sv0t3 = lower_vecf_text_is(source, starts, ends, t, "u32");
  if (_sv0t3) {
    return 1;
  } else {
  }
  int _sv0t4 = lower_vecf_text_is(source, starts, ends, t, "u8");
  if (_sv0t4) {
    return 1;
  } else {
  }
  int _sv0t5 = lower_vecf_text_is(source, starts, ends, t, "i8");
  if (_sv0t5) {
    return 1;
  } else {
  }
  int _sv0t6 = lower_vecf_text_is(source, starts, ends, t, "u16");
  if (_sv0t6) {
    return 1;
  } else {
  }
  int _sv0t7 = lower_vecf_text_is(source, starts, ends, t, "i16");
  if (_sv0t7) {
    return 1;
  } else {
  }
  int _sv0t8 = lower_vecf_text_is(source, starts, ends, t, "isize");
  if (_sv0t8) {
    return 1;
  } else {
  }
  int _sv0t9 = lower_vecf_text_is(source, starts, ends, t, "usize");
  if (_sv0t9) {
    return 1;
  } else {
  }
  int _sv0t10 = lower_vecf_text_is(source, starts, ends, t, "bool");
  if (_sv0t10) {
    return 1;
  } else {
  }
...
static int lower_vecf_evidence_at(int tok_tags, const char* source, int starts, int ends, int lo, int p, int fnames, int fkinds, int pfn_names, int pfn_off, int pfn_cnt, int pkinds, int retkind, int out) {
  int _sv0t0 = (p - 1);
  int _sv0t1 = lower_vecf_tag_at(tok_tags, _sv0t0);
  int prev = _sv0t1;
  int _sv0t2 = (p + 1);
  int _sv0t3 = lower_vecf_tag_at(tok_tags, _sv0t2);
  int next = _sv0t3;
  if ((prev == 6)) {
    int _sv0t4 = (p - 2);
    int _sv0t5 = lower_vecf_tag_at(tok_tags, _sv0t4);
    if ((_sv0t5 == 5)) {
      if ((next == 12)) {
        int callee = (p - 2);
        int elem = (0 - 1);
        int _sv0t6 = lower_vecf_text_is(source, starts, ends, callee, "vec_push");
        if (_sv0t6) {
          elem = (p + 2);
        } else {
        }
        int _sv0t7 = lower_vecf_text_is(source, starts, ends, callee, "vec_set");
        if (_sv0t7) {
          int q = (p + 2);
          int d = 0;
          while (1) {
            int _sv0t8 = sv0_vec_len(tok_tags);
            int _sv0t10 = (q < _sv0t8);
            if ((!_sv0t10)) {
              break;
            } else {
            }
            int _sv0t9 = sv0_vec_get(tok_tags, q);
            int u = _sv0t9;
            if ((u == 6)) {
              d = (d + 1);
            } else {
        ...
        if ((elem >= 0)) {
          int _sv0t11 = lower_vecf_tag_at(tok_tags, elem);
          int et = _sv0t11;
          if ((et == 1)) {
            int _sv0t12 = sv0_vec_get(out, 0);
            int _sv0t13 = (_sv0t12 + 1);
            sv0_vec_set(out, 0, _sv0t13);
          } else {
          }
          if ((et == 0)) {
            int _sv0t14 = sv0_vec_get(out, 1);
            int _sv0t15 = (_sv0t14 + 1);
            sv0_vec_set(out, 1, _sv0t15);
          } else {
          }
        } else {
        }
        int _sv0t16 = lower_vecf_text_is(source, starts, ends, callee, "vec_get");
        if (_sv0t16) {
          int _sv0t17 = (p - 3);
          int _sv0t18 = lower_vecf_tag_at(tok_tags, _sv0t17);
          if ((_sv0t18 == 35)) {
            int _sv0t19 = (p - 5);
            int _sv0t20 = lower_vecf_tag_at(tok_tags, _sv0t19);
            if ((_sv0t20 == 14)) {
              int _sv0t21 = (p - 4);
              int _sv0t22 = lower_vecf_text_is(source, starts, ends, _sv0t21, "f64");
              if (_sv0t22) {
                int _sv0t23 = sv0_vec_get(out, 0);
                int _sv0t24 = (_sv0t23 + 1);
                sv0_vec_set(out, 0, _sv0t24);
              } else {
              }
              int _sv0t25 = (p - 4);
              int _sv0t26 = lower_vecf_word_type(source, starts, ends, _sv0t25);
              if (_sv0t26) {
                int _sv0t27 = sv0_vec_get(out, 1);
                int _sv0t28 = (_sv0t27 + 1);
                sv0_vec_set(out, 1, _sv0t28);
              } else {
              }
            } else {
            }
  ...
  if (argpos) {
    if (endpos) {
      ...
      int open = (0 - 1);
      while ((q >= lo)) {
        int _sv0t29 = sv0_vec_get(tok_tags, q);
        int u = _sv0t29;
        if ((u == 7)) {
          d = (d + 1);
        } else {
      ...
      if ((open >= 1)) {
        int _sv0t30 = (open - 1);
        int _sv0t31 = lower_vecf_tag_at(tok_tags, _sv0t30);
        if ((_sv0t31 == 5)) {
          int _sv0t32 = (open - 1);
          int _sv0t33 = lower_vecf_accessor_kind(source, starts, ends, _sv0t32);
          if ((_sv0t33 == 0)) {
            int _sv0t34 = (open - 1);
            int _sv0t35 = lower_vecf_fn_param_kind(pfn_names, pfn_off, pfn_cnt, pkinds, source, starts, ends, _sv0t34, commas);
            int pk = _sv0t35;
            if ((pk == 1)) {
              int _sv0t36 = sv0_vec_get(out, 0);
              int _sv0t37 = (_sv0t36 + 1);
              sv0_vec_set(out, 0, _sv0t37);
            } else {
            }
            if ((pk == 3)) {
              int _sv0t38 = sv0_vec_get(out, 1);
              int _sv0t39 = (_sv0t38 + 1);
              sv0_vec_set(out, 1, _sv0t39);
            } else {
            }
          } else {
          }
  ...
  if ((prev == 85)) {
    if ((next == 13)) {
      if ((retkind == 1)) {
        int _sv0t40 = sv0_vec_get(out, 0);
        int _sv0t41 = (_sv0t40 + 1);
        sv0_vec_set(out, 0, _sv0t41);
      } else {
      }
      if ((retkind == 3)) {
        int _sv0t42 = sv0_vec_get(out, 1);
        int _sv0t43 = (_sv0t42 + 1);
        sv0_vec_set(out, 1, _sv0t43);
      } else {
      }
    } else {
    }
  ...
  if ((prev == 14)) {
    int _sv0t44 = (p - 2);
    int _sv0t45 = lower_vecf_tag_at(tok_tags, _sv0t44);
    if ((_sv0t45 == 5)) {
      int _sv0t46 = (p - 3);
      int _sv0t47 = lower_vecf_tag_at(tok_tags, _sv0t46);
      int before = _sv0t47;
      if ((before == 8)) {
        if (endpos) {
          int _sv0t48 = (p - 2);
          int _sv0t49 = lower_vecf_field_evidence(fnames, fkinds, source, starts, ends, _sv0t48, out);
        } else {
        }
      } else {
      }
      if ((before == 12)) {
        if (endpos) {
          int _sv0t50 = (p - 2);
          int _sv0t51 = lower_vecf_field_evidence(fnames, fkinds, source, starts, ends, _sv0t50, out);
        } else {
        }
      } else {
      }
      if ((before == 9)) {
        if (endpos) {
          int _sv0t52 = (p - 2);
          int _sv0t53 = lower_vecf_field_evidence(fnames, fkinds, source, starts, ends, _sv0t52, out);
        } else {
        }
      } else {
      }
...
static int lower_vecf_field_evidence(int fnames, int fkinds, const char* source, int starts, int ends, int field_tok, int out) {
  int _sv0t0 = lower_vecf_lookup(fnames, fkinds, source, starts, ends, field_tok);
  int code = _sv0t0;
  if ((code == 1)) {
    int _sv0t1 = sv0_vec_get(out, 0);
    int _sv0t2 = (_sv0t1 + 1);
    sv0_vec_set(out, 0, _sv0t2);
  } else {
  }
  if ((code == 3)) {
    int _sv0t3 = sv0_vec_get(out, 1);
    int _sv0t4 = (_sv0t3 + 1);
    sv0_vec_set(out, 1, _sv0t4);
  } else {
  }
  return 0;
}
...
static int lower_vecf_resolve_fresh(int tok_tags, const char* source, int starts, int ends, int lo, int body_lo, int hi, int names, int kinds, int fnames, int fkinds, int pfn_names, int pfn_off, int pfn_cnt, int pkinds, int retkind) {
  ...
  int nd = _sv0t0;
  while ((d < nd)) {
    int _sv0t1 = sv0_vec_get(kinds, d);
    if ((_sv0t1 == 4)) {
      int _sv0t2 = sv0_vec_new();
      int out = _sv0t2;
      sv0_vec_push(out, 0);
      ...
      while ((p < hi)) {
        int _sv0t3 = sv0_vec_get(tok_tags, p);
        if ((_sv0t3 == 5)) {
          int _sv0t4 = sv0_vec_get(names, d);
          if ((p != _sv0t4)) {
            int _sv0t5 = sv0_vec_get(names, d);
            int _sv0t6 = lower_vecf_same_text(source, starts, ends, _sv0t5, p);
            if (_sv0t6) {
              int _sv0t7 = lower_vecf_evidence_at(tok_tags, source, starts, ends, lo, p, fnames, fkinds, pfn_names, pfn_off, pfn_cnt, pkinds, retkind, out);
              int _e = _sv0t7;
            } else {
            }
          } else {
...
static int lower_vecf_accessor_kind(const char* source, int starts, int ends, int t) {
  ...
  int _sv0t2 = sv0_vec_get(ends, t);
  int _sv0t3 = sv0_vec_get(starts, t);
  int len = (_sv0t2 - _sv0t3);
  if ((len != 7)) {
    if ((len != 8)) {
      if ((len != 11)) {
        if ((len != 12)) {
  ...
  int _sv0t4 = lower_vecf_text_is(source, starts, ends, t, "vec_push");
  if (_sv0t4) {
    return 1;
  } else {
  }
  int _sv0t5 = lower_vecf_text_is(source, starts, ends, t, "vec_get");
  if (_sv0t5) {
    return 1;
  } else {
  }
  int _sv0t6 = lower_vecf_text_is(source, starts, ends, t, "vec_set");
  if (_sv0t6) {
    return 1;
  } else {
  }
  int _sv0t7 = lower_vecf_text_is(source, starts, ends, t, "vec_push_f64");
  if (_sv0t7) {
    return 2;
  } else {
  }
  int _sv0t8 = lower_vecf_text_is(source, starts, ends, t, "vec_get_f64");
  if (_sv0t8) {
    return 2;
  } else {
  }
  int _sv0t9 = lower_vecf_text_is(source, starts, ends, t, "vec_set_f64");
  if (_sv0t9) {
    return 2;
  } else {
  }
...
static int lower_vecf_site_map(int tok_tags, const char* source, int starts, int ends) {
  ...
  int i = 0;
  while ((i < n)) {
    int _sv0t12 = sv0_vec_get(tok_tags, i);
    int t = _sv0t12;
    if ((t == 89)) {
      int _sv0t13 = lower_vecf_collect_struct(tok_tags, source, starts, ends, i, fnames, fkinds);
      i = _sv0t13;
    } else {
      if ((t == 65)) {
        int _sv0t14 = (i + 1);
        int _sv0t15 = lower_vecf_tag_at(tok_tags, _sv0t14);
        if ((_sv0t15 == 5)) {
          int _sv0t16 = lower_vecf_params_open(tok_tags, i);
          int open = _sv0t16;
          if ((open >= 0)) {
            int _sv0t17 = lower_vecf_close_paren(tok_tags, open);
            int close = _sv0t17;
            if ((close >= 0)) {
              int _sv0t18 = sv0_vec_len(pkinds);
              int before_n = _sv0t18;
              int _sv0t19 = lower_vecf_param_kinds(tok_tags, source, starts, ends, i, pkinds);
              int _pk = _sv0t19;
              int _sv0t20 = (i + 1);
              sv0_vec_push(pfn_names, _sv0t20);
              sv0_vec_push(pfn_off, before_n);
              int _sv0t21 = sv0_vec_len(pkinds);
              int _sv0t22 = (_sv0t21 - before_n);
              sv0_vec_push(pfn_cnt, _sv0t22);
              int _sv0t23 = (close + 1);
              int _sv0t24 = lower_vecf_tag_at(tok_tags, _sv0t23);
              if ((_sv0t24 == 19)) {
                int _sv0t25 = (i + 1);
                sv0_vec_push(rnames, _sv0t25);
                int _sv0t26 = (close + 2);
                int _sv0t27 = lower_vecf_type_kind(tok_tags, source, starts, ends, _sv0t26);
                sv0_vec_push(rkinds, _sv0t27);
              } else {
              }
            } else {
            }
  ...
  i = 0;
  while ((i < n)) {
    int _sv0t28 = sv0_vec_get(tok_tags, i);
    int ft = _sv0t28;
    int advance = (i + 1);
    if ((ft == 65)) {
      int _sv0t29 = (i + 1);
      int _sv0t30 = lower_vecf_tag_at(tok_tags, _sv0t29);
      if ((_sv0t30 == 5)) {
        int _sv0t31 = lower_vecf_fn_end(tok_tags, i);
        int hi = _sv0t31;
        int _sv0t32 = sv0_vec_new();
        int names = _sv0t32;
        int _sv0t33 = sv0_vec_new();
        int kinds = _sv0t33;
        int _sv0t34 = lower_vecf_collect_params(tok_tags, source, starts, ends, i, names, kinds);
        int _p = _sv0t34;
        int _sv0t35 = lower_vecf_collect_lets(tok_tags, source, starts, ends, i, hi, rnames, rkinds, names, kinds, map);
        int _l = _sv0t35;
        int _sv0t36 = lower_vecf_params_open(tok_tags, i);
        int fopen = _sv0t36;
        int body_lo = i;
        int retkind = 2;
        if ((fopen >= 0)) {
          int _sv0t37 = lower_vecf_close_paren(tok_tags, fopen);
          int fclose = _sv0t37;
          if ((fclose >= 0)) {
            body_lo = (fclose + 1);
            int _sv0t38 = (fclose + 1);
            int _sv0t39 = lower_vecf_tag_at(tok_tags, _sv0t38);
            if ((_sv0t39 == 19)) {
              int _sv0t40 = (fclose + 2);
              int _sv0t41 = lower_vecf_type_kind(tok_tags, source, starts, ends, _sv0t40);
              retkind = _sv0t41;
            } else {
            }
          } else {
        ...
        int _sv0t42 = lower_vecf_resolve_fresh(tok_tags, source, starts, ends, i, body_lo, hi, names, kinds, fnames, fkinds, pfn_names, pfn_off, pfn_cnt, pkinds, retkind);
        int _r = _sv0t42;
        int s = i;
        while ((s < hi)) {
          ...
          if ((_sv0t43 == 5)) {
            ...
            if ((_sv0t45 == 10)) {
              ...
              int _sv0t47 = lower_vecf_tag_at(tok_tags, _sv0t46);
              if ((_sv0t47 == 16)) {
                int _sv0t48 = lower_vecf_lookup(fnames, fkinds, source, starts, ends, s);
                icode = _sv0t48;
              } else {
                int _sv0t49 = lower_vecf_lookup(names, kinds, source, starts, ends, s);
                icode = _sv0t49;
              }
              if ((icode == 1)) {
                sv0_vec_set(map, s, 1);
            ...
            int _sv0t50 = (s + 1);
            int _sv0t51 = lower_vecf_tag_at(tok_tags, _sv0t50);
            if ((_sv0t51 == 6)) {
              int _sv0t52 = lower_vecf_accessor_kind(source, starts, ends, s);
              int ak = _sv0t52;
              if ((ak != 0)) {
                int a0 = (s + 2);
                int code = 0;
                int _sv0t53 = lower_vecf_tag_at(tok_tags, a0);
                if ((_sv0t53 == 5)) {
                  int _sv0t54 = (a0 + 1);
                  int _sv0t55 = lower_vecf_tag_at(tok_tags, _sv0t54);
                  int nx = _sv0t55;
                  if ((nx == 12)) {
                    int _sv0t56 = lower_vecf_lookup(names, kinds, source, starts, ends, a0);
                    code = _sv0t56;
                  } else {
                  }
                  if ((nx == 7)) {
                    int _sv0t57 = lower_vecf_lookup(names, kinds, source, starts, ends, a0);
                    code = _sv0t57;
                  } else {
                  }
                  if ((nx == 16)) {
                    ...
                    while (1) {
                      int _sv0t58 = (fq + 1);
                      int _sv0t59 = lower_vecf_tag_at(tok_tags, _sv0t58);
                      ...
                      int _sv0t60 = (fq + 2);
                      int _sv0t61 = lower_vecf_tag_at(tok_tags, _sv0t60);
                      if ((_sv0t61 != 5)) {
                        break;
                      } else {
                      }
                    ...
                    int _sv0t63 = (fq + 1);
                    int _sv0t64 = lower_vecf_tag_at(tok_tags, _sv0t63);
                    int nx2 = _sv0t64;
                    if ((fq != a0)) {
                      if ((nx2 == 12)) {
                        int _sv0t65 = lower_vecf_lookup(fnames, fkinds, source, starts, ends, fq);
                        code = _sv0t65;
                      } else {
                      }
                      if ((nx2 == 7)) {
                        int _sv0t66 = lower_vecf_lookup(fnames, fkinds, source, starts, ends, fq);
                        code = _sv0t66;
                      } else {
                      }
                    } else {
                  ...
                  if ((nx == 6)) {
                    int _sv0t67 = (a0 + 1);
                    int _sv0t68 = lower_vecf_close_paren(tok_tags, _sv0t67);
                    int cc = _sv0t68;
                    if ((cc > 0)) {
                      int _sv0t69 = (cc + 1);
                      int _sv0t70 = lower_vecf_tag_at(tok_tags, _sv0t69);
                      int nx3 = _sv0t70;
                      if ((nx3 == 12)) {
                        int _sv0t71 = lower_vecf_lookup(rnames, rkinds, source, starts, ends, a0);
                        code = _sv0t71;
                      } else {
                      }
                      if ((nx3 == 7)) {
                        int _sv0t72 = lower_vecf_lookup(rnames, rkinds, source, starts, ends, a0);
                        code = _sv0t72;
                      } else {
                      }
                    } else {
...
static int lower_index_base_tok(int et, int ed1, int ed2, int pp, int ob) {
  ...
  int _sv0t3 = sv0_vec_get(et, ob);
  int t = _sv0t3;
  if ((t == 1)) {
    int _sv0t4 = sv0_vec_get(ed1, ob);
    int pps = _sv0t4;
    if ((pps < 0)) {
      int _sv0t5 = (0 - 1);
      return _sv0t5;
...
static int lower_index_bid(int builtin_map, int tok, int plain, int f64_bid) {
  ...
  int _sv0t0 = sv0_vec_len(builtin_map);
  if ((tok >= _sv0t0)) {
    return plain;
  } else {
  }
  int _sv0t1 = sv0_vec_get(builtin_map, tok);
  if ((_sv0t1 == 41)) {
    return f64_bid;
  } else {
  }
...
static int lower_array_lit_is_f64(int et, int ed1, int ed2, int ed3, int pp, int tok_tags, int builtin_map, int idx) {
  int _sv0t0 = sv0_vec_get(ed2, idx);
  int count = _sv0t0;
  if ((count < 1)) {
    return 0;
  } else {
  }
  int _sv0t1 = (idx - count);
  int _sv0t2 = expr_any_tok(et, ed1, ed2, ed3, pp, _sv0t1);
  int first = _sv0t2;
  if ((first < 1)) {
    return 0;
  } else {
  ...
  int b = (first - 1);
  while ((b >= 0)) {
    int _sv0t3 = sv0_vec_get(tok_tags, b);
    int u = _sv0t3;
    if ((u == 6)) {
      b = (b - 1);
    } else {
  ...
  int _sv0t4 = sv0_vec_len(builtin_map);
  if ((b >= _sv0t4)) {
    return 0;
  } else {
  }
  int _sv0t5 = sv0_vec_get(tok_tags, b);
  if ((_sv0t5 != 10)) {
    return 0;
  } else {
  }
  int _sv0t6 = sv0_vec_get(builtin_map, b);
  int _sv0t7 = (_sv0t6 == 44);
  return _sv0t7;
}

static int lower_vecf_apply(int bm, int sites, int tok_tags) {
  int _sv0t0 = sv0_vec_len(bm);
  int n = _sv0t0;
  int i = 0;
  while ((i < n)) {
    int _sv0t1 = sv0_vec_len(sites);
    if ((i < _sv0t1)) {
      int _sv0t2 = sv0_vec_get(sites, i);
      int sv = _sv0t2;
      if ((sv == 1)) {
        int _sv0t3 = sv0_vec_get(bm, i);
        int b = _sv0t3;
        if ((b == 8)) {
          sv0_vec_set(bm, i, 38);
        } else {
        ...
        if ((b == 0)) {
          int _sv0t4 = (i + 1);
          int _sv0t5 = sv0_vec_len(tok_tags);
          if ((_sv0t4 < _sv0t5)) {
            int _sv0t6 = (i + 1);
            int _sv0t7 = sv0_vec_get(tok_tags, _sv0t6);
            if ((_sv0t7 == 10)) {
              sv0_vec_set(bm, i, 41);
            } else {
            }
...
static int build_builtin_map_classify(const char* s) {
  ...
  int _sv0t23 = sv0_string_eq(s, "vec_push_f64");
  if (_sv0t23) {
    return 38;
  } else {
  }
  int _sv0t24 = sv0_string_eq(s, "vec_get_f64");
  if (_sv0t24) {
    return 39;
  } else {
  }
  int _sv0t25 = sv0_string_eq(s, "vec_set_f64");
  if (_sv0t25) {
    return 40;
  } else {
  }
...
static int lower_tok_str_eq(const char* source, int starts, int ends, int a, int b) {
  ...
  int _sv0t2 = sv0_vec_get(starts, a);
  int as2 = _sv0t2;
  int _sv0t3 = sv0_vec_get(ends, a);
  int ae = _sv0t3;
  int _sv0t4 = sv0_vec_get(starts, b);
  int bs = _sv0t4;
  int _sv0t5 = sv0_vec_get(ends, b);
  int be = _sv0t5;
  int _sv0t6 = (ae - as2);
  const char* _sv0t7 = sv0_string_substr(source, as2, _sv0t6);
  int _sv0t8 = (be - bs);
  const char* _sv0t9 = sv0_string_substr(source, bs, _sv0t8);
  int _sv0t10 = sv0_string_eq(_sv0t7, _sv0t9);
  return _sv0t10;
}
...
static int lower_name_is_enum(int enum_names, int tok, const char* source, int starts, int ends) {
  ...
  int _sv0t3 = (te - ts);
  const char* _sv0t4 = sv0_string_substr(source, ts, _sv0t3);
  const char* nm;
  nm = _sv0t4;
  int _sv0t5 = sv0_vec_len(enum_names);
  int ne = _sv0t5;
  int j = 0;
  while ((j < ne)) {
    int _sv0t6 = sv0_vec_get(enum_names, j);
    int et = _sv0t6;
    if ((et >= 0)) {
      int _sv0t7 = sv0_vec_len(starts);
      if ((et < _sv0t7)) {
        ...
        int _sv0t12 = sv0_string_eq(nm, _sv0t11);
        if (_sv0t12) {
          return 1;
        } else {
        }
...
static const char* lower(int item_tags, int item_names, int item_d2, int item_d3, int item_d4, int item_field_counts, int item_vmax, int struct_fnames_flat, int enum_vnames_flat, int sf_names, int sf_types, const char* source, int starts, int ends, int out_block_labels, int out_struct_names, int out_struct_offsets, int out_struct_counts, int out_struct_flat, int out_enum_names, int out_enum_tag_offsets, int out_enum_tag_counts, int out_enum_tag_flat, int out_enum_max, int body_et, int body_ed1, int body_ed2, int body_ed3, int body_ed4, int body_sf, int body_pp, int tok_tags, int out_blocks, int import_aliases, int fn_contract_root, int fn_contract_base, int fn_ret_ty_root, int pty_tt, int pty_td1, int contract_mode, int proven_lines, int enum_variant_payload_count_by_variant, int enum_variant_payload_ty_name_tok) {
  ...
  const char* _sv0t3 = collect_typedefs_str(item_tags, item_names, item_field_counts, item_vmax, sf_names, sf_types, source, starts, ends, out_struct_names, out_enum_names, item_d3, enum_variant_payload_count_by_variant, enum_variant_payload_ty_name_tok);
  const char* typedefs;
  typedefs = _sv0t3;
  ...
  if ((has_body_arenas > 0)) {
    ...
    builtin_map = _sv0t6;
    int _sv0t7 = lower_vecf_site_map(tok_tags, source, starts, ends);
    int vecf_sites = _sv0t7;
    int _sv0t8 = lower_vecf_apply(builtin_map, vecf_sites, tok_tags);
    int _va = _sv0t8;
  } else {
  }
  int _sv0t9 = sv0_vec_new();
  int fn_ctx = _sv0t9;
  int _sv0t10 = sv0_vec_len(item_tags);
  int n = _sv0t10;
  int _sv0t11 = sv0_vec_new();
  int ctx_names = _sv0t11;
  int _sv0t12 = sv0_vec_new();
  int ctx_enums = _sv0t12;
  int ci = 0;
  while ((ci < n)) {
    int _sv0t13 = sv0_vec_get(item_tags, ci);
    if ((_sv0t13 == 0)) {
      int _sv0t14 = sv0_vec_len(fn_ret_ty_root);
      if ((ci < _sv0t14)) {
        int _sv0t15 = sv0_vec_get(fn_ret_ty_root, ci);
        int rroot = _sv0t15;
        if ((rroot >= 0)) {
          int _sv0t16 = sv0_vec_len(pty_tt);
          if ((rroot < _sv0t16)) {
            int _sv0t17 = sv0_vec_get(pty_tt, rroot);
            if ((_sv0t17 == 0)) {
              int _sv0t18 = sv0_vec_get(pty_td1, rroot);
              int rpps = _sv0t18;
              if ((rpps >= 0)) {
                int _sv0t19 = sv0_vec_len(body_pp);
                if ((rpps < _sv0t19)) {
                  int _sv0t20 = sv0_vec_get(body_pp, rpps);
                  int rtok = _sv0t20;
                  int _sv0t21 = lower_name_is_enum(out_enum_names, rtok, source, starts, ends);
                  if (_sv0t21) {
                    int _sv0t22 = sv0_vec_get(item_names, ci);
                    sv0_vec_push(ctx_names, _sv0t22);
                    sv0_vec_push(ctx_enums, rtok);
                  } else {
                  }
                } else {
                }
  ...
  int _sv0t23 = sv0_vec_len(ctx_names);
  sv0_vec_push(fn_ctx, _sv0t23);
  int ck = 0;
  while (1) {
    int _sv0t24 = sv0_vec_len(ctx_names);
    int _sv0t26 = (ck < _sv0t24);
    if ((!_sv0t26)) {
      break;
    } else {
    }
    int _sv0t25 = sv0_vec_get(ctx_names, ck);
    sv0_vec_push(fn_ctx, _sv0t25);
    ck = (ck + 1);
  }
  ck = 0;
  while (1) {
    int _sv0t27 = sv0_vec_len(ctx_enums);
    int _sv0t29 = (ck < _sv0t27);
    if ((!_sv0t29)) {
      break;
    } else {
    }
    int _sv0t28 = sv0_vec_get(ctx_enums, ck);
...
static int lower_callee_ret_ty_head(int et, int ed1, int ed2, int pp, int call_idx, int item_tags, int item_names, int item_d2, int tok_tags, const char* source, int starts, int ends) {
  ...
  const char* cname;
  cname = _sv0t20;
  int _sv0t21 = lower_ret_ty_head_from_name(cname, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
  return _sv0t21;
}
...
static int lower_ret_ty_head_from_name(const char* cname, int item_tags, int item_names, int item_d2, int tok_tags, const char* source, int starts, int ends) {
  ...
  int nn = _sv0t1;
  int _sv0t2 = sv0_vec_len(item_d2);
  int nd = _sv0t2;
  int i = 0;
  while ((i < n)) {
    if ((i < nn)) {
      if ((i < nd)) {
        int _sv0t3 = sv0_vec_get(item_tags, i);
        if ((_sv0t3 == 0)) {
          int _sv0t4 = sv0_vec_get(item_names, i);
          int nm = _sv0t4;
          int _sv0t5 = lower_tok_in_bounds(nm, starts, ends);
          if (_sv0t5) {
            const char* _sv0t6 = handle_to_str(nm, source, starts, ends);
            int _sv0t7 = sv0_string_eq(_sv0t6, cname);
            if (_sv0t7) {
              int _sv0t8 = sv0_vec_get(item_d2, i);
              int packed = _sv0t8;
              int _sv0t9 = (packed % 2);
              if ((_sv0t9 == 1)) {
                int _sv0t10 = lower_scan_fn_ret_ty_tok(tok_tags, source, starts, ends, nm);
                int p3 = _sv0t10;
                int _sv0t11 = lower_tok_in_bounds(p3, starts, ends);
                if (_sv0t11) {
                  return p3;
                } else {
                }
...
static int lower_mcall_ret_user_ty_tok(int et, int ed2, int mcall_idx, int item_tags, int item_names, int item_d2, int tok_tags, const char* source, int starts, int ends) {
  ...
  int _sv0t3 = sv0_vec_get(et, mcall_idx);
  if ((_sv0t3 != 5)) {
    int _sv0t4 = (0 - 1);
    return _sv0t4;
  } else {
  }
  int _sv0t5 = sv0_vec_get(ed2, mcall_idx);
  int mtok = _sv0t5;
  int _sv0t6 = lower_tok_in_bounds(mtok, starts, ends);
  if ((_sv0t6 != 1)) {
    int _sv0t7 = (0 - 1);
    return _sv0t7;
  } else {
  }
  const char* _sv0t8 = handle_to_str(mtok, source, starts, ends);
  const char* mname;
  mname = _sv0t8;
  int _sv0t9 = lower_ret_ty_head_from_name(mname, item_tags, item_names, item_d2, tok_tags, source, starts, ends);
  int head = _sv0t9;
  if ((head < 0)) {
    int _sv0t10 = (0 - 1);
    return _sv0t10;
  ...
  const char* _sv0t11 = handle_to_str(head, source, starts, ends);
  const char* rty;
  rty = _sv0t11;
  int _sv0t12 = lower_name_is_user_type(rty, item_tags, item_names, source, starts, ends);
  if (_sv0t12) {
    return head;
  } else {
  }
...
static int enum_variant_def_tok_lookup_str(int enum_names, int enum_tags_flat, int enum_tag_offsets, int enum_tag_counts, int enum_name, int variant_name, const char* source, int starts, int ends) {
  ...
  int ei2 = 0;
  while ((ei2 < n2)) {
    int _sv0t3 = sv0_vec_get(enum_names, ei2);
    const char* _sv0t4 = handle_to_str(_sv0t3, source, starts, ends);
    const char* e_nm2;
    e_nm2 = _sv0t4;
    int _sv0t5 = sv0_string_eq(e_nm2, want_e2);
    if (_sv0t5) {
      int _sv0t6 = sv0_vec_get(enum_tag_offsets, ei2);
      int offset2 = _sv0t6;
      int _sv0t7 = sv0_vec_get(enum_tag_counts, ei2);
      int count2 = _sv0t7;
      int vi2 = 0;
      while ((vi2 < count2)) {
        int _sv0t8 = (vi2 * 2);
        int flat_idx2 = (offset2 + _sv0t8);
        int _sv0t9 = sv0_vec_get(enum_tags_flat, flat_idx2);
        const char* _sv0t10 = handle_to_str(_sv0t9, source, starts, ends);
        const char* v_nm2;
        v_nm2 = _sv0t10;
        int _sv0t11 = sv0_string_eq(v_nm2, want_v2);
        if (_sv0t11) {
          int _sv0t12 = sv0_vec_get(enum_tags_flat, flat_idx2);
          return _sv0t12;
        } else {
        }
        vi2 = (vi2 + 1);
...
static int enum_max_payload_lookup_str(int enum_names, int enum_max_payloads, int name, const char* source, int starts, int ends) {
  const char* _sv0t0 = handle_to_str(name, source, starts, ends);
  const char* want;
  want = _sv0t0;
  ...
  int i = 0;
  while ((i < n)) {
    int _sv0t2 = sv0_vec_get(enum_names, i);
    const char* _sv0t3 = handle_to_str(_sv0t2, source, starts, ends);
    const char* e_nm;
    e_nm = _sv0t3;
    int _sv0t4 = sv0_string_eq(e_nm, want);
    if (_sv0t4) {
      int _sv0t5 = sv0_vec_get(enum_max_payloads, i);
      return _sv0t5;
    } else {
    }
    i = (i + 1);
...
static const char* emit_enum_td(int name_h, int max_payload, int slot_cats, const char* source, int starts, int ends) {
  ...
  int i = 0;
  while ((i < max_payload)) {
    const char* _sv0t1 = sv0_string_concat(r, "  ");
    r = _sv0t1;
    int _sv0t2 = sv0_vec_get(slot_cats, i);
    const char* _sv0t3 = enum_slot_category_to_cty(_sv0t2);
    const char* _sv0t4 = sv0_string_concat(r, _sv0t3);
    r = _sv0t4;
    const char* _sv0t5 = sv0_string_concat(r, " p");
    r = _sv0t5;
    const char* _sv0t6 = lower_int_to_str(i);
    const char* _sv0t7 = sv0_string_concat(r, _sv0t6);
    r = _sv0t7;
    const char* _sv0t8 = sv0_string_concat(r, ";\n");
    r = _sv0t8;
    i = (i + 1);
  }
  const char* _sv0t9 = sv0_string_concat(r, "} ");
  r = _sv0t9;
  const char* _sv0t10 = sv0_string_concat(r, name);
  r = _sv0t10;
  const char* _sv0t11 = sv0_string_concat(r, ";\n");
  r = _sv0t11;
  return r;
...
static int enum_payload_ty_name_to_category(const char* name) {
  int _sv0t0 = sv0_string_eq(name, "i64");
  if (_sv0t0) {
    return 2;
  } else {
  }
  int _sv0t1 = sv0_string_eq(name, "u64");
  if (_sv0t1) {
    return 2;
  } else {
  }
  int _sv0t2 = sv0_string_eq(name, "u32");
  if (_sv0t2) {
    return 4;
  } else {
  }
  int _sv0t3 = sv0_string_eq(name, "f32");
  if (_sv0t3) {
    return 1;
  } else {
  }
  int _sv0t4 = sv0_string_eq(name, "f64");
  if (_sv0t4) {
    return 1;
  } else {
  }
...
static int enum_slot_ctys_for_item(int vbase, int vcount, int max_payload, int evpv, int evptn, int payload_offset_by_variant, const char* source, int starts, int ends) {
  ...
  while ((s < max_payload)) {
    ...
    int v = vbase;
    while (1) {
      int _sv0t1 = (vbase + vcount);
      int _sv0t9 = (v < _sv0t1);
      if ((!_sv0t9)) {
        break;
      } else {
      }
      int _sv0t2 = sv0_vec_get(evpv, v);
      int vpc = _sv0t2;
      if ((s < vpc)) {
        int _sv0t3 = sv0_vec_get(payload_offset_by_variant, v);
        int _sv0t4 = (_sv0t3 + s);
        int _sv0t5 = sv0_vec_get(evptn, _sv0t4);
        int ptok = _sv0t5;
        int cat = 0;
        if ((ptok >= 0)) {
          int _sv0t6 = lower_tok_in_bounds(ptok, starts, ends);
          if (_sv0t6) {
            const char* _sv0t7 = handle_to_str(ptok, source, starts, ends);
            int _sv0t8 = enum_payload_ty_name_to_category(_sv0t7);
            cat = _sv0t8;
          } else {
          }
        } else {
    ...
    if (conflict) {
      sv0_vec_push(slot_cats, 0);
    } else {
      if ((agreed_cat < 0)) {
        sv0_vec_push(slot_cats, 0);
      } else {
        sv0_vec_push(slot_cats, agreed_cat);
      }
    }
    s = (s + 1);
  }
...
static const char* collect_typedefs_str(int item_tags, int item_names, int item_field_counts, int item_vmax, int sf_names, int sf_types, const char* source, int starts, int ends, int sn, int en, int item_d3, int evpv, int evptn) {
  ...
  int _sv0t6 = sv0_vec_new();
  int payload_offset_by_variant = _sv0t6;
  int _sv0t7 = sv0_vec_len(evpv);
  int vn = _sv0t7;
  int running = 0;
  int vi = 0;
  while ((vi < vn)) {
    sv0_vec_push(payload_offset_by_variant, running);
    int _sv0t8 = sv0_vec_get(evpv, vi);
    running = (running + _sv0t8);
    vi = (vi + 1);
  }
  i = 0;
  while ((i < n)) {
    ...
    if ((_sv0t9 == 2)) {
      int _sv0t10 = sv0_vec_get(item_vmax, i);
      int mp = _sv0t10;
      int _sv0t11 = sv0_vec_get(item_d3, i);
      int _sv0t12 = sv0_vec_get(item_field_counts, i);
      int _sv0t13 = enum_slot_ctys_for_item(_sv0t11, _sv0t12, mp, evpv, evptn, payload_offset_by_variant, source, starts, ends);
      int slot_cats = _sv0t13;
      int _sv0t14 = sv0_vec_get(item_names, i);
      const char* _sv0t15 = emit_enum_td(_sv0t14, mp, slot_cats, source, starts, ends);
      const char* td2;
      td2 = _sv0t15;
      const char* _sv0t16 = sv0_string_concat(r, td2);
      r = _sv0t16;
    } else {
    }
    i = (i + 1);
...
static int test_typedef_builders(void) {
  ...
  int _sv0t10 = sv0_vec_new();
  int slot_cats0 = _sv0t10;
  sv0_vec_push(slot_cats0, 0);
  const char* _sv0t11 = emit_enum_td(5, 1, slot_cats0, src, st, nd);
  const char* td2;
  td2 = _sv0t11;
  int _sv0t12 = sv0_string_eq(td2, "typedef struct {\n  int tag;\n  int p0;\n} E;\n");
  if ((_sv0t12 != 1)) {
    return 3;
  } else {
  }
  ...
  int _sv0t21 = sv0_vec_new();
  int of1 = _sv0t21;
  int _sv0t22 = build_struct_order(tags, nms, fcs, sff, on, oo, oc, of1);
  if ((_sv0t22 != 1)) {
    return 4;
  } else {
  }
  int _sv0t23 = sv0_vec_get(oc, 0);
  if ((_sv0t23 != 2)) {
    return 5;
  } else {
  }
  ...
  int _sv0t29 = sv0_vec_new();
  int emp = _sv0t29;
  int _sv0t30 = build_enum_variants_data(tags, nms, fcs, vmx, evn, en2, eto, etc, etf, emp);
  if ((_sv0t30 != 1)) {
    return 6;
  } else {
  }
  int _sv0t31 = sv0_vec_get(emp, 0);
  if ((_sv0t31 != 1)) {
    return 7;
  } else {
  }
...
static int test_lower_vecf_lookup_kinds(void) {
  ...
  int _sv0t3 = sv0_vec_new();
  int kinds = _sv0t3;
  sv0_vec_push(names, 0);
  sv0_vec_push(kinds, 1);
  int _sv0t4 = lower_vecf_lookup(names, kinds, src, st, en, 1);
  if ((_sv0t4 != 1)) {
    return 1;
  } else {
  }
  sv0_vec_push(names, 1);
  sv0_vec_push(kinds, 2);
  int _sv0t5 = lower_vecf_lookup(names, kinds, src, st, en, 1);
  if ((_sv0t5 != 2)) {
    return 2;
  } else {
  }
  ...
  sv0_vec_push(n3, 1);
  sv0_vec_push(k3, 3);
  int _sv0t8 = lower_vecf_lookup(n3, k3, src, st, en, 1);
  if ((_sv0t8 != 3)) {
    return 3;
  } else {
  }
  ...
  sv0_vec_push(n2, 0);
  sv0_vec_push(k2, 2);
  int _sv0t11 = lower_vecf_lookup(n2, k2, src, st, en, 1);
  if ((_sv0t11 != 0)) {
    return 4;
  } else {
  }
  int _sv0t12 = sv0_vec_new();
  int empty = _sv0t12;
  int _sv0t13 = lower_vecf_lookup(empty, empty, src, st, en, 1);
  if ((_sv0t13 != 0)) {
    return 5;
  } else {
  }
...
static int lower_vecf_test_lex(const char* src, int tags, int st, int en) {
  ...
  int i = 0;
  while ((i < n)) {
    int _sv0t1 = sv0_string_char_at(src, i);
    int c = _sv0t1;
    if ((c == 32)) {
      i = (i + 1);
    } else {
      ...
      int _sv0t9 = (c == 95);
      int alpha = (_sv0t8 || _sv0t9);
      if (alpha) {
        while ((j < n)) {
          int _sv0t10 = sv0_string_char_at(src, j);
          int d = _sv0t10;
          int _sv0t11 = (d >= 97);
          int _sv0t12 = (d <= 122);
          ...
          int _sv0t17 = (_sv0t13 || _sv0t16);
          int _sv0t18 = (d == 95);
          int _sv0t19 = (_sv0t17 || _sv0t18);
          int _sv0t20 = (d >= 48);
          int _sv0t21 = (d <= 57);
          int _sv0t22 = (_sv0t20 && _sv0t21);
          if ((_sv0t19 || _sv0t22)) {
            j = (j + 1);
          } else {
            break;
        ...
        tag = 5;
        int _sv0t23 = (j - i);
        const char* _sv0t24 = sv0_string_substr(src, i, _sv0t23);
        const char* w;
        w = _sv0t24;
        int _sv0t25 = sv0_string_eq(w, "fn");
        if (_sv0t25) {
          tag = 65;
        } else {
        }
        int _sv0t26 = sv0_string_eq(w, "struct");
        if (_sv0t26) {
          tag = 89;
        } else {
        }
        int _sv0t27 = sv0_string_eq(w, "let");
        if (_sv0t27) {
          tag = 71;
        } else {
        }
        int _sv0t28 = sv0_string_eq(w, "mut");
        if (_sv0t28) {
          tag = 77;
        } else {
        }
        int _sv0t29 = sv0_string_eq(w, "return");
        if (_sv0t29) {
          tag = 85;
        } else {
        }
        ...
        if ((_sv0t30 && _sv0t31)) {
          int fl = 0;
          while ((j < n)) {
            int _sv0t32 = sv0_string_char_at(src, j);
            int d = _sv0t32;
            if ((d == 46)) {
              fl = 1;
              j = (j + 1);
              ...
              int _sv0t33 = (d >= 48);
              int _sv0t34 = (d <= 57);
              int _sv0t35 = (_sv0t33 && _sv0t34);
              int _sv0t36 = (d >= 97);
              int _sv0t37 = (d <= 122);
              int _sv0t38 = (_sv0t36 && _sv0t37);
              if ((_sv0t35 || _sv0t38)) {
                j = (j + 1);
              } else {
                break;
          ...
          if ((c == 45)) {
            if ((j < n)) {
              int _sv0t39 = sv0_string_char_at(src, j);
              if ((_sv0t39 == 62)) {
                tag = 19;
                j = (j + 1);
              } else {
...
static int lower_vecf_test_site(const char* src, const char* want, int occ, int mode, int expect) {
  ...
  int _sv0t2 = sv0_vec_new();
  int en = _sv0t2;
  int _sv0t3 = lower_vecf_test_lex(src, tags, st, en);
  int _l = _sv0t3;
  int _sv0t4 = lower_vecf_site_map(tags, src, st, en);
  int m = _sv0t4;
  int seen = 0;
  int k = 0;
  while (1) {
    int _sv0t5 = sv0_vec_len(tags);
    int _sv0t18 = (k < _sv0t5);
    if ((!_sv0t18)) {
      break;
    } else {
    }
    int _sv0t6 = lower_vecf_text_is(src, st, en, k, want);
    if (_sv0t6) {
      int hit = 0;
      if ((mode == 0)) {
        int _sv0t7 = (k + 1);
        int _sv0t8 = lower_vecf_tag_at(tags, _sv0t7);
        if ((_sv0t8 == 6)) {
          hit = 1;
        } else {
        }
      ...
      if ((mode == 1)) {
        int _sv0t9 = (k + 1);
        int _sv0t10 = lower_vecf_tag_at(tags, _sv0t9);
        if ((_sv0t10 == 10)) {
          hit = 1;
        } else {
        }
      ...
      if ((mode == 3)) {
        int _sv0t11 = (k + 1);
        int _sv0t12 = lower_vecf_tag_at(tags, _sv0t11);
        if ((_sv0t12 == 10)) {
          int _sv0t13 = (k - 1);
          int _sv0t14 = lower_vecf_tag_at(tags, _sv0t13);
          if ((_sv0t14 == 16)) {
            hit = 1;
          } else {
          }
      ...
      if (hit) {
        if ((seen == occ)) {
          ...
          int _sv0t15 = sv0_vec_get(m, at);
          if ((_sv0t15 != expect)) {
            int _sv0t16 = sv0_vec_get(m, at);
            int _sv0t17 = (1000 + _sv0t16);
            return _sv0t17;
          } else {
          }
          return 0;
...
static int test_lower_vecf_site_table(void) {
  int _sv0t0 = lower_vecf_test_site("fn f(v: Vec<f64>) -> f64 { return vec_get(v, 0); }", "vec_get", 0, 0, 1);
  int r0 = _sv0t0;
  if ((r0 != 0)) {
    return 1;
  } else {
  }
  int _sv0t1 = lower_vecf_test_site("fn f() -> f64 { let v: Vec<f64> = vec_new(); vec_push(v, 1.5); return 0.0; }", "vec_push", 0, 0, 1);
  int r1 = _sv0t1;
  if ((r1 != 0)) {
    return 2;
  } else {
  }
  int _sv0t2 = lower_vecf_test_site("fn f() -> i32 { let v: Vec<f64> = vec_new(); let v: Vec<i32> = vec_new(); return vec_get(v, 0); }", "vec_get", 0, 0, 2);
  int r2 = _sv0t2;
  if ((r2 != 0)) {
    return 3;
  } else {
  }
  int _sv0t3 = lower_vecf_test_site("fn f(v: Vec<i32>) -> i32 { return vec_get_f64(v, 0); }", "vec_get_f64", 0, 0, 3);
  int r3 = _sv0t3;
  if ((r3 != 0)) {
    return 4;
  } else {
  }
  int _sv0t4 = lower_vecf_test_site("struct B { d: Vec<f64>, n: i32 } fn f(b: B) -> f64 { return vec_get(b.d, 0); }", "vec_get", 0, 0, 1);
  int r4 = _sv0t4;
  if ((r4 != 0)) {
    return 5;
  } else {
  }
  int _sv0t5 = lower_vecf_test_site("fn m() -> Vec<f64> { return vec_new(); } fn f() -> f64 { let w = m(); return vec_get(w, 0); }", "vec_get", 0, 0, 1);
  int r5 = _sv0t5;
  if ((r5 != 0)) {
    return 6;
  } else {
  }
  int _sv0t6 = lower_vecf_test_site("fn f(v: Vec<i32>) -> i32 { return vec_get(v, 0); }", "vec_get", 0, 0, 0);
  int r6 = _sv0t6;
  if ((r6 != 0)) {
    return 7;
  } else {
  }
  int _sv0t7 = lower_vecf_test_site("fn a(v: Vec<f64>) -> i32 { return 0; } fn b(v: Vec<i32>) -> i32 { return vec_get(v, 0); }", "vec_get", 0, 0, 0);
  int r7 = _sv0t7;
  if ((r7 != 0)) {
    return 8;
  } else {
  }
  int _sv0t8 = lower_vecf_test_site("fn f() -> f64 { let v = vec_new(); vec_push(v, 1.5); return vec_get(v, 0); }", "vec_get", 0, 0, 1);
  int r8 = _sv0t8;
  if ((r8 != 0)) {
    return 9;
  } else {
  }
  int _sv0t9 = lower_vecf_test_site("fn f() -> i32 { let v = vec_new(); vec_push(v, 2); return vec_get(v, 0); }", "vec_get", 0, 0, 0);
  int r9 = _sv0t9;
  if ((r9 != 0)) {
    return 10;
  } else {
  }
  int _sv0t10 = lower_vecf_test_site("fn f() -> i32 { let v = vec_new(); vec_push(v, 1.5); vec_push(v, 2); return vec_get(v, 0); }", "vec_get", 0, 0, 2);
  int r10 = _sv0t10;
  if ((r10 != 0)) {
    return 11;
  } else {
  }
  int _sv0t11 = lower_vecf_test_site("fn f(x: f64) -> f64 { let v = vec_new(); vec_push(v, x); let y: f64 = vec_get(v, 0); return y; }", "vec_push", 0, 0, 1);
  int r11 = _sv0t11;
  if ((r11 != 0)) {
    return 12;
  } else {
  }
  int _sv0t12 = lower_vecf_test_site("fn f() -> i32 { let v = vec_new(); vec_push(v, 0); vec_set(v, 0, 2.5); return 0; }", "vec_push", 0, 0, 2);
  int r12 = _sv0t12;
  if ((r12 != 0)) {
    return 13;
  } else {
  }
  int _sv0t13 = lower_vecf_test_site("fn g(w: Vec<f64>) -> i32 { return 0; } fn f() -> i32 { let v = vec_new(); let n = g(v); return vec_get(v, 0); }", "vec_get", 0, 0, 1);
  int r13 = _sv0t13;
  if ((r13 != 0)) {
    return 14;
  } else {
  }
  int _sv0t14 = lower_vecf_test_site("fn g(k: i32, w: Vec<f64>) -> i32 { return 0; } fn f() -> i32 { let v = vec_new(); let n = g(3, v); return vec_get(v, 0); }", "vec_get", 0, 0, 1);
  int r14 = _sv0t14;
  if ((r14 != 0)) {
    return 15;
  } else {
  }
  int _sv0t15 = lower_vecf_test_site("fn g(w: Vec<i32>) -> i32 { return 0; } fn f() -> i32 { let v = vec_new(); let n = g(v); return vec_get(v, 0); }", "vec_get", 0, 0, 0);
  int r15 = _sv0t15;
  if ((r15 != 0)) {
    return 16;
  } else {
  }
  int _sv0t16 = lower_vecf_test_site("fn f(x: f64) -> Vec<f64> { let v = vec_new(); vec_push(v, x); return v; }", "vec_push", 0, 0, 1);
  int r16 = _sv0t16;
  if ((r16 != 0)) {
    return 17;
  } else {
  }
  int _sv0t17 = lower_vecf_test_site("struct B { d: Vec<f64> } fn f(x: f64) -> B { let v = vec_new(); vec_push(v, x); return B { d: v }; }", "vec_push", 0, 0, 1);
  int r17 = _sv0t17;
  if ((r17 != 0)) {
    return 18;
  } else {
  }
  int _sv0t18 = lower_vecf_test_site("fn f(x: i32) -> i32 { let v = vec_new(); vec_push(v, x); return vec_get(v, 0); }", "vec_get", 0, 0, 0);
  int r18 = _sv0t18;
  if ((r18 != 0)) {
    return 19;
  } else {
  }
  int _sv0t19 = lower_vecf_test_site("fn f(s: &[f64]) -> f64 { return s[0]; }", "s", 0, 1, 1);
  int r19 = _sv0t19;
  if ((r19 != 0)) {
    return 20;
  } else {
  }
  int _sv0t20 = lower_vecf_test_site("fn f() -> f64 { let a: [f64; 3] = [1.0, 2.0, 3.0]; return a[1]; }", "a", 0, 1, 1);
  int r20 = _sv0t20;
  if ((r20 != 0)) {
    return 21;
  } else {
  }
  int _sv0t21 = lower_vecf_test_site("fn f() -> f64 { let a: [f64; 3] = [1.0, 2.0, 3.0]; return a[1]; }", "=", 0, 2, 4);
  int r21 = _sv0t21;
  if ((r21 != 0)) {
    return 22;
  } else {
  }
  int _sv0t22 = lower_vecf_test_site("fn f() -> f64 { let b = [0.5, 1.5]; return b[0]; }", "b", 0, 1, 1);
  int r22 = _sv0t22;
  if ((r22 != 0)) {
    return 23;
  } else {
  }
  int _sv0t23 = lower_vecf_test_site("fn f() -> f64 { let b = [0.5, 1.5]; return b[0]; }", "=", 0, 2, 4);
  int r23 = _sv0t23;
  if ((r23 != 0)) {
    return 24;
  } else {
  }
  int _sv0t24 = lower_vecf_test_site("fn f(n: [i32; 3]) -> i32 { return n[0]; }", "n", 0, 1, 0);
  int r24 = _sv0t24;
  if ((r24 != 0)) {
    return 25;
  } else {
  }
  int _sv0t25 = lower_vecf_test_site("fn f() -> i32 { let n = [1, 2]; return n[0]; }", "=", 0, 2, 0);
  int r25 = _sv0t25;
  if ((r25 != 0)) {
    return 26;
  } else {
  }
  int _sv0t26 = lower_vecf_test_site("struct P { v: [f64; 2] } fn f(p: P) -> f64 { return p.v[0]; }", "v", 0, 3, 1);
  int r26 = _sv0t26;
  if ((r26 != 0)) {
    return 27;
  } else {
  }
  int _sv0t27 = lower_vecf_test_site("fn f(s: &mut [f64]) -> i32 { s[0] = 1.0; return 0; }", "s", 0, 1, 1);
  int r27 = _sv0t27;
  if ((r27 != 0)) {
    return 28;
  } else {
  }
  int _sv0t28 = lower_vecf_test_site("struct A { v: Vec<f64> } struct B { a: A } fn f(b: B) -> f64 { return vec_get(b.a.v, 0); }", "vec_get", 0, 0, 1);
  int rc0 = _sv0t28;
  if ((rc0 != 0)) {
    return 100;
  } else {
  }
  int _sv0t29 = lower_vecf_test_site("fn m() -> Vec<f64> { return vec_new(); } fn f() -> f64 { return vec_get(m(), 0); }", "vec_get", 0, 0, 1);
  int rc1 = _sv0t29;
  if ((rc1 != 0)) {
    return 101;
  } else {
  }
  int _sv0t30 = lower_vecf_test_site("fn m() -> Vec<i32> { return vec_new(); } fn f() -> i32 { return vec_get(m(), 0); }", "vec_get", 0, 0, 0);
  int rc2 = _sv0t30;
  if ((rc2 != 0)) {
    return 102;
  } else {
  }
  int _sv0t31 = lower_vecf_test_site("fn m(k: i32) -> Vec<f64> { return vec_new(); } fn f(x: Vec<i32>) -> f64 { return vec_get(m(vec_len(x)), 0); }", "vec_get", 0, 0, 1);
  int rc3 = _sv0t31;
  if ((rc3 != 0)) {
    return 103;
  } else {
  }
  int _sv0t32 = lower_vecf_test_site("fn m() -> Vec<i32> { return vec_new(); } fn f() -> f64 { return vec_get_f64(m(), 0); }", "vec_get_f64", 0, 0, 3);
  int rc4 = _sv0t32;
  if ((rc4 != 0)) {
    return 104;
  } else {
...
int main(void) {
  ...
  int _sv0t176 = test_lower_vecf_lookup_kinds();
  int rv8 = _sv0t176;
  if ((rv8 != 0)) {
    int _sv0t177 = (970 + 80);
    int _sv0t178 = (_sv0t177 + rv8);
    return _sv0t178;
  } else {
  }
  int _sv0t179 = test_lower_vecf_site_table();
  int rt = _sv0t179;
  if ((rt != 0)) {
    int _sv0t180 = (1200 + rt);
    return _sv0t180;
  } else {
  }
  return 0;
