...
- The **native** mega-TU VM emitter (**`scripts/build-sv0-megatu-vm-native.sh`** → **`build/sv0-megatu-vm-native`**) implements the same emit contract and is **byte-identical** to the SML **`--target=vm`** golden for every tier-2 program (span, token, bytecode, ast) as of **P4/D2**. **`scripts/sv0-vm-tier2-native-emitter.sh`** is the **`SV0_VM_BYTECODE_EMITTER`** wrapper for it (it builds the native emitter on demand, then re-emits one manifest path into **`build/vm/<stem>.sv0b`**).
- **`./scripts/sv0 vm-parity-tier2-emit`** runs the byte-parity leg standalone: it defaults **`SV0_VM_BYTECODE_EMITTER`** to that wrapper and **`cmp`**s each manifest path to its golden. Grow **`tier2-manifest.txt`** toward full **`manifest.txt`** as more programs reach byte-parity.
