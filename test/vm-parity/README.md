...
- The **native** mega-TU VM emitter (**`scripts/build-sv0-megatu-vm-native.sh`** → **`build/sv0-megatu-vm-native`**) implements the same emit contract and is **byte-identical** to the SML **`--target=vm`** golden for **17 of the 18 mega-TU compiler modules** as of **P4/D2** (driver closed via `prepool_loop_seq`); the 18th, **`lib/span.sv0`**, **diverges by design** since nested struct values got a real VM layout (`CompileError` has a struct-valued field the SML reference lays out as one word) and is excluded from `tier2-manifest.txt` — it still builds and runs on the VM, and the layout is gated behaviorally (`test/behavior/cases/nested_struct.sv0`). **`scripts/sv0-vm-tier2-native-emitter.sh`** is the **`SV0_VM_BYTECODE_EMITTER`** wrapper for it (it builds the native emitter on demand, then re-emits one manifest path into **`build/vm/<stem>.sv0b`**). The former SML-heap surrogate (`sv0-vm-tier2-emit-bootstrap.sh`) was a tautological SML-vs-SML check and is **retired** — the native emitter is the real gate.
